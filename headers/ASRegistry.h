/**
    ...
    \brief   AppleScript Registry constants.
...
enum {
  /** Add this parameter to a Get Data result if your app handled the 'as'
     parameter */
...
enum {
  ...
  kDialectBundleResType =
      FOUR_CHAR_CODE('Dbdl'), /** AppleScript Classes and Enums: */
...
enum {
  ...
  enumBooleanValues =
      FOUR_CHAR_CODE('boov'), /**  Use this instead of typeBoolean to avoid
                                 with/without conversion  */
  kAETrue = typeTrue,
  kAEFalse = typeFalse,
  enumMiscValues = FOUR_CHAR_CODE('misc'),
  kASCurrentApplication =
      FOUR_CHAR_CODE('cura'), /** User-defined property ospecs: */
  formUserPropertyID = FOUR_CHAR_CODE('usrp')
};

enum {
  cString =
      cStringClass /** old name for cStringClass - can't be used in .r files*/
};
...
enum {
  ...
  pLangCode =
      FOUR_CHAR_CODE('plcd'), /** Magic Tell and End Tell events for logging: */
