/**
    ...
    \brief   Network Setup Interfaces
...
enum {
  kCfgErrDatabaseChanged =
      -3290, /** database has changed since last call - close and reopen DB*/
  ...
  kCfgErrEntityAlreadyExists =
      -3296, /** An entity with this name already exists*/
  kCfgErrPrefsTypeNotFound =
      -3297, /** A preference with this prefsType doesn't exist*/
  kCfgErrDataTruncated = -3298, /** Data truncated when read buffer too small*/
  kCfgErrFileCorrupted =
      -3299, /** The database format appears to be corrupted.*/
...
/**    -------------------------------------------------------------------------
    ...
    Use of different classes allow to store type of information in the same
   database
 ...
 *    \non_carbon_cfm   in CfgOpenTpt 1.0 and later
...
/**
    ...
    Opens Network Setup for a given client. This call should be made prior to
   any other call.
*/
/**
 *  OTCfgCloseDatabase()
...
/**
    ...
    Outputs:    CfgDatabaseRef* dbRef           Reference to opened database is
   cleared Returns:    OSStatus                        *** list errors ***

    Closes Network Setup for a given client. This call should be made when the
   client no longer wants to use Network Setup.
*/
/**    -------------------------------------------------------------------------
    Area management
    ------------------------------------------------------------------------- */
 ...
 */
EXTERN_API(OSStatus)
OTCfgGetAreasCount(CfgDatabaseRef dbRef, ItemCount *itemCount);
 ...
 */
EXTERN_API(OSStatus)
OTCfgGetAreasList(CfgDatabaseRef dbRef, ItemCount *itemCount,
...
/**
    ...
    Inputs:     CfgDatabaseRef dbRef            Reference to opened database
                ...
                CfgAreaID areaID[]              Pointer to array of *itemCount
   area IDs Str255 areaName[]               Pointer to array of *itemCount area
   names Outputs:    ItemCount* itemCount            Number of entities defined
                CfgAreaID areaID[]              Filled in array of area IDs
                Str255 areaName[]               Filled in array of area names
    Returns:    OSStatus                        *** list errors ***

    Returns a list of area IDs and names. On entry, count should be set to
   whatever OTCfgGetAreasCount returned.  On exit, count contains the actual
   number of areas found. This can be less than the initial count value if areas
   were deleted in the meantime.  The id and name parameters are stored in
   arrays that should each be able to contain count values.
*/
/**
 *  OTCfgGetCurrentArea()
 *
 ...
 */
EXTERN_API(OSStatus)
OTCfgGetCurrentArea(CfgDatabaseRef dbRef, CfgAreaID *areaID);
 ...
 */
EXTERN_API(OSStatus)
OTCfgSetCurrentArea(CfgDatabaseRef dbRef, CfgAreaID areaID);
 ...
 */
EXTERN_API(OSStatus)
OTCfgCreateArea(CfgDatabaseRef dbRef, ConstStr255Param areaName,
                CfgAreaID *areaID);
...
/**
    ...
    Creates a new area with the specified name. Then name must be unique or
   kCfgErrAreaAlreadyExists will be returned.
*/
/**
 *  OTCfgDeleteArea()
 *
 ...
 */
EXTERN_API(OSStatus)
OTCfgDeleteArea(CfgDatabaseRef dbRef, CfgAreaID areaID);
 ...
 */
EXTERN_API(OSStatus)
OTCfgDuplicateArea(CfgDatabaseRef dbRef, CfgAreaID sourceAreaID,
 ...
 */
EXTERN_API(OSStatus)
OTCfgSetAreaName(CfgDatabaseRef dbRef, CfgAreaID areaID,
                 ConstStr255Param areaName, CfgAreaID *newAreaID);
 ...
 *    \non_carbon_cfm   in CfgOpenTpt 1.0.1 and later
 *    \carbon_lib        not available
 *    \mac_os_x         not available
 */
EXTERN_API(OSStatus)
OTCfgGetAreaName(CfgDatabaseRef dbRef, CfgAreaID areaID, Str255 areaName);
 ...
 */
EXTERN_API(OSStatus)
OTCfgOpenArea(CfgDatabaseRef dbRef, CfgAreaID areaID);
 ...
 */
EXTERN_API(OSStatus)
OTCfgCloseArea(CfgDatabaseRef dbRef, CfgAreaID areaID);
 ...
 */
EXTERN_API(OSStatus)
OTCfgBeginAreaModifications(CfgDatabaseRef dbRef, CfgAreaID readAreaID,
...
/**
    ...
    Outputs:    CfgAreaID* writeAreaID          ID of area opened for
   modification Returns:    OSStatus                        *** list errors ***
 ...
 */
EXTERN_API(OSStatus)
OTCfgCommitAreaModifications(CfgDatabaseRef dbRef, CfgAreaID readAreaID,
                             CfgAreaID writeAreaID);
...
/**
    ...
    Inputs:     CfgDatabaseRef dbRef            Reference to opened database
                CfgAreaID readAreaID            ID of area opened for reading
                CfgAreaID writeAreaID           ID of area opened for
   modification Outputs:    none Returns:    OSStatus                        ***
   list errors ***

    Closes an area opened for writing.  All modifications are committed and
 ...
 */
EXTERN_API(OSStatus)
OTCfgAbortAreaModifications(CfgDatabaseRef dbRef, CfgAreaID readAreaID);
...
/**
    ...
    Entities can be manipulated as soon as an area has been opened.  The same
   calls work both for areas opened for reading or for modification. In the
   latter case, the calls can be used on the original or new area id to access
   the original data or the modified data.
*/
/**
    For everybody
 ...
 */
EXTERN_API(OSStatus)
OTCfgGetEntitiesCount(CfgDatabaseRef dbRef, CfgAreaID areaID,
                      CfgEntityClass entityClass, CfgEntityType entityType,
                      ItemCount *itemCount);
...
/**
    ...
    count as output, receives the actual number of entities or the number you
   specified.
*/
/**
 ...
 */
EXTERN_API(OSStatus)
OTCfgGetEntitiesList(CfgDatabaseRef dbRef, CfgAreaID areaID,
                     CfgEntityClass entityClass, CfgEntityType entityType,
                     ItemCount *itemCount,
...
/**
    ...
    Inputs:     CfgDatabaseRef dbRef            Reference to opened database
                ...
                CfgEntityRef entityRef[]        Pointer to array of *itemCount
   entity refs CfgEntityInfo entityInfo[]      Pointer to array of *itemCount
   entity infos Outputs:    ItemCount* itemCount            Count of entities
   listed CfgEntityRef entityRef[]        Filled in array of *itemCount entity
   refs CfgEntityInfo entityInfo[]      Filled in array of *itemCount entity
   infos Returns:    OSStatus                        *** list errors ***

    Returns the list of entities of the specified class and type in the
   specified area. To obtain all entities regardless of their class or type pass
 ...
 */
EXTERN_API(OSStatus)
OTCfgCreateEntity(CfgDatabaseRef dbRef, CfgAreaID areaID,
                  const CfgEntityInfo *entityInfo, CfgEntityRef *entityRef);
...
/**
    ...
    Inputs:     CfgDatabaseRef dbRef            Reference to opened database
                ...
                CfgEntityInfo* entityInfo       Information that defines the
   entity Outputs:    CfgEntityRef* entityRef         Reference to entity
 ...
 */
EXTERN_API(OSStatus)
OTCfgDeleteEntity(CfgDatabaseRef dbRef, const CfgEntityRef *entityRef);
...
/**
    ...
    Deletes the specified entity. If there is no entity with this id,
 ...
 */
EXTERN_API(OSStatus)
OTCfgDuplicateEntity(CfgDatabaseRef dbRef, const CfgEntityRef *entityRef,
 ...
 */
EXTERN_API(OSStatus)
OTCfgSetEntityName(CfgDatabaseRef dbRef, const CfgEntityRef *entityRef,
                   ConstStr255Param entityName, CfgEntityRef *newEntityRef);
 ...
 *    \non_carbon_cfm   in CfgOpenTpt 1.1 and later
 *    \carbon_lib        not available
 *    \mac_os_x         not available
 */
EXTERN_API(void)
OTCfgGetEntityArea(const CfgEntityRef *entityRef, CfgAreaID *areaID);

/**
    OTCfgGetEntityArea()

    Inputs:     CfgEntityRef *entityRef         Reference to an entity
    Outputs:    CfgAreaID *areaID               ID of area that contains the
   entity Returns:    none
 ...
 */
EXTERN_API(void)
OTCfgGetEntityName(const CfgEntityRef *entityRef, Str255 entityName);
 ...
 *    \non_carbon_cfm   in CfgOpenTpt 1.2 and later
 *    \carbon_lib        not available
 *    \mac_os_x         not available
 */
EXTERN_API(OSStatus)
OTCfgGetEntityLogicalName(CfgDatabaseRef dbRef, const CfgEntityRef *entityRef,
                          Str255 entityName);
 ...
 */
EXTERN_API(void)
OTCfgChangeEntityArea(CfgEntityRef *entityRef, CfgAreaID newAreaID);

/**
    OTCfgChangeEntityArea()

    Inputs:     CfgEntityRef *entityRef         Reference to an entity
                CfgAreaID newAreaID             ID of area to contain moved
   entity Outputs:    none Returns:    none
...
/**
    ...
    Compare two entity references. If ignoreArea is true, and the two entity
   names are the same, then return true. If ignoreArea is false, then the area
   IDs must be the same, as well as the entity names must be the same, then can
   return true.
*/
/**
 *  OTCfgIsSameAreaID()
 ...
 */
EXTERN_API(Boolean)
OTCfgIsSameAreaID(CfgAreaID areaID1, CfgAreaID areaID2);
...
/**
    ...
    Compare two area IDs. Return true for matching area IDs, and return false
   for the different area IDs.
*/
/**    -------------------------------------------------------------------------
...
/**    -------------------------------------------------------------------------
    Open Preferences
    if writer = true, GetPrefs and SetPrefs are allowed, else only GetPrefs is
   allowed.
    ------------------------------------------------------------------------- */
/**
 *  OTCfgOpenPrefs()
 ...
 */
EXTERN_API(OSStatus)
OTCfgOpenPrefs(CfgDatabaseRef dbRef, const CfgEntityRef *entityRef,
               Boolean writer, CfgEntityAccessID *accessID);
...
/**
    ...
    Open the specified entity and return the CfgEntityAccessID for the following
   access of the content of the entity. If writer is true, CfgGetPrefs and
   CfgSetPrefs are allowed, otherwise only CfgGetPrefs is allowed.
*/
/**
 *  OTCfgClosePrefs()
 *
...
/**    -------------------------------------------------------------------------
    ...
    These API calls are for the protocol developers. It supports multiple
   preferences per entity. Each preference is identified by an OSType (typically
   called prefsType). The structure of the preference data is protocol stack
   dependent.
    ------------------------------------------------------------------------- */
/**
 *  OTCfgSetPrefs()
 ...
 */
EXTERN_API(OSStatus)
OTCfgSetPrefs(CfgEntityAccessID accessID, OSType prefsType, const void *data,
              ByteCount length);
...
/**
    ...
    Write the data to the specified preference, creating the preference if
   necessary. The preference is identified by the prefsType. If the entity is
   not opened for the writer, an error code is returned.
*/
#endif /** CALL_NOT_IN_CARBON */
 ...
 */
EXTERN_API(OSStatus)
OTCfgDeletePrefs(CfgEntityAccessID accessID, OSType prefsType);
...
/**
    ...
    Delete the specified preference. The preference is identified by the
   prefsType.
*/
#endif /** CALL_NOT_IN_CARBON */
 ...
 */
EXTERN_API(OSStatus)
OTCfgGetPrefs(CfgEntityAccessID accessID, OSType prefsType, void *data,
              ByteCount length);
...
/**
    ...
    Inputs:     CfgEntityAccessID* accessID     ID of entity to access
                ...
                ByteCount length                Number of bytes of data
   requested Outputs:    none Returns:    OSStatus                        ***
   list errors ***

    Read the data from the specified preference to the passed buffer. The
   preference is identified by the prefsType. If the passed buffer is too small,
   kCfgErrDataTruncated is returned, but will copy as many data as possible to
   the buffer.
*/
/**
 *  OTCfgGetPrefsSize()
 ...
 */
EXTERN_API(OSStatus)
OTCfgGetPrefsSize(CfgEntityAccessID accessID, OSType prefsType,
                  ByteCount *length);
...
/**
    ...
    Inputs:     CfgEntityAccessID* accessID     ID of entity to access
                OSType prefsType                Preference type to get
                ByteCount length                Number of bytes of data
   available Outputs:    none Returns:    OSStatus                        ***
   list errors ***

    Returns the length, in bytes, of the specified preference. The preference is
   identified by the prefsType.
*/
#endif /** CALL_NOT_IN_CARBON */
 ...
 */
EXTERN_API(OSStatus)
OTCfgGetTemplate(CfgEntityClass entityClass, CfgEntityType entityType,
                 OSType prefsType, void *data, ByteCount *dataSize);
...
/**
    ...
    Outputs:    void*       data                template for the preference
   class/type/prefsType ByteCount*  dataSize            actual length of
   template Returns:    OSStatus                        *** list errors ***

    This routine returns the template for the specified preference class, type,
   and prefsType.
*/
/**
    Due to a human error, OTCfgGetDefault was published with the entity type and
   class parameters reversed, with respect to the other calls that take these
   same parameters.  Please, use OTCfgGetTemplate instead.
*/
#endif /** CALL_NOT_IN_CARBON */
 ...
 */
EXTERN_API(Handle)
OTCfgGetDefault(OSType entityType, OSType entityClass, OSType prefsType);
...
/**
    ...
    Inputs:     OSType  entityType              entityType
                ...
                OSType  prefsType               prefsType
    Outputs:    none
    Returns:    Handle                          default preference for the
   preference type/class/prefsType

    This routine returns the default preference value for the specified
   preference type, class, and prefsType.
*/
/**    -------------------------------------------------------------------------
 ...
 */
EXTERN_API(OSStatus)
OTCfgGetPrefsTOCCount(CfgEntityAccessID accessID, ItemCount *itemCount);
 ...
 */
EXTERN_API(OSStatus)
OTCfgGetPrefsTOC(CfgEntityAccessID accessID, ItemCount *itemCount,
...
/**
    ...
    Inputs:     CfgEntityAccessID* accessID     ID of entity to access
                ...
                CfgPrefsHeader prefsTOC[]       Pointer to array of *itemCount
   pref headers Outputs:    ItemCount* itemCount            Number of entries
   returned CfgPrefsHeader prefsTOC[]       Filled in array of *itemCount pref
   headers Returns:    OSStatus                        *** list errors ***

    Get the list of all the preference in the entity. Return the number of
   ...
   that:
     ...
     a) always call OTCfgGetPrefsTOCCount before calling this routine and then
   allocate an appropriate sized array to pass to prefsTOC b) before calling
   this routine, always set up itemCount to the size of the prefsTOC array so
   that your code will run on future systems
*/
/**    -------------------------------------------------------------------------
...
typedef CALLBACK_API(void, OTCfgNotifyProcPtr)(void *contextPtr, UInt32 code,
                                               SInt32 result, void *cookie);
 ...
 */
EXTERN_API(OSStatus)
OTCfgInstallNotifier(CfgDatabaseRef dbRef, CfgEntityClass theClass,
                     CfgEntityType theType, OTCfgNotifyProcPtr notifier,
...
/**
    ...
    Inputs:     CfgDatabaseRef dbRef            Reference to opened database
                CfgEntityClass theClass         receives notifications about
   changes to this class CfgEntityType theType           receives notifications
   about changes to this type OTCfgNotifyProcPtr notifier     address of
   callback routine void* contextPtr                refCon for callback routine
    Outputs:    none
    Returns:    OSStatus                        *** list errors ***

    Installs a notifier that Network Setup will call when interesting things
   happen.
 ...
 */
EXTERN_API(OSStatus)
OTCfgRemoveNotifier(CfgDatabaseRef dbRef, CfgEntityClass theClass,
                    CfgEntityType theType);
...
/**
    ...
    Requires Network Setup 1.1 or higher.
*/
/** Constants for the code parameter of the notifier installed by
 * OTCfgInstallNotifier.*/

#endif /** CALL_NOT_IN_CARBON */

enum {
  kOTCfgDatabaseChanged =
      0x10000000 +
      0 /** result will be kCfgErrDatabaseChanged, cookie is meaningless*/
};
 ...
 */
EXTERN_API(SInt16)
OTCfgEncrypt(UInt8 *key, UInt8 *data, SInt16 dataLen);
...
/**
    ...
    Encrypt the password, using the user name as the encryption key.  Return the
   encrypted password and its length.
 ...
 */
EXTERN_API(SInt16)
OTCfgDecrypt(UInt8 *key, UInt8 *data, SInt16 dataLen);
...
/**
    ...
    Decrypt the password, using the user name as the encryption key.  Return the
   decrypted password and its length.
...
enum {
  ...
  kOTCfgProtocolUserVisibleNamePref =
      FOUR_CHAR_CODE('prot'), /** C string (TCP/IP = "tcp", AppleTalk = "ddp",
                                 n/a for others)*/
  kOTCfgAdminPasswordPref = FOUR_CHAR_CODE('pwrd'), /** not to be documented*/
  kOTCfgProtocolOptionsPref =
      FOUR_CHAR_CODE('opts') /** UInt32, protocol specific flags*/
};
...
enum {
  kOTCfgUserModePref =
      FOUR_CHAR_CODE('ulvl'), /** OTCfgUserMode, TCP/IP and AppleTalk only*/
  kOTCfgPrefWindowPositionPref = FOUR_CHAR_CODE(
      'wpos') /** Point, global coordinates, TCP/IP, AppleTalk, Infrared only*/
};
...
enum {
  ...
  kOTCfgAdminUserMode = 3 /** admin mode is never valid in a preference, here
...
enum {
  ...
  kOTCfgATalkPortDeviceTypePref =
      FOUR_CHAR_CODE('ptfm') /** OTCfgATalkPortDeviceType*/
};
...
enum {
  kOTCfgATalkNetworkArchitecturePref =
      FOUR_CHAR_CODE('neta') /** OTCfgATalkNetworkArchitecture*/
};
...
struct OTCfgATalkGeneralDDP {
  ...
  OTCfgATalkUnloadOptions
      fLoadType;   /** whether AppleTalk is active, see the discussion above*/
  ...
  UInt16 fRTMPRequestInterval; /** ms, must be 200, ignored by current versions
                                  of OT*/
  UInt32 fAddressGenLimit;     /** default of 250*/
  UInt32 fBRCAgingInterval;    /** ms, must be 40000, BRC = best routing cache,
                                  ignored by current versions of OT*/
  UInt32 fRTMPAgingInterval; /** ms, must be 50000, ignored by current versions
                                of OT*/
...
struct OTCfgATalkGeneralATP {
  ...
  UInt8 fDefaultReleaseTimer; /** default of 0, same format as ATP_OPT_RELTIMER
                                 option*/
...
struct OTCfgATalkGeneralADSP {
  ...
  UInt32 fDefaultRecvWindow;        /** bytes, must be 27648, ignored by current
                                       versions of OT*/
...
struct OTCfgATalkGeneralASP {
  ...
  UInt32 fDefaultTickleInterval; /** ms, must be 30000, ignored by current
                                    versions of OT*/
  UInt8
      fDefaultTickleRetries; /** must be 8, ignored by current versions of OT*/
  UInt8 fDefaultReplies;     /** must be 8, ignored by current versions of OT*/
};
...
struct OTCfgATalkGeneral {
  ...
  UInt32 fPort;     /** a reference to the port over which this configuration is
                       running*/
...
/**
   OTCfgATalkPortDeviceType holds either the OT device type (eg
   kOTEthernetDevice) or an ADEV ID for the current port.  It is not used by the
   AppleTalk protocol stack, but it is used by the AppleTalk control panel.
*/
...
struct OTCfgIRGeneral {
  ...
  OTCfgIRPortSetting
      fPortSetting; /** IrDA or IRTalk, use constants defined above*/
...
enum {
  kOTCfgTCPInterfacesPref =
      FOUR_CHAR_CODE('iitf'), /** OTCfgTCPInterfaces (packed)*/
  ...
  kOTCfgTCPSearchListPref =
      FOUR_CHAR_CODE('ihst'), /** OTCfgTCPSearchList (packed)*/
  kOTCfgTCPDNSServersListPref =
      FOUR_CHAR_CODE('idns'), /** OTCfgTCPDNSServersList*/
  kOTCfgTCPSearchDomainsPref =
      FOUR_CHAR_CODE('isdm'), /** OTCfgTCPSearchDomains*/
  kOTCfgTCPDHCPLeaseInfoPref =
      FOUR_CHAR_CODE('dclt'), /** OTCfgTCPDHCPLeaseInfo*/
  kOTCfgTCPDHCPClientIDPref =
      FOUR_CHAR_CODE('dcid'), /** DHCP client ID, Pascal string*/
  kOTCfgTCPUnloadAttrPref = FOUR_CHAR_CODE('unld'), /** OTCfgTCPUnloadAttr*/
  kOTCfgTCPLocksPref = FOUR_CHAR_CODE('stng'),      /** OTCfgTCPLocks*/
  kOTCfgTCPPushBelowIPPref = FOUR_CHAR_CODE(
      'crpt'), /** single module to push below IP, Pascal string*/
  kOTCfgTCPPushBelowIPListPref =
      FOUR_CHAR_CODE('blip') /** list of modules to push below IP (Mac OS 9.0
                                and higher), 'STR#' format*/
};
...
/**
    This structure also contains an IP address and subnet mask that are not
   aligned on a four byte boundary. In order to avoid compiler warnings, and the
   possibility of code that won't work, these fields are defined here as four
   character arrays. It is suggested that BlockMoveData be used to copy to and
   from a field of type InetHost.
*/
...
struct OTCfgTCPSearchList {
  ...
  UInt8 fLocalDomainName[256];  /** in the file, these strings are packed*/
...
struct OTCfgTCPSearchDomains {
  UInt16 fCount;
  Str255 fFirstSearchDomains; /** subsequent search domains are packed after
                                 this one*/
};
...
enum {
  ...
  kOTCfgDontShutDownOnARPCollisionMask =
      0x0002, /** To be able to Disable ARP Collision ShutDown */
  kOTCfgDHCPInformMask = 0x0004, /** Enables DHCPINFORM instead of DHCPREQUEST*/
  kOTCfgOversizeOffNetPacketsMask =
      0x0008, /** With PMTU off, don't limit off-net packet to 576 bytes*/
  kOTCfgDHCPDontPreserveLeaseMask =
      0x0010 /** Turns off DHCP INIT-REBOOT capability.*/
};
...
enum {
  ...
  kOTCfgModemAdminPasswordPref =
      FOUR_CHAR_CODE('mdpw') /** not to be documented*/
};
...
enum {
  kOTCfgModemApplicationPref =
      FOUR_CHAR_CODE('mapt') /** OTCfgModemApplication*/
};
...
struct OTCfgModemLocks {
  ...
  UInt32
      dialing; /** the pulse/tone dial setting is locked (1) or not locked (0)*/
};
...
struct OTCfgModemApplication {
  ...
  OTCfgUserMode32 userMode; /** must be kOTCfgBasicUserMode, Modem control panel
                               has no "advance" mode*/
};
...
enum {
  ...
  kOTCfgRemoteAddressPref =
      FOUR_CHAR_CODE('cadr'), /** 'TEXT' format, max 255 characters, see also
                                 OTCfgRemoteConnect*/
  kOTCfgRemoteChatPref =
      FOUR_CHAR_CODE('ccha'), /** 'TEXT', see also OTCfgRemoteConnect*/
  kOTCfgRemoteDialingPref = FOUR_CHAR_CODE('cdia'), /** OTCfgRemoteDialing*/
  kOTCfgRemoteAlternateAddressPref =
      FOUR_CHAR_CODE('cead'), /** OTCfgRemoteAlternateAddress*/
  kOTCfgRemoteClientLocksPref =
      FOUR_CHAR_CODE('clks'), /** OTCfgRemoteClientLocks*/
  kOTCfgRemoteClientMiscPref =
      FOUR_CHAR_CODE('cmsc'),                       /** OTCfgRemoteClientMisc*/
  ...
  kOTCfgRemoteDialAssistPref =
      FOUR_CHAR_CODE('dass'),                    /** OTCfgRemoteDialAssist*/
  ...
  kOTCfgRemoteLogOptionsPref =
      FOUR_CHAR_CODE('logo'),                        /** OTCfgRemoteLogOptions*/
  ...
  kOTCfgRemoteSecurityDataPref =
      FOUR_CHAR_CODE('csec'), /** untyped data for external security modules*/
...
struct OTCfgRemoteARAP {
  ...
  char lowerLayerName[36]; /** C string, name of underlying modem port, must be
                              "Script"*/
};
...
struct OTCfgRemoteClientLocks {
  ...
  UInt32 errorCheck;     /** "Allow error correction and compression in modem"
                            checkbox is locked (1) or unlocked (0)*/
  UInt32 headerCompress; /** "Use TCP header compression" checkbox is locked (1)
                            or unlocked (0)*/
  UInt32 termWindow; /** "Connect to a command-line host" options are locked (1)
                        or unlocked (0)*/
  UInt32 reminder;   /** "Reminders" options are locked (1) or unlocked (0)*/
  UInt32 autoConn; /** "Connect automatically when starting TCP/IP applications"
                      checkbox is locked (1) or unlocked (0)*/
  UInt32 redial;   /** "Redialing" panel is locked (1) or unlocked (0)*/
  UInt32
      useProtocolLock; /** "Use protocol" popup is locked (1) or unlocked (0)*/
  UInt32 useVerboseLogLock;  /** "Use verbose logging" checkbox is locked (1) or
                                unlocked (0)*/
  UInt32 regUserOrGuestLock; /** "Register User"/"Guest" radio buttons are
                                locked (1) or unlocked (0)*/
  UInt32 dialAssistLock; /** "Use DialAssist" checkbox is locked (1) or unlocked
                            (0)*/
  UInt32 savePasswordLock;     /** "Save password" checkbox is locked (1) or
                                  unlocked (0)*/
  UInt32 useOpenStatusAppLock; /** "Open Status Application" checkbox is locked
                                  (1) or unlocked (0)*/
...
struct OTCfgRemoteClientMisc {
  UInt32 version;              /** see "Remote Access Versions" note, above*/
  UInt32 connectAutomatically; /** connect automatically when starting TCP/IP
                                  applications (1), or not (0)*/
};
...
enum {
  ...
  kOTCfgRemotePPPConnectScriptScript =
      2 /** use save chat script (OTCfgRemoteChat)*/
};
...
enum {
  ...
  kOTCfgRemoteProtocolAuto =
      3 /** auto-detect PPP or ARAP (not support in ARA 3.5 and above)*/
};
...
struct OTCfgRemoteConnect {
  ...
  UInt32 issueConnectedReminders;   /** issue Notification Manager reminders (1)
                                       or not (0)*/
  SInt32 reminderMinutes;           /** minutes, time between each reminder*/
  UInt32 connectManually;           /** must be 0*/
  UInt32 allowModemDataCompression; /** allow modem data compression (1) or not
                                       (0), only valid for PPP connections*/
  ...
  UInt32 passwordPtr;      /** run-time use only, initialise to 0, read ignore &
                              preserve*/
  UInt32 userNamePtr;      /** run-time use only, initialise to 0, read ignore &
                              preserve*/
  UInt32 addressLength;    /** length of phone number (OTCfgRemoteAddress)*/
  UInt32 addressPtr;       /** run-time use only, initialise to 0, read ignore &
                              preserve*/
  Str63 chatScriptName;    /** user-visible name of chat script*/
  UInt32 chatScriptLength; /** length of chat script (OTCfgRemoteChat)*/
  UInt32 chatScriptPtr;    /** run-time use only, initialise to 0, read ignore &
                              preserve*/
  UInt32 additional;       /** run-time use only, initialise to 0, read ignore &
                              preserve*/
...
struct OTCfgRemoteIPCP {
  ...
  UInt16 compressTCPHeaders; /** Van Jacobsen header compression allowed (1) or
                                not (0)*/
  UInt32 idleTimerMilliseconds; /** ms, if idleTimerEnabled, disconnect if idle
                                   for*/
};
...
struct OTCfgRemoteLogOptions {
  ...
  UInt32 additionalPtr; /** run-time use only, initialise to 0, read ignore &
                           preserve*/
  OTCfgRemoteLogLevel logLevel; /** constants are given above*/
  UInt32 launchStatusApp; /** 0 for not launch, 1 for launch, Remote Access 3.5
                             and higher */
...
struct OTCfgRemoteTerminal {
  ...
  Style fTypedCharStyle; /** style used for characters type,    default to bold
                            (2)*/
  Style fPrintedCharStyle; /** style used for characters printed, default to
                              normal (0)*/
  Style fEchoedCharStyle;  /** style used for characters echoed,  default to
                              italic (1)*/
...
struct OTCfgRemoteServer {
  UInt32 version;      /** see "Remote Access Versions" note, above*/
  SInt16 configCount;  /** number of active configurations, must be 1 for
                          personal server*/
  SInt16 configIDs[1]; /** array of IDs of active configurations, must have one
                          entry containing 0 for personal server*/
};
...
enum {
  ...
  kOTCfgAnswerModeTransfer =
      2, /** answering as a callback server, not valid for personal server*/
...
enum {
  ...
  kOTCfgNetProtoAny = (kOTCfgNetProtoIP |
                       kOTCfgNetProtoAT) /** allow connections of either type*/
};

typedef UInt8 OTCfgRemoteNetAccessMode;
enum {
  kOTCfgNetAccessModeUnrestricted =
      0, /** connected client can see things on server and things on server's
            network*/
  kOTCfgNetAccessModeThisMacOnly =
      1 /** connected client can only see things on server machine*/
};
...
struct OTCfgRemoteServerPort {
  UInt32 version; /** see "Remote Access Versions" note, above*/
  SInt16
      configID; /** ID of this port config, matches element of configIDs array
                   in OTCfgRemoteServer, must be 0 for personal server*/
  Str255 password; /** security zone bypass password, plain text*/
  OTCfgRemoteAnswerMode answerMode; /** see values defined above*/
  Boolean limitConnectTime; /** if true, following field limits duration of
                               client connections*/
  UInt8 pad;                /** must be 0*/
...
struct OTCfgRemoteApplication {
  UInt32 version; /** IMPORTANT: NOT a standard Remote Access version number,
                     must be 3 for ARA, 1 for OT/PPP*/
  Point fWindowPosition; /** global coordinates, window position*/
  UInt32 tabChoice; /** currently chosen tab in Options dialog, 1 for Redialing,
                       2 for Connection, 3 for Protocol*/
...
/**
   ...
   sake of source code compatibility, the previous definitions are still
   available, but you have to define the compiler variable OLDROUTINENAMES to
   get them. This is a temporary measure.  Apple will remove these definitions
   in a future version of this header file.  You should start transitioning your
   source code to the new definitions are soon as possible.
*/
...
enum {
  ...
  kOTCfgTypeTCPara =
      FOUR_CHAR_CODE('ipcp'), /** defining this as 'ipcp' makes no sense,*/
...
/** OLDROUTINENAMES*/
/**  This is your worst case, a fixed size structure, tacked on after a variable
 * length string.*/
...
#define kIITFPartP(h)                                                          \
  ((OTCfgIITFPrefsPart *)&(                                                    \
      (**((OTCfgIITFPrefs **)h))                                               \
          .fAppleTalkZone[(**((OTCfgIITFPrefs **)h)).fAppleTalkZone[0] + 1]))
...
/** OLDROUTINENAMES*/
typedef CALLBACK_API_C(void, RANotifyProcPtr)(void *contextPtr,
                                              OTCfgRemoteEventCode code,
                                              OSStatus result, void *cookie);
/** OLDROUTINENAMES*/
