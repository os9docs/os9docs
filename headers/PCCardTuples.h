/**
    ...
    \brief   List of PCMCIA tuple types and definitions of tuple contents.
...
#pragma pack(2)
  ...
  {
    Byte TPLDEV_TYPE_WPS_SPEED; /** Device Type Code: 7-4, WPS: 3,  Device Speed:
                                   2-0*/
    Byte deviceData[253];
  };
  ...
  enum
  {
    ...
    TPLDEV_WPS_MASK =
        0x08,                     /** write-protect switch mask for TPLDEV_TYPE_WPS_SPEED*/
    ...
    TPLDEV_SPEED_EXTENDED_MASK =
        0x80 /** bit set if more extended speed data follows*/
  };
  ...
  /**Multi-Function Tuple (06H)    - */

  struct LongLinkMFCTuple
  {
    UInt8 TPLMFC_NUM; /** Number of sets of config registers for individual
                         functions*/

    UInt8 TPLMFC_TAS1; /** CIS target address space for first function (00 = Attr,
                          01 = Common)*/
    UInt32
        TPLMFC_ADDR1; /** Target address of first CIS, unsigned long, lsb first*/

    UInt8 TPLMFC_TAS2;     /** CIS target address space for second function (00 =
                              Attr, 01 = Common)*/
    UInt8 TPLMFC_ADDR2[4]; /** [MISALIGNED!] Target address of second CIS,
                              unsigned long, lsb first*/

    /** the following fields are of undetermined length and should be calculated
     * at runtime*/
    /** */
    /** UInt8   TPLMFC_TASn;*/
    /** UInt32     TPLMFC_ADDRn;*/
  ...
  char TPLLV2_OEM[1];   /** vendor of software that formatted card,
                           zero-terminated*/
  char TPLLV2_INFO[1];  /** informational message about the card,
                           zero-terminated*/
  Byte reserved;        /** padding*/
};
...
struct FormatDiskTuple
{
  ...
  UInt32 TPLFMT_EDCLOC;  /**    location of error detection code, or zero if
                            interleaved*/
};
...
struct FormatMemTuple
{
  ...
  UInt32 TPLFMT_ADDRESS; /**    physical location at which this memory partion
                            must be mapped*/
  UInt32 TPLFMT_EDCLOC;  /**    location of error detection code, or zero if
                            interleaved*/
};
...
struct FormatTuple
{
  ...
  char TPLFMT_EDC;    /** error detection method and length of error detection
                         code*/
