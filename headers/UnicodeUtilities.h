/**
    ...
    \brief   Types, constants, prototypes for Unicode Utilities (Unicode input
   and text utils)
...
/**
   ...
   If they are 01, then bits 0-13 are an index in UCKeyStateRecordsIndex
   (resource-wide list). If they are 10, then bits 0-13 are an index in
   UCKeySequenceDataIndex (resource-wide list), or if UCKeySequenceDataIndex is
   not present or the index is beyond the end of the list, then bits 0-15 are a
   single Unicode character. Otherwise, bits 0-15 are a single Unicode
   character; a value of 0xFFFE-0xFFFF means no character output. UCKeyCharSeq
   is similar, but does not support indices in UCKeyStateRecordsIndex. For bits
   15-14: If they are 10, then bits 0-13 are an index in UCKeySequenceDataIndex
   (resource-wide list), or if UCKeySequenceDataIndex is not present or the
   index is beyond the end of the list, then bits 0-15 are a single Unicode
   character. Otherwise, bits 0-15 are a single Unicode character; a value of
...
/**
   ...
   The UCKeyStateRecord information is used as follows. If the current state is
   zero, output stateZeroCharData and set the state to stateZeroNextState. If
   the current state is non-zero and there is an entry for it in stateEntryData,
   then output the corresponding charData and set the state to nextState.
   Otherwise, output the state terminator from UCKeyStateTerminators for the
   current state (or nothing if there is no UCKeyStateTerminators table or it
   has no entry for the current state), then output stateZeroCharData and set
...
/**
   ...
   then it matches the entry, and we transform charData and nextState as
   follows: If charData < 0xFFFE, then charData +=
   (curState-curStateStart)*deltaMultiplier If nextState != 0, then nextState +=
...
/**
   ...
   The UCKeyboardLayout struct given here is only for the resource header. It
   specifies offsets to the various subtables which each have their own structs,
   given below. The keyboardTypeHeadList array selects table offsets that depend
   on keyboardType. The first entry in keyboardTypeHeadList is the default
   entry, which will be used if the keyboardType passed to UCKeyTranslate does
   not match any other entry - i.e. does not fall within the range
   keyboardTypeFirst..keyboardTypeLast for some entry. The first entry should
...
struct UCKeyModifiersToTableNum {
  ...
  /** Then there is padding to a 4-byte boundary with bytes containing 0, if
   * necessary.*/
};
...
struct UCKeyToCharTableIndex {
  ...
  ItemCount keyToCharTableCount;    /** Dimension for keyToCharTableOffsets[]
                                       (usually 6 to 12 tables)*/
  ByteOffset keyToCharTableOffsets[1];

  /** Each offset in keyToCharTableOffsets is from the beginning of the resource
   * to a*/
  /** table as follows:*/
...
struct UCKeyStateRecordsIndex {
  ...
  /** Each offset in keyStateRecordOffsets is from the beginning of the resource
   * to a*/
  /** UCKeyStateRecord. These UCKeyStateRecords follow the
   * keyStateRecordOffsets[] array.*/
  /** Then there is padding to a 4-byte boundary with bytes containing 0, if
   * necessary.*/
};
...
struct UCKeyStateTerminators {
  ...
  UInt16 keyStateTerminatorCount; /** Dimension for keyStateTerminators[] (# of
                                     nonzero states)*/
...
struct UCKeySequenceDataIndex {
  ...
  UInt16 charSequenceCount;          /** Dimension of charSequenceOffsets[] is
                                        charSequenceCount+1*/
  UInt16 charSequenceOffsets[1];

  /** Each offset in charSequenceOffsets is in bytes, from the beginning of*/
  /** UCKeySequenceDataIndex to a sequence of UniChars; the next offset
   * indicates the*/
  /** end of the sequence. The UniChar sequences follow the
...
/** -------------------------------------------------------------------------------------------------*/
/** Current format codes for the various tables (bits 12-15 indicate which
 * table)*/
...
enum {
  ...
  kUCKeyActionDisplay =
      3 /** get information for key display (as in Key Caps)      */
};
...
enum {
  kUCKeyTranslateNoDeadKeysBit =
      0 /** Prevents setting any new dead-key states*/
};
...
enum {
  ...
  kUCCollatePunctuationSignificantMask = 1L
                                         << 15, /** Number-handling options */
...
/** These constants are used for masking and shifting the invariant order
 * type.*/
 ...
 *    \non_carbon_cfm   in UnicodeUtilitiesCoreLib 8.5 and later
 *    \carbon_lib        in CarbonLib 1.0 and later
 *    \mac_os_x         in version 10.0 and later
 */
EXTERN_API(OSStatus)
UCKeyTranslate(const UCKeyboardLayout *keyLayoutPtr, UInt16 virtualKeyCode,
               UInt16 keyAction, UInt32 modifierKeyState, UInt32 keyboardType,
               OptionBits keyTranslateOptions, UInt32 *deadKeyState,
 ...
 *    \non_carbon_cfm   in UnicodeUtilitiesLib 8.6 and later
 *    \carbon_lib        in CarbonLib 1.0 and later
 *    \mac_os_x         in version 10.0 and later
 */
 ...
 *    \non_carbon_cfm   in UnicodeUtilitiesCoreLib 8.6 and later
 *    \carbon_lib        in CarbonLib 1.0 and later
 *    \mac_os_x         in version 10.0 and later
 */
EXTERN_API_C(OSStatus)
 ...
 *    \non_carbon_cfm   in UnicodeUtilitiesLib 9.0 and later
 *    \carbon_lib        in CarbonLib 1.0 and later
 *    \mac_os_x         in version 10.0 and later
 */
EXTERN_API_C(OSStatus)
