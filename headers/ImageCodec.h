...
NewImageCodecTimeTriggerUPP(ImageCodecTimeTriggerProcPtr userRoutine) {
  return (ImageCodecTimeTriggerUPP)NewRoutineDescriptor(
...
inline ImageCodecDrawBandCompleteUPP NewImageCodecDrawBandCompleteUPP(
  ...
  return (ImageCodecDrawBandCompleteUPP)NewRoutineDescriptor(
...
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
inline void DisposeImageCodecTimeTriggerUPP(ImageCodecTimeTriggerUPP userUPP) {
...
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
inline void InvokeImageCodecTimeTriggerUPP(void *refcon,
                                           ImageCodecTimeTriggerUPP userUPP) {
  CALL_ONE_PARAMETER_UPP(userUPP, uppImageCodecTimeTriggerProcInfo, refcon);
}
#else
#define InvokeImageCodecTimeTriggerUPP(refcon, userUPP)                        \
  CALL_ONE_PARAMETER_UPP((userUPP), uppImageCodecTimeTriggerProcInfo, (refcon))
#endif
#endif
...
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
inline void InvokeImageCodecDrawBandCompleteUPP(
    void *refcon, ComponentResult drawBandResult, UInt32 drawBandCompleteFlags,
    ImageCodecDrawBandCompleteUPP userUPP) {
  CALL_THREE_PARAMETER_UPP(userUPP, uppImageCodecDrawBandCompleteProcInfo,
                           refcon, drawBandResult, drawBandCompleteFlags);
}
#else
#define InvokeImageCodecDrawBandCompleteUPP(refcon, drawBandResult,            \
                                            drawBandCompleteFlags, userUPP)    \
  CALL_THREE_PARAMETER_UPP((userUPP), uppImageCodecDrawBandCompleteProcInfo,   \
...
#define CallImageCodecTimeTriggerProc(userRoutine, refcon)                     \
  InvokeImageCodecTimeTriggerUPP(refcon, userRoutine)
#define CallImageCodecDrawBandCompleteProc(                                    \
...
/* substitution codec in case effect is missing -- placed in root container,
 * recommended */
enum {
...
/**
   enumeration list in container -- placed in root container, optional unless
...
NewImageCodecMPDrawBandUPP(ImageCodecMPDrawBandProcPtr userRoutine) {
  return (ImageCodecMPDrawBandUPP)NewRoutineDescriptor(
...
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
inline void DisposeImageCodecMPDrawBandUPP(ImageCodecMPDrawBandUPP userUPP) {
  DisposeRoutineDescriptor((UniversalProcPtr)userUPP);
}
#else
...
ImageCodecTrimImage(ComponentInstance ci, ImageDescriptionHandle Desc,
                    ...
                    long outBufferSize, ICMFlushProcRecordPtr flushProc,
...
ImageCodecExtractAndCombineFields(ComponentInstance ci, long fieldFlags,
                                  ...
                                  long dataSize2, ImageDescriptionHandle desc2,
                                  void *outputData, long *outDataSize,
...
/* source identifier -- placed in root container of description, one or more
 * required */
...
ImageCodecEffectRenderSMPTEFrame(ComponentInstance effect, PixMapPtr destPixMap,
                                 ...
                                 Fixed effectPercentageOdd, Rect *pSourceRect,
                                 MatrixRecord *pMatrix,
                                 SMPTEWipeType effectNumber, long xRepeat,
                                 long yRepeat, SMPTEFlags flags, Fixed penWidth,
                                 long strokeValue)
