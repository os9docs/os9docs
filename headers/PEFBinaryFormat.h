/**
    ...
    \brief   PEF Types and Macros

    \introduced_in  Master Interfaces
...
#define PEFFirstSectionNameOffset(container) \
  ...
  /**
   \name Packed Data Contents
   The raw contents of a packed data section are a sequence of byte codes.  The basic format has a 3 bit opcode followed by a 5 bit count. Additional bytes might be used to contain counts larger than 31, and to contain a second or third count.  Further additional bytes contain actual data values to transfer.
   All counts are represented in a variable length manner. A zero in the initial 5 bit count indicates the actual value follows.  In this case, and for the second and third counts, the count is represented as a variable length sequence of bytes.  The bytes are stored in big endian manner, most significant part first. The high order bit is set in all but the last    byte. The value is accumulated by shifting the current value up 7 bits and adding in the low order 7 bits of the next byte. */
...
#define PEFComposeImportedSymbol(class, nameOffset) \
  ...
  /** \name Exported Symbol Hash Table
   * Exported symbols are described in four parts, optimized for speed of lookup.
   * These parts are the "export hash table", the "export key table", the "export symbol table", and the       "export name table".  Overall they contain a flattened representation of a fairly normal hashed symbol table.

  The export hash table is an array of small fixed size elements.  The number of elements is
  a power of 2.  A 32 bit hash word for a symbol is converted into an indexinto this array.
  Each hash slot contains a count of the number of exported symbols that map tothis slot and
  the index of the first of those symbols in the key and symbol tables.  Ofcourse some hash
  slots will have a zero count.

  The key and symbol tables are also arrays of fixed size elements, one foreach exported
  symbol.  Their entries are grouped by hash slot, those elements mapping tothe same hash
  slot are contiguous.  The key table contains just the full 32 bit hash wordfor each
  exported symbol.  The symbol table contains the offset of the symbol's namein the string
  ...
  the indicated portion of the key table for matching hashwords.  If a hashwordmatches, you
  look at the corresponding symbol table entry to find the full symbol name. Ifthe names
  match the symbol is found.


  The following function may be used to compute the hash table size.  Signed values are used just to avoid potential code generation overhead for unsigned division.
...
#define PEFComposeExportedSymbol(class, nameOffset) \
  ...
  /**
   * \name Loader Relocations

  The relocations for a section are defined by a sequence of instructions for an abstract       machine that is specifically geared to performing relocations commonly needed for the "CFM" code generation model.  These instructions occur in 16 bit chunks.  Most instructions have just a single chunk.  Instructions that are larger than 16 bits have an opcode and some of
  the operands in the first chunk, with other operands in following chunks.

  \note The multi-chunk relocations have separate "Compose" macros for each chunk. The macros have the same basic name with a positional suffix of "_1st", "_2nd", etc.

  \note Note that the relocCount field is the number of 16 bit relocation chunks, i.e. 1/2 the total number of bytes of relocation instructions.  While most relocation instructions are 16 bits long, some are longer so the number of complete relocation instructions may be less than the relocCount value. */
...
#define PEFMaskedBasicOpcodes                                                 \
   ...
   *      | 2 |<-- 8 bits --->|<--  6 --->|
   *
   * \note The stored skip count and reloc count are the actual values!
   */
...
#define PEFRelocComposeWithSkip(skipCount, relocCount) \
  ...
  /**
   * \name RelocRun Group (CODE, DATA, DESC, DSC2, VTBL, SYMR)
   * The "RelocRun" group includes the "RelocBySectC" (CODE), "RelocBySectD" (DATA), "RelocTVector12" (DESC), "RelocTVector8" (DSC2), "RelocVTable8" (VTBL), and "RelocImportRun" (SYMR) instructions.  This group has the following bit field layout.
  ...
  \note Note that the stored run length is the actual value minus 1, but the macros deal with the actual value!
...
#define PEFRelocComposeImportRun(runLength) PEFRelocComposeRun(5, (runLength))
  ...
  /**
   * \name RelocSmIndex Group (SYMB, CDIS, DTIS, SECN)
   * The "RelocSmIndex" group includes the "RelocSmByImport" (SYMB), "RelocSmSetSectC" (CDIS), "RelocSmSetSectD" (DTIS) and "RelocSmBySection" (SECN) instructions.  This group has the following bit field layout:
  ...
  \note The stored index is the actual value!
  */
...
#define PEFRelocComposeSmBySection(index) PEFRelocComposeSmIndex(3, (index))
  ...
  /**
   * \name RelocIncrPosition Instruction (DELT)
  ...
  \note The stored offset is the actual value minus 1, but the macros deal with the actual value! */
...
#define PEFRelocComposeIncrPosition(offset) (0x8000 | ((UInt16)((offset)-1)))
  ...
  /**
   * \name RelocSmRepeat Instruction (RPT)
...
#define PEFRelocComposeSmRepeat(chunkCount, repeatCount) \
  ...
  /**
   * \name RelocSetPosition Instruction (LABS)
  ...
  \note The stored offset is the actual value! */
...
#define PEFRelocComposeSetPosition_2nd(fullOffset) \
  ...
  /**
   * \name RelocLgByImport Instruction (LSYM)
...
#define PEFRelocComposeLgByImport_2nd(fullIndex) \
  ...
  /**
   * \name RelocLgRepeat Instruction (LRPT)
  ...
  \note stored chunk count is the actual value minus 1, but the macros deal with the actual value!  The stored repeat count is the actual value! */
...
#define PEFRelocComposeLgRepeat_2nd(chunkCount, fullRepeatCount) \
  ...
  /**
   * \name RelocLgSetOrBySection Group (LSEC)
   *
   * The "RelocLgSetOrBySection" (LSEC) instruction is a group including the "RelocLgBySection", "RelocLgSetSectC" and "RelocLgSetSectD" instructions.  This group has the following bit field layout.
   *
                            1         1                                   1
        0         5 6     9 0         5     0                             5
       +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+   +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
  ...
  \note Note that the stored index is the actual value!
  */
