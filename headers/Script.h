/**
    ...
    \brief   Script Manager interfaces
...
/**
   ...
   Some of the encodings have several variants (e.g. for different localized
   systems) which all share the same script code. Not all of these script codes
   are currently supported by Apple software. Notes:
   - Script code 0 (smRoman) is also used (instead of smGreek) for the Greek
   encoding in the Greek localized system.
   - Script code 28 (smEthiopic) is also used for the Inuit encoding in the
   Inuktitut system.
*/
enum {
  smRoman = 0,
  ...
  smCentralEuroRoman =
      29, /** For Czech, Slovak, Polish, Hungarian, Baltic langs*/
  smVietnamese = 30,
...
enum {
  ...
  langIrishGaelic =
      35, /** smRoman or modified smRoman/Celtic script (without dot above)   */
...
enum {
  ...
  langIrishGaelicScript =
      146, /** modified smRoman/Gaelic script (using dot above)               */
...
enum {
  langUnspecified =
      32767 /** Special code for use in resources (such as 'itlm')           */
};
...
/**
   Region codes:
   These typically specify a combination of a language code and a particular
   region. Some of these numeric values are reserved just for extra resource IDs
   associated with certain regions; these are not actual region codes, and are
   noted in the comments. Not all of the region codes are currently supported by
   Apple software. When relevant, the following list also provides:
   - The Apple part number (P/N) code for software localized for the specified
   region.
   - The two-letter ISO language and country codes (from ISO 639 and ISO 3166).
   The language code (lowercase) is first, then '_', then the country code
   (uppercase).
*/
...
enum {
  ...
  verArabic =
      16, /** AB ar       Arabic for N Africa, Arabian peninsula, Levant*/
...
enum {
  ...
  verIreland =
      50, /** GA ga_IE     Irish Gaelic for Ireland (without dot above)     */
  ...
  verScriptGeneric =
      55, /** SS          Generic script system (no language or script)       */
  verCzech = 56,  /** CZ cs_CZ*/
  verSlovak = 57, /** SL sk_SK*/
  verFarEastGeneric =
      58, /** FE          Generic Far East system (no language or script)   */
...
enum {
  ...
  verCroatia = 68,    /** CR hr_HR,sh_HR*/
                      /**              69 is unassigned*/
  verGermanReformed = 70, /**       de_DE     Reformed orthography (used
                             formerly unassigned 70) */
  ...
  verIrishGaelicScript =
      81, /** GS ga_IE     Irish Gaelic for Ireland (using dot above)*/
...
/**
   ...
   Misspelled or alternate form, ambiguous, misleading, considered pejorative,
   archaic, etc.
*/
enum {
  verFrBelgiumLux =
      6, /** Incorrect; 6 is Flemish, not French, for Belgium; use verFlemish */
  verBelgiumLux = 6,  /** Use verFlemish*/
  verArabia = 16,     /** Use verArabic*/
  verYugoslavia = 25, /** Use verYugoCroatian (same number, now unused), or
                         newer verCroatia*/
...
enum {
  ...
  smCharNonContextualLR =
      0x000A,              /**Non-contextual left-right: Cyrillic, Greek*/
...
enum {
  ...
  smPunctBlank = 0x0300,   /** Additional CharType classes for punctuation in
                              two-byte systems */
...
enum {
  ...
  smCharLower = 0x0000,
  smCharUpper =
      0x4000, /** CharType character size modifiers (1 or multiple bytes). */
  smChar1byte = 0x0000,
  smChar2byte = 0x8000
};
...
enum {
  ...
  smTransPreDoubleByting =
      1, /**Convert all text to double byte before transliteration*/
  smTransPreLowerCasing =
      2 /**Convert all text to lower case before transliteration*/
};
...
enum {
  /** Result values from GetScriptManagerVariable and SetScriptManagerVariable
     calls. */
...
/** special negative verbs for Get/SetScriptVariable that were associated with
 * WorldScriptI */
/** move them here to be public */
enum {
  /** WorldScript private verbs */
  smLayoutCache =
      -309, /** HiWrd(param) is # entries, LoWrd is max input length*/
  ...
  smScriptSysBase = -281,  /** param is associated font to use w/ system font
                              (old verb = -26)*/
  smScriptAppBase =
      -283, /** param is associated font to use w/ app font (old verb = -28)*/
  smScriptFntBase = -285, /** param is associated font to use w/ all other fonts
                             (old verb = -30)*/
...
enum {
  ...
  smKeySwapKybd =
      -5, /** Switch to previously-used keyboard in current keyscript */
  ...
  smKeyNextInputMethod =
      -10, /** Switch to next input method in current keyscript */
  smKeySwapInputMethod =
      -11, /** Switch to last-used input method in current keyscript */
  smKeyDisableKybdSwitch =
      -12, /** Disable switching from the current keyboard */
  smKeySetDirLeftRight =
      -15, /** Set default line dir to left-right, align left */
  smKeySetDirRightLeft =
      -16,         /** Set default line dir to right-left, align right */
  smKeyRoman = -17 /** Set keyscript to Roman. Does nothing if Roman-only
                      system, unlike KeyScript(smRoman) which forces an update
                      to current default Roman keyboard */
};
...
enum {
  ...
  smfDisableKeyScriptSync =
      27 /**Disable font and keyboard script synchronization*/
};

enum {
  /** We should define masks, too. */
  smfDisableKeyScriptSyncMask =
      1L << smfDisableKeyScriptSync /**Disable font and keyboard script
                                       synchronization mask*/
};

enum {
  /** Force keyboard script switching flag and mask for zero and positive
     KeyScript verbs */
...
enum {
  ...
  smfUseAssocFontInfo =
      28 /**Use the associated font info for FontMetrics calls <48>*/
};
...
enum {
  delimPad = -2, /** obsolete, misspelled token names kept for backward
                    compatibility */
  tokenTilda = 44,
  tokenCarat = 55
};
...
enum {
  ...
  iuWordSelectTable =
      0, /** <obsolete>  get word select break table from 'itl2' */
  iuWordWrapTable = 1, /** <obsolete>  get word wrap break table from 'itl2' */
  iuNumberPartsTable =
      2, /** <obsolete>  get default number parts table from 'itl4' */
...
#define IUSetIntl(refNum, theID, intlHandle)                                   \
  SetIntlResource(refNum, theID, intlHandle)
