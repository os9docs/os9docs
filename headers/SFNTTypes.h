...
#pragma pack(2)
  ...
  FourCharCode format;
  UInt16 numOffsets; // number of tables   UInt16 searchRange;
  // (max2 <= numOffsets)*16   UInt16 entrySelector;
  // log2(max2 <= numOffsets)   UInt16 rangeShift;
  // numOffsets*16-searchRange  sfntDirectoryEntry table[1];
  // table[numOffsets]
};
...
{
  UInt16 platformID; // base-0   UInt16 scriptID;
  // base-0   UInt32 offset;
};
...
typedef struct sfntCM     // base-0
    enum { sizeof_sfntCMa // base-0
           ...
           struct sfntNameRecord{
               UInt16 platformID; // base-0   UInt16 scriptID;
                                  // base-0   UInt16 languageID;
                                  // base-0   UInt16 nameID;
                                  // base-0   UInt16 length;
               UInt16 offset;};
...
struct sfntVariationHeader
{
  ...
  UInt16 instanceSize;
  // �other <count,size> pairs   sfntVariationAxis axis[1];
  // [axisCount]   sfntInstance instance[1];
...
/**
**  FontVariation is used to specify a coordinate along a variation axis. The
*name
