...
#pragma parameter __D0 PBCatSearchAsync(__A0)
    ...
    * \note <pre>SetVol lets you select a default volume for use in subsequent file
    ...
    err = SetVol( 0, theRefNum ); //  set by reference number
        ...
        this parameter is checked first and anything but a valid name(except a NIL).You can pass a working directory number(i.e., the value of ioVRefNum after a call to PBOpenWD, or a volume reference returned by Standard File), but only if volName is NIL on entry(see the third example).Also, if you do use a working directory reference, a subsequent call to GetVol will return that number, rather than a "hard" volume ID.</ pre>
        \copyright THINK Reference © 1991 - 1992 Symantec Corporation
        \non_carbon_cfm in InterfaceLib 7.1 and later
        \carbon_lib not available
        \mac_os_x not available
        */
        OSErr SetVol(ConstStr63Param volName, //  can be NULL
                     short vRefNum);
