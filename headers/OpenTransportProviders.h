/**
    ...
    \brief   This file contains provider-specific definitions for various
   built-in providers.
...
#define kRawIPName "rawip"
  ...
  enum
  {
    ...
    IP_BROADCAST_IFNAME =
        kIP_BROADCAST_IFNAME,    /** Set interface for broadcasts   */
...
#define SET_TOS(prec, tos) (((0x7 & (prec)) << 5) | (0x1c & (tos)))
  ...
  */
   ...
   */
  void
  OTInitInetAddress(InetAddress *addr, InetPort port, InetHost host);
   ...
   */
  OSStatus
  OTInetStringToHost(const char *str, InetHost *host);
   ...
   */
  void
  OTInetHostToString(InetHost host, char *str);
   ...
   */
  OSStatus
  OTInetGetInterfaceInfo(InetInterfaceInfo *info, SInt32 val);
   ...
   */
  OSStatus
  OTInetGetSecondaryAddresses(InetHost *addr, UInt32 *count, SInt32 val);
   ...
   */
  OSStatus
  OTInetGetDHCPConfigInfo(InetDHCPOption *buf, UInt32 bufSize, SInt32 index,
                          SInt32 opt);
...
#if !OTKERNEL
  /**
     Under Carbon, OTOpenInternetServices routines take a client context pointer.
  ...
  */
   ...
   */
  InetSvcRef
  OTOpenInternetServicesInContext(OTConfigurationRef cfig, OTOpenFlags oflag,
                                  OSStatus *err,
   ...
   */
  OSStatus
  OTAsyncOpenInternetServicesInContext(OTConfigurationRef cfig, OTOpenFlags oflag,
   ...
   */
  InetSvcRef
  OTOpenInternetServices(OTConfigurationRef cfig, OTOpenFlags oflag,
                         OSStatus *err);
   ...
   */
  OSStatus
  OTAsyncOpenInternetServices(OTConfigurationRef cfig, OTOpenFlags oflag,
...
#define OTAsyncOpenInternetServices(cfig, oflags, proc, contextPtr) \
  OTAsyncOpenInternetServicesInContext(cfig, oflags, proc, contextPtr, NULL)
   ...
   */
  OSStatus
  OTInetStringToAddress(InetSvcRef ref, char *name, InetHostInfo *hinfo);
   ...
   */
  OSStatus
  OTInetAddressToName(InetSvcRef ref, InetHost addr, InetDomainName name);
   ...
   */
  OSStatus
  OTInetSysInfo(InetSvcRef ref, char *name, InetSysInfo *sysinfo);
   ...
   */
  OSStatus
  OTInetMailExchange(InetSvcRef ref, char *name, UInt16 *num,
                     InetMailExchange *mx);
   ...
   */
  OSStatus
  OTInetQuery(InetSvcRef ref, char *name, UInt16 qClass, UInt16 qType, char *buf,
              OTByteCount buflen, void **argv, OTByteCount argvlen,
              OTFlags flags);
...
public:
  ...
  OSStatus Query(char *name, UInt16 qClass, UInt16 qType, char *buf,
                 OTByteCount buflen, void **argv, OTByteCount argvlen,
                 OTFlags flags)
  {
    return OTInetQuery(this, name, qClass, qType, buf, buflen, argv, argvlen,
                       flags);
  }
};
...
#define kLTalkBName "ltlkB"
  ...
  */
  ...
  enum
  {
    ...
    DDP_OPT_SRCADDR =
        0x2101,                /** DDP UnitDataReq Only - set src address, Value is DDPAddress */
    ATP_OPT_REPLYCNT = 0x2110, /** AppleTalk - ATP Resp Pkt Ct Type, Value is
                                  (unsigned long)  pkt count */
    ATP_OPT_DATALEN = 0x2111,  /** AppleTalk - ATP Pkt Data Len Type, Value is
                                  (unsigned long) length */
    ATP_OPT_RELTIMER =
        0x2112, /** AppleTalk - ATP Release Timer Type, Value is (unsigned long)
                   timer, (See Inside AppleTalk, second edition */
    ATP_OPT_TRANID =
        0x2113,                /** Value is (unsigned long) Boolean, Used to request Transaction
                                  ID, Returned with Transaction ID on requests */
    PAP_OPT_OPENRETRY = 0x2120 /** AppleTalk - PAP OpenConn Retry count, Value is
                                  (unsigned long) T_YES/T_NO */
  };
  ...
  */
  ...
  enum
  {
    ...
    T_ATALKROUTERDOWNEVENT =
        kAppleTalkEvent +
        51, /** No routers have been seen for a while.  If the cookie is NULL, all
               routers are gone.  Otherwise, there is still an ARA router hanging
               around being used, and only the local cable has been  timed out.*/
    T_ATALKROUTERUPEVENT =
        kAppleTalkEvent + 52, /** We didn't have a router, but now one has come
                                 up. Cookie is NULL for a normal router coming up,
                                 non-NULL for an ARA router coming on-line*/
    T_ATALKZONENAMECHANGEDEVENT =
        kAppleTalkEvent + 53, /** A Zone name change was issued from the router,
                                 so our AppleTalk Zone has changed.*/
    T_ATALKCONNECTIVITYCHANGEDEVENT =
        kAppleTalkEvent + 54, /** An ARA connection was established (cookie !=
                                 NULL), or was disconnected (cookie == NULL)*/
    T_ATALKINTERNETAVAILABLEEVENT =
        kAppleTalkEvent +
        55, /** A router has appeared, and our address is in the startup range.
               Cookie is hi/lo of new cable range.*/
    T_ATALKCABLERANGECHANGEDEVENT =
        kAppleTalkEvent +
        56 /** A router has appeared, and it's incompatible with our current
              address.  Cookie is hi/lo of new cable range.*/
  };

  enum
  {
    kAllATalkRoutersDown = 0,     /** This indicates that all routers are offline*/
    kLocalATalkRoutersDown = -1L, /** This indicates that all local routers went
                                     offline, but an ARA router is still active*/
    kARARouterDisconnected =
        -2L /** This indicates that ARA was disconnected, do it's router went
               offline, and we have no local routers to fall back onto.*/
  };

  enum
  {
    kARARouterOnline =
        -1L, /** We had no local routers, but an ARA router is now online.*/
    kATalkRouterOnline =
        0, /** We had no routers, but a local router is now online*/
    kLocalATalkRouterOnline =
        -2L /** We have an ARA router, but now we've seen a local router as well*/
  };
...
#define IsAppleTalkEvent(x)         ((x) & 0xffff0000) == kAppleTalkEvent)
  ...
  enum
  {
    ATALK_IOC_FULLSELFSEND =
        ((MIOC_ATALK << 8) | 47),                    /** Turn on/off full self-send (it's automatic
                                                        for non-backward-compatible links)*/
  ...
  enum
  {
    ...
    kNBPMaxEntityLength =
        (kNBPMaxNameLength + kNBPMaxTypeLength + kNBPMaxZoneLength + 3),
  ...
  enum
  {
    kDDPAddressLength = 8, /** value to use in netbuf.len field, Maximum length of
                              AppleTalk address*/
...
#define kDefaultAppleTalkServicesPath ((OTConfigurationRef)-3L)
  /**
     Under Carbon, OpenAppleTalkServices routines take a client context pointer.
     Applications may pass NULL after calling
     InitOpenTransport(kInitOTForApplicationMask, ...).  Non-applications must
     always pass a valid client context.
  */
  ...
  OTAsyncOpenAppleTalkServicesInContext(OTConfigurationRef cfig,
                                        OTOpenFlags flags, OTNotifyUPP proc,
                                        void *contextPtr,
                                        OTClientContextPtr clientContext);
   ...
   */
  ATSvcRef
  OTOpenAppleTalkServicesInContext(OTConfigurationRef cfig, OTOpenFlags flags,
                                   OSStatus *err,
                                   OTClientContextPtr clientContext);
   ...
   */
  OSStatus
  OTAsyncOpenAppleTalkServices(OTConfigurationRef cfig, OTOpenFlags flags,
                               OTNotifyUPP proc, void *contextPtr);
   ...
   */
  ATSvcRef
  OTOpenAppleTalkServices(OTConfigurationRef cfig, OTOpenFlags flags,
                          OSStatus *err);
...
#if OTCARBONAPPLICATION
/** The following macro may be used by applications only.*/
#define OTOpenAppleTalkServices(cfig, oflags, err) \
  OTOpenAppleTalkServicesInContext(cfig, oflags, err, NULL)
#define OTAsyncOpenAppleTalkServices(cfig, oflags, proc, contextPtr) \
  OTAsyncOpenAppleTalkServicesInContext(cfig, oflags, proc, contextPtr, NULL)
#endif /** OTCARBONAPPLICATION */
   ...
   */
  OSStatus
  OTATalkGetMyZone(ATSvcRef ref, TNetbuf *zone);
  ...
  */
   ...
   */
  OSStatus
  OTATalkGetLocalZones(ATSvcRef ref, TNetbuf *zones);
   ...
   */
  OSStatus
  OTATalkGetZoneList(ATSvcRef ref, TNetbuf *zones);

  /** Stores an AppleTalkInfo structure into the TNetbuf (see later in this
   * file)*/
  /**
   *  OTATalkGetInfo()
   *
   ...
   */
  OSStatus
  OTATalkGetInfo(ATSvcRef ref, TNetbuf *info);
...
{      /** resume C definitions */
  ...
  */
   ...
   */
  void
  OTInitDDPAddress(DDPAddress *addr, UInt16 net, UInt8 node, UInt8 socket,
                   UInt8 ddpType);
   ...
   */
  OTByteCount
  OTInitNBPAddress(NBPAddress *addr, const char *name);
   ...
   */
  OTByteCount
  OTInitDDPNBPAddress(DDPNBPAddress *addr, const char *name, UInt16 net,
   ...
   */
  Boolean
  OTCompareDDPAddresses(const DDPAddress *addr1, const DDPAddress *addr2);
   ...
   */
  OTByteCount
  OTSetAddressFromNBPEntity(UInt8 *nameBuf, const NBPEntity *entity);
   ...
   */
  OTByteCount
  OTSetAddressFromNBPString(UInt8 *addrBuf, const char *name, SInt32 len);
  ...
  */
   ...
   */
  Boolean
  OTSetNBPEntityFromAddress(NBPEntity *entity, const UInt8 *addrBuf,
                            OTByteCount len);
   ...
   */
  Boolean
  OTSetNBPName(NBPEntity *entity, const char *name);
   ...
   */
  Boolean
  OTSetNBPType(NBPEntity *entity, const char *typeVal);
   ...
   */
  Boolean
  OTSetNBPZone(NBPEntity *entity, const char *zone);
   ...
   */
  void
  OTExtractNBPName(const NBPEntity *entity, char *name);
   ...
   */
  void
  OTExtractNBPType(const NBPEntity *entity, char *typeVal);
   ...
   */
  void
  OTExtractNBPZone(const NBPEntity *entity, char *zone);
  ...
  inline void DDPAddress::SetSocket(UInt8 socket) { fSocket = socket; }

  inline void DDPAddress::SetNode(UInt8 node) { fNodeID = node; }

  inline void DDPAddress::SetType(UInt8 type) { fDDPType = type; }

  inline void DDPAddress::SetNetwork(UInt16 net) { fNetwork = net; }
  ...
  inline OTAddressType DDPAddress::GetAddressType() const { return fAddressType; }

  inline UInt8 DDPAddress::GetSocket() const { return fSocket; }

  inline UInt8 DDPAddress::GetNode() const { return fNodeID; }

  inline UInt8 DDPAddress::GetType() const { return fDDPType; }

  inline UInt16 DDPAddress::GetNetwork() const { return fNetwork; }

  inline void DDPAddress::Init(UInt16 net, UInt8 node, UInt8 socket)
  {
  ...
  inline void DDPAddress::Init(UInt16 net, UInt8 node, UInt8 socket, UInt8 type)
  {
    fAddressType = AF_ATALK_DDP;
    SetNetwork(net);
    SetNode(node);
  ...
  inline OTByteCount NBPAddress::Init(const NBPEntity &addr)
  {
    fAddressType = AF_ATALK_NBP;
    return sizeof(OTAddressType) +
           OTSetAddressFromNBPEntity(fNBPNameBuffer, &addr);
  }
  ...
  inline OTByteCount NBPAddress::Init(const char *name)
  {
    fAddressType = AF_ATALK_NBP;
    return sizeof(OTAddressType) +
           OTSetAddressFromNBPString(fNBPNameBuffer, name, -1);
  }
  ...
  inline OTByteCount NBPAddress::Init(const char *name, OTByteCount len)
  {
    fAddressType = AF_ATALK_NBP;
    return sizeof(OTAddressType) +
           OTSetAddressFromNBPString(fNBPNameBuffer, name, (SInt32)len);
  }
  ...
  inline OTAddressType NBPAddress::GetAddressType() const { return fAddressType; }
  ...
  inline void DDPNBPAddress::SetSocket(UInt8 socket) { fSocket = socket; }

  inline void DDPNBPAddress::SetNode(UInt8 node) { fNodeID = node; }

  inline void DDPNBPAddress::SetType(UInt8 type) { fDDPType = type; }

  inline void DDPNBPAddress::SetNetwork(UInt16 net) { fNetwork = net; }
  ...
  inline UInt8 DDPNBPAddress::GetSocket() const { return fSocket; }

  inline UInt8 DDPNBPAddress::GetNode() const { return fNodeID; }

  inline UInt8 DDPNBPAddress::GetType() const { return fDDPType; }

  inline UInt16 DDPNBPAddress::GetNetwork() const { return fNetwork; }

  inline void DDPNBPAddress::Init(UInt16 net, UInt8 node, UInt8 socket)
  {
  ...
  {
    return OTSetAddressFromNBPString(fNBPNameBuffer, name, -1) +
           kDDPAddressLength;
  }
  ...
  inline OTByteCount DDPNBPAddress::SetNBPEntity(const char *name,
                                                 OTByteCount len)
  {
    return OTSetAddressFromNBPString(fNBPNameBuffer, name, (SInt32)len) +
           kDDPAddressLength;
  }

  inline Boolean DDPNBPAddress::ExtractEntity(NBPEntity &entity,
                                              OTByteCount len)
  {
    return OTSetNBPEntityFromAddress(&entity, fNBPNameBuffer, len);
  }
...
#define kFibreChannelName "fibre"
  ...
  enum
  {
    ...
    k8022SNAPLength = 5,
    kEnetAddressLength =
        k48BitAddrLength + k8022DLSAPLength, /** length of an address field used
                                                by the ENET enpoint*/
  ...
  enum
  {
    k8022BasicAddressLength =
        sizeof(OTAddressType) + k48BitAddrLength + sizeof(UInt16),
...
#define OTClear48BitAddress(p1) \
  (*(UInt32 *)((UInt8 *)(p1)) = 0, *(UInt16 *)(((UInt8 *)(p1)) + 4) = 0)
...
#define OTIs48BitZeroAddress(p1) \
  ...
  enum
  {
    ...
    k8022BasicHeaderLength =
        3, /** define the length of the header portion of an 802.2 packet.*/
  ...
  ********************************************************************************/
  ...
  enum
  {
    ...
    kT8022FullPacketHeaderLength =
        kEnetPacketHeaderLength + kT8022SNAPHeaderLength
  };
...
#define kSerialPortBName "serialB"
  ...
  enum
  {
    ...
    kOTSerialFramingPPP = 0x10           /** Port does its own PPP framing - wants unframed
                                            packets from PPP */
  };
  ...
  enum
  {
    ...
    kOTSerialSetDTROn = 1,
    I_SetSerialBreak =
        ((MIOC_SRL << 8) | 1), /** Send a break on the line - kOTSerialSetBreakOff
                                  = off, kOTSerialSetBreakOn = on,*/
    ...
    I_SetSerialXOffState =
        ((MIOC_SRL << 8) | 2), /** Force XOFF state - 0 = Unconditionally clear
                                  XOFF state, 1 = unconditionally set it*/
    kOTSerialForceXOffTrue = 1,
    kOTSerialForceXOffFalse = 0,
    I_SetSerialXOn =
        ((MIOC_SRL << 8) | 3), /** Send an XON character, 0 = send only if in XOFF
                                  state, 1 = send always*/
    kOTSerialSendXOnAlways = 1,
    kOTSerialSendXOnIfXOffTrue = 0,
    I_SetSerialXOff =
        ((MIOC_SRL << 8) | 4), /** Send an XOFF character, 0 = send only if in XON
                                  state, 1 = send always*/
  ...
  */
  ...
  enum
  {
    ...
    SERIAL_OPT_RCVTIMEOUT =
        0x0106, /** The "RcvTimeout" option defines how long the receiver should
                   wait before delivering*/
    /** less than the RcvLoWat number of characters.  If RcvLoWat is 0, then the
       RcvTimeout*/
    /** is how long a gap to wait for before delivering characters.  This
       parameter is advisory,*/
    /** and serial drivers are free to deliver data whenever they deem it
       convenient.  For instance,*/
    /** many serial drivers will deliver data whenever 64 bytes have been
       received, since 64 bytes*/
    /** is the smallest STREAMS buffer size. Keep in mind that timeouts are
       quantized, so be sure to*/
    /** look at the return value of the option to determine what it was negotiated
       to.*/
    SERIAL_OPT_ERRORCHARACTER = 0x0107, /** This option defines how characters
                                           with parity errors are handled.*/
    /** A 0 value will disable all replacement.  A single character value in the
       low*/
    /** byte designates the replacement character.  When characters are received
       with a */
    /** parity error, they are replaced by this specified character.  If a valid
       incoming*/
    /** character matches the replacement character, then the received character's
       msb is*/
    /** cleared. For this situation, the alternate character is used, if specified
       in bits*/
    /** 8 through 15 of the option long, with 0xff being place in bits 16
       through 23.*/
    /** Whenever a valid character is received that matches the first replacement
       character,*/
    /** it is replaced with this alternate character.*/
    SERIAL_OPT_EXTCLOCK = 0x0108, /** The "ExtClock" requests an external clock.
                                     A 0-value turns off external clocking.*/
    /** Any other value is a requested divisor for the external clock.  Be aware
       that*/
    /** not all serial implementations support an external clock, and that not
       all*/
    /** requested divisors will be supported if it does support an external
       clock.*/
    SERIAL_OPT_BURSTMODE = 0x0109, /** The "BurstMode" option informs the serial
                                      driver that it should continue looping,*/
  ...
  enum
  {
    kOTSerialXOnOffInputHandshake =
        1, /** Want XOn/XOff handshake for incoming characters    */
    kOTSerialXOnOffOutputHandshake =
        2, /** Want XOn/XOff handshake for outgoing characters    */
    kOTSerialCTSInputHandshake =
        4, /** Want CTS handshake for incoming characters     */
    kOTSerialDTROutputHandshake =
        8 /** Want DTR handshake for outoing characters   */
  };
...
#define OTSerialHandshakeData(type, onChar, offChar) \
  ...
  inline UInt32 OTSerialSetErrorCharacterWithAlternate(UInt8 rep,
                                                       UInt8 alternate)
  {
...
#define kISDNName "isdn"
  ...
  */
  ...
  {
    I_OTISDNAlerting =
        ((MIOC_ISDN << 8) | 100),               /** Send or receive an ALERTING message*/
    ...
    I_OTISDNSuspendAcknowledge =
        ((MIOC_ISDN << 8) | 102), /** Receive a SUSPEND ACKNOWLEDGE message*/
    I_OTISDNSuspendReject =
        ((MIOC_ISDN << 8) | 103),              /** Receive a SUSPEND REJECT message*/
    I_OTISDNResume = ((MIOC_ISDN << 8) | 104), /** Send a RESUME message*/
    /** The parameter is the Call Identity (Maximum 8 octets)*/
    I_OTISDNResumeAcknowledge =
        ((MIOC_ISDN << 8) | 105), /** Receive a RESUME ACKNOWLEDGE message*/
    I_OTISDNResumeReject =
        ((MIOC_ISDN << 8) | 106), /** Receive a RESUME REJECT message*/
    I_OTISDNFaciltity =
        ((MIOC_ISDN << 8) | 107) /** Send or receive a FACILITY message*/
  };
