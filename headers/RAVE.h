...
/******************************************************************************
 *
 * Platform dependent datatypes: TQAImagePixelType, TQADevice, TQAClip, and
 *TQARect.
 *
 *****************************************************************************/
enum TQAImagePixelType {
  ...
  kQAPixel_CL4 = 5, /* 4 bit color look up table, always big endian, ie high 4
                       bits effect left pixel */
  kQAPixel_CL8 = 6, /* 8 bit color look up table */
  kQAPixel_RGB16_565 =
      7,              /* Win32 ONLY  16 bits/pixel, no alpha, R:5, G:6, B:5 */
  ...
  kQAPixel_ACL16_88 =
      11, /* 16 bits/pixel, A=15:8, CL=7:0, 8 bit alpha + 8 bit color lookup */
  kQAPixel_I8 = 12, /* 8 bits/pixel, I=7:0, intensity map (grayscale) */
  kQAPixel_AI16_88 =
      13, /* 16 bits/pixel, A=15:8, I=7:0, intensity map (grayscale) */
  kQAPixel_YUVS = 14,    /* 16 bits/pixel, QD's kYUVSPixelFormat (4:2:2, YUYV
  ...
  kQAPixel_YVYU422 = 16, /* 16 bits/pixel, QD's kYVYU422PixelFormat (4:2:2, YVYU
                            ordering, unsigned UV) */
  kQAPixel_UYVY422 = 17  /* 16 bits/pixel, QD's kUYVY422PixelFormat (4:2:2, UYVY
                            ordering, unsigned UV) */
};
...
enum TQAColorTableType {
  kQAColorTable_CL8_RGB32 =
      0, /* 256 entry, 32 bit/pixel, R=23:16, G=15:8, B=7:0 */
  kQAColorTable_CL4_RGB32 =
      1 /* 16 entry, 32 bit/pixel, R=23:16, G=15:8, B=7:0 */
};
...
struct TQAZBuffer {
  ...
  long isBigEndian; /* true if zbuffer values are in big-endian format, false if
                       little-endian */
};
...
enum TQAError {
  ...
  kQADisplayModeUnsupported =
      7, /* Engine cannot render to the display in its current , */
 ...
 * Gsrc B = (1 - Asrc) * Bdest + Bsrc           B = (1 - Asrc) * Bdest + Asrc *
 * Bsrc
 *
 * Note that the use of other blending modes to implement antialiasing is
 * performed automatically by the drawing engine when the kQATag_Antialias
 * variable != kQAAntiAlias_Fast. The driving software should continue to use
 * the alpha fields for transparency even when antialiasing is being used (the
 ...
 *      kQATag_DontSwap             (Int) 1 = dont swap buffers during
 QARenderEnd, 0 = do swap buffers during QARenderEnd.
 ...
 *      kQATag_MultiTextureFactor    (Float) used with kQAMultiTexture_Fixed to
 ...
 *      kQATagGL_ScissorXMin        (Int)   Minimum X value for scissor
 rectangle
...
enum TQATagInt {
  ...
  kQATag_BitmapFilter =
      54, /* filter to use while scaling bitmaps, one of kQAFilter_xxx */
  kQATag_DrawContextFilter =
      55, /* filter to use while scaling draw contexts, one of kQAFilter_xxx */
...
enum TQATagFloat {
  ...
  kQATag_BitmapScale_x =
      52, /* horizontal bitmap scale factor, default value is 1.0 */
  kQATag_BitmapScale_y =
      53, /* vertical bitmap scale factor, default value is 1.0 */
...
enum {
  kQAZFunction_None =
      0, /* Z is neither tested nor written (same as no Z buffer) */
...
enum {
  kQABufferComposite_None =
      0, /* Default: New pixels overwrite initial buffer contents */
  kQABufferComposite_PreMultiply = 1, /* New pixels are blended with initial
...
enum {
  ...
  kQATextureOp_Shrink =
      (1 << 3),                 /* This is a non-wrapping texture, so the ??? */
...
enum {
  ...
  kQAMultiTexture_BlendAlpha =
      2, /* texels are blended according to 2nd texel's alpha */
...
enum TQAGestaltSelector {
  ...
  kQAGestalt_FastTextureMemory =
      8, /* amount of texture RAM currently available */
  ...
  kQAGestalt_DrawContextPixelTypesPreferred =
      10, /* returns all the draw context pixel types that are preferred by the
             RAVE engine. */
  kQAGestalt_TexturePixelTypesAllowed =
      11, /* returns all the texture pixel types that are supported by the RAVE
             engine */
  kQAGestalt_TexturePixelTypesPreferred =
      12, /* returns all the texture pixel types that are preferred by the RAVE
             engine.*/
  ...
  kQAGestalt_BitmapPixelTypesPreferred =
      14, /* returns all the bitmap pixel types that are preferred by the RAVE
             engine. */
  kQAGestalt_OptionalFeatures2 = 15, /* Mask of one or more kQAOptional2_xxx */
  kQAGestalt_MultiTextureMax =
      16, /* max number of multi textures supported by this engine */
...
enum TQAMethodSelector {
  kQAMethod_RenderCompletion =
      0, /* Called when rendering has completed and buffers swapped */
  kQAMethod_DisplayModeChanged = 1, /* Called when a display mode has changed */
  kQAMethod_ReloadTextures =
      2, /* Called when texture memory has been invalidated */
  kQAMethod_BufferInitialize =
      3, /* Called when a buffer needs to be initialized */
  kQAMethod_BufferComposite =
      4, /* Called when rendering is finished and its safe to composite */
...
enum TQAMethodSelector {
  ...
  kQAMethod_ReloadTextures =
      2, /* Called when texture memory has been invalidated */
  kQAMethod_ImageBufferInitialize =
      3, /* Called when a buffer needs to be initialized */
...
/**
 * kQATexture_xxx are ORed together to generate the 'flags' parameter to
 * QATextureNew().
 */
enum {
  ...
  kQATexture_HighCompression =
      (1 << 3), /* Compress texture, even if it takes a while */
  kQATexture_NonRelocatable =
      (1 << 4), /* Image buffer in VRAM should be non-relocatable */
  kQATexture_NoCopy = (1 << 5), /* Don't copy image to VRAM when creating it */
  kQATexture_FlipOrigin =
      (1 << 6), /* The image(s) is(are) in a bottom-up format. (The image(s)
                   is(are) flipped vertically.) */
  kQATexture_PriorityBits =
      (1 << 31) | (1 << 30) | (1 << 29) |
      (1 << 28) /* Texture priority: 4 upper bits for 16 levels of priority */
};

/**
 * kQABitmap_xxx are ORed together to generate the 'flags' parameter to
 * QABitmapNew().
 */
enum {
  ...
  kQABitmap_HighCompression =
      (1 << 3), /* Compress bitmap, even if it takes a while */
  kQABitmap_NonRelocatable =
      (1 << 4), /* Image buffer in VRAM should be non-relocatable */
  kQABitmap_NoCopy = (1 << 5), /* Don't copy image to VRAM when creating it */
  kQABitmap_FlipOrigin = (1 << 6), /* The image is in a bottom-up format. (The
                                      image is flipped vertically.) */
  kQABitmap_PriorityBits =
      (1 << 31) | (1 << 30) | (1 << 29) |
      (1 << 28) /* Bitmap priority: 4 upper bits for 16 levels of priority */
};

/**
 * kQAContext_xxx are ORed together to generate the 'flags' parameter for
 * QADrawContextNew().
 */
enum {
  ...
  kQAContext_Scale =
      (1 << 5), /* The draw context is to be scaled.  The front buffer is a
                   different size than the back buffer. */
  kQAContext_NonRelocatable =
      (1 << 6), /* The back buffer and the z buffer must not move in memory */
...
enum {
  ...
  kQAOptional_TextureHQ =
      (1 << 2), /* High quality texture (tri-linear mip or better) */
  kQAOptional_TextureColor =
      (1 << 3), /* Full color modulation and highlight of textures */
  kQAOptional_Blend = (1 << 4), /* Transparency blending of RGB */
  kQAOptional_BlendAlpha =
      (1 << 5), /* Transparency blending includes alpha channel */
  kQAOptional_Antialias = (1 << 6), /* Antialiased rendering */
  kQAOptional_ZSorted =
      (1 << 7), /* Z sorted rendering (for transparency, etc.) */
  kQAOptional_PerspectiveZ =
      (1 << 8), /* Hidden surface removal using InvW instead of Z */
  kQAOptional_OpenGL =
      (1 << 9), /* Extended rasterization features for OpenGL� */
  kQAOptional_NoClear =
      (1 << 10), /* This drawing engine doesn't clear before drawing */
  kQAOptional_CSG = (1 << 11), /* kQATag_CSGxxx are implemented */
  kQAOptional_BoundToDevice =
      (1 << 12),               /* This engine is tightly bound to GDevice */
  ...
  kQAOptional_BufferComposite =
      (1 << 15), /* This engine can composite with initial buffer contents */
  ...
  kQAOptional_MultiTextures =
      (1 << 19), /* This bit set if engine supports texture compositing */
  kQAOptional_MipmapBias = (1 << 20), /* This bit is set if the engine supports
  ...
  kQAOptional_AccessTexture =
      (1 << 24), /* if engine supports access to texture */
  kQAOptional_AccessBitmap =
      (1 << 25), /* if engine supports access to bitmaps */
  kQAOptional_AccessDrawBuffer =
      (1 << 26), /* if engine supports access to draw buffer */
  kQAOptional_AccessZBuffer =
      (1 << 27), /* if engine supports access to zbuffer */
  kQAOptional_ClearDrawBuffer =
      (1 << 28), /* if engine supports QAClearDrawBuffer() */
  kQAOptional_ClearZBuffer =
      (1 << 29), /* if engine supports QAClearZBuffer() */
  kQAOptional_OffscreenDrawContexts =
      (1 << 30) /* if engine supports TQADeviceOffscreen */
};
...
enum {
  ...
  kQAOptional2_TextureDrawContexts =
      (1 << 1), /* if engine supports QATextureNewFromDrawContext() */
  kQAOptional2_BitmapDrawContexts =
      (1 << 2), /* if engine supports QABitmapNewFromDrawContext() */
  ...
  kQAOptional2_Chromakey =
      (1 << 5), /* if engine supports chromakeying via kQATag_Chromakey_xxx */
  kQAOptional2_NonRelocatable =
  ...
  kQAOptional2_PriorityBits =
      (1 << 8), /* if engine supports texture & bitmap priority levels */
  kQAOptional2_FlipOrigin = (1 << 9), /* if engine supports textures & bitmaps
                                         that are vertically flipped */
  kQAOptional2_BitmapScale =
      (1 << 10), /* if engine supports scaled bitmap drawing */
  kQAOptional2_DrawContextScale =
      (1 << 11), /* if engine supports scaled draw contexts */
...
enum {
  ...
  kQAFast_TextureHQ =
      (1 << 3), /* High quality texture (tri-linear mip or better) */
  ...
  kQAFast_MultiTextures =
      (1 << 11), /* This engine accelerates texture compositing */
  kQAFast_BitmapScale =
      (1 << 12), /* This engine accelerates scaled bitmap drawing */
  kQAFast_DrawContextScale =
 ...
 *
 * The priority value must be a number between 0 and 15.  0 is top priority, and
 *15 is lowest priority. Note that this is different than OpenGL which uses
 *floats from 0.0 to 1.0 where 1.0 is top priority. In RAVE, this system is more
 *like texture ranking, thus 0 is top rank, and 15 is the last ranking.
 *
 ...
 *
 *         unsigned long flags = kQATexture_Mipmap;            // Initialize
 *flags
 *
 *       flags |= QACalculatePriorityBits(0);                // Setting top
 *priority (this actually does
 *                                                          // nothing since the
 *bits are still 0000) QATextureNew(myEngine, flags, pixelType, image,
 *&raveTexture);
 *
 *
 ...
 *
 *      drawContext = QADrawContextNew (rect, gdevice, engine,
 *kQAContext_ZBuffer);
 *      ...
 ...
 *handling NULL parameters to QARenderStart().
 *
 ***********************************************************************************************/
...
enum TQAVersion {
  ...
  kQAVersion_1_5 =
      3, /* Added call backs, texture compression, and new error return code */
...
/************************************************************************************************
 *
 * Typedefs of draw method functions provided by the drawing engine. One
 *function pointer for each of these function types in stored in the
 *TQADrawContext public data structure.
 *
...
typedef CALLBACK_API_C(void,
                       TQABufferNoticeMethod)(const TQADrawContext *drawContext,
                                              const TQADevice *buffer,
                                              const TQARect *dirtyRect,
                                              void *refCon);
union TQANoticeMethod {
  TQAStandardNoticeMethod
      standardNoticeMethod; /* Used for non-buffer related methods */
...
typedef CALLBACK_API_C(void, TQADrawTriTexture)(
    ...
    const TQAVTexture *v1, const TQAVTexture *v2, unsigned long flags);
...
typedef CALLBACK_API_C(void, TQASubmitVerticesTexture)(
    const TQADrawContext *drawContext, unsigned long nVertices,
...
typedef CALLBACK_API_C(TQAError,
                       TQARenderEnd)(const TQADrawContext *drawContext,
...
typedef CALLBACK_API_C(TQAError,
                       TQASetNoticeMethod)(const TQADrawContext *drawContext,
...
struct TQADrawContext {
  ...
  TQARenderAbort
      renderAbort; /* Method: Abort any outstanding rendering (blocking) */
  TQAFlush
      flush;    /* Method: Start render of any queued commands (non-blocking) */
  TQASync sync; /* Method: Wait for completion of all rendering (blocking) */
  TQASubmitVerticesGouraud
      submitVerticesGouraud; /* Method: Submit Gouraud vertices for trimesh */
  TQASubmitVerticesTexture
      submitVerticesTexture; /* Method: Submit Texture vertices for trimesh */
  TQADrawTriMeshGouraud
      drawTriMeshGouraud; /* Method: Draw a Gouraud triangle mesh */
  TQADrawTriMeshTexture
      drawTriMeshTexture;             /* Method: Draw a Texture triangle mesh */
  ...
  TQASubmitMultiTextureParams
      submitMultiTextureParams; /* Method: Submit Secondary texture params */
