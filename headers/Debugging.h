/**
    ...
    \brief   Macros to handle exceptions and assertions.
...
/**
  ...
  emitted
    ...
    #defining DEBUG to 0 is equivalent to #defining PRODUCTION 1 when DEBUG
  is 1. (No code for debugger breaks is emitted in either case.)
...
/**
  ______________________________________________________________________________________
    ...
    DEBUGASSERTMSG - all error reporting is routed through this macro, which
  calls the system routine DebugAssert().  If you wish to use your own
  assertion/debugger break routine, you can override DEBUGASSERTMSG by defining
...
#pragma pack(2)
   ...
   *    \non_carbon_cfm   in DebugLib 1.0 and later
...
#define DEBUGASSERTMSG(componentSignature, options, assertionString,       \
                       exceptionString, errorString, fileName, lineNumber, \
                       value)                                              \
  Debugger()
...
#define DEBUGASSERTMSG(componentSignature, options, assertionString,       \
                       exceptionString, errorString, fileName, lineNumber, \
                       value)                                              \
  DebugAssert(componentSignature, options, kComponentSignatureString,      \
              assertionString, NULL, NULL, 0, value)
#else
#define DEBUGASSERTMSG(componentSignature, options, assertionString,       \
                       exceptionString, errorString, fileName, lineNumber, \
                       value)                                              \
  DebugAssert(componentSignature, options,                                 \
              kComponentSignatureString ": " assertionString, NULL, NULL,  \
              NULL, 0, value)
#endif
...
#define DEBUGASSERTMSG(componentSignature, options, assertionString,       \
                       ...
                       value)                                              \
  DebugAssert(componentSignature, options, kComponentSignatureString,      \
              assertionString, NULL, fileName, lineNumber, value)
#else
#define DEBUGASSERTMSG(componentSignature, options, assertionString,           \
                       exceptionString, errorString, fileName, lineNumber,     \
                       value)                                                  \
  DebugAssert(componentSignature, options,                                     \
              kComponentSignatureString ": " assertionString, exceptionString, \
  ...
  */
  ...
  enum
  {
    kComponentDebugOption =
        0 /** optionSelectorNum to turn breaks for component on/off*/
  };
  ...
  /**
      ...
      DebugComponentCallback is the callback into a component that registers with
     DebugLib. It is called to get the debug option setting, or to turn a debug
     option on or off. Inputs: optionSelectorNum   The component debug option to
     set command             The command: kGetDebugOption     - get current debug
     option setting kSetDebugOption     - set debug option Outputs: optionSetting
     The current setting if kGetDebugOption; the new debug option if
     kSetDebugOption
  */
  typedef CALLBACK_API(void,
                       DebugComponentCallbackProcPtr)(SInt32 optionSelectorNum,
                                                      UInt32 command,
  ...
  /**
      ...
      If TaskLevel doesn't return 0, then the following masks can be used to learn
     more: k68kInterruptLevelMask      = 0x00000007 kInVBLTaskMask              =
     0x00000010 kInDeferredTaskMask         = 0x00000020
          kInSecondaryIntHandlerMask  = 0x00000040
          kInNestedInterruptMask      = 0x00000080
  */
  /**
...
#define ATTASKLEVEL0() (TaskLevel() == 0)
  ...
  /**
      ...
      NewDebugComponent registers a component with DebugLib.
          Inputs:
              ...
              componentCallback   The callback into component for working with
     options Result: noErr                           no error memFullErr could not
     allocate memory debuggingExecutionContextErr    routine cannot be called at
     this time debuggingDuplicateSignatureErr  componentSignature already
     registered debuggingInvalidNameErr         componentName is invalid (NULL)
  */
  /**
   *  NewDebugComponent()
   *
  ...
  /**
      ...
      NewDebugOption registers a debug option with DebugLib.
          Inputs:
              componentSignature  The signature of component to register a debug
     option for optionSelectorNum   The selector number of this debug option
  ...
  */
  ...
  /**
      ...
      DisposeDebugComponent removes a component registration and all related debug
     options from DebugLib. Input: componentSignature  The unique signature of a
     component Result: noErr                           no error
              debuggingExecutionContextErr    called at interrupt time
              debuggingInvalidSignatureErr    componentSignature not registered
  */
  /**
  ...
  /**
      ...
      GetDebugComponentInfo returns a component registered with DebugLib.
          Inputs:
              index               The index into the list of registered components
     (1-based) Outputs: componentSignature  The unique signature of a component
              componentName       The displayable string naming a component
          Result:
              noErr                           no error
              debuggingNoMatchErr             debugging component not found at
     this index
  */
  /**
   *  GetDebugComponentInfo()
  ...
  /**
      ...
      GetDebugOptionInfo returns a debug option registered with DebugLib.
          Inputs:
              index               The index into the list of registered debug
     options (0-based); 0 = kComponentDebugOption componentSignature  The unique
     signature of a component Outputs: optionSelectorNum   The selector number of
     this debug option optionName          The displayable string naming this
     debug option optionSetting       The current debug option setting Result:
              noErr                           no error
              debuggingInvalidSignatureErr    componentSignature not registered
  ...
  */
  ...
  /**
      ...
      DebugAssertOutputHandler is the callback that registers with DebugLib to
     handle the output from DebugAssert. Inputs: "componentSignature" through
     "value" are the raw values passed to DebugAssert when an exception occurs.
              outputMsg is the string DebugAssert build which would normally be
     passed to DebugStr if a DebugAssertOutputHandler isn't installed.
  */
  typedef CALLBACK_API(void, DebugAssertOutputHandlerProcPtr)(
      OSType componentSignature, UInt32 options, const char *assertionString,
      const char *exceptionString, const char *errorString, const char *fileName,
      long lineNumber, void *value, ConstStr255Param outputMsg);
  typedef STACK_UPP_TYPE(DebugAssertOutputHandlerProcPtr)
      DebugAssertOutputHandlerUPP;
  /**
      InstallDebugAssertOutputHandler
      InstallDebugAssertOutputHandler installs a DebugAssertOutputHandler which
     DebugAssert calls instead of DebugStr. Inputs: handler     the
     DebugAssertOutputHandler to install or NULL to switch back to the default
     handler (DebugStr).
  */
  /**
   *  InstallDebugAssertOutputHandler()
...
/**
   ...
   output handler.  Calling dprintf() from anything but C or C++ is tricky.
*/
   ...
   *    \non_carbon_cfm   in DebugLib 1.1 and later
...
#if DEBUG
  /**  For debug builds, you can use DPRINTF as a dprintf which goes away in
   * non-debug builds.  To use it, double-parenthesize the argument - ie, use
   *
   *  DPRINTF(("formatString %d", 5 ));
   *
   *  This is sadly necessary because a macro can not have a variable number of
   * arguments.
   *
   */
...
#define DPRINTF(x) \
  ...
  enum
  {
    uppDebugComponentCallbackProcInfo = 0x00000FC0
  }; /** pascal no_return_value Func(4_bytes, 4_bytes, 4_bytes) */
#ifdef __cplusplus
  inline DebugComponentCallbackUPP
  NewDebugComponentCallbackUPP(DebugComponentCallbackProcPtr userRoutine)
  {
    return (DebugComponentCallbackUPP)NewRoutineDescriptor(
        (ProcPtr)(userRoutine), uppDebugComponentCallbackProcInfo,
        GetCurrentArchitecture());
  }
#else
#define NewDebugComponentCallbackUPP(userRoutine)                \
  (DebugComponentCallbackUPP) NewRoutineDescriptor(              \
      (ProcPtr)(userRoutine), uppDebugComponentCallbackProcInfo, \
      GetCurrentArchitecture())
#endif
#endif
...
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
  inline void InvokeDebugComponentCallbackUPP(SInt32 optionSelectorNum,
                                              UInt32 command,
                                              Boolean *optionSetting,
                                              DebugComponentCallbackUPP userUPP)
  {
    CALL_THREE_PARAMETER_UPP(userUPP, uppDebugComponentCallbackProcInfo,
                             optionSelectorNum, command, optionSetting);
  }
#else
#define InvokeDebugComponentCallbackUPP(optionSelectorNum, command,      \
                                        optionSetting, userUPP)          \
  CALL_THREE_PARAMETER_UPP((userUPP), uppDebugComponentCallbackProcInfo, \
                           (optionSelectorNum), (command), (optionSetting))
#endif
#endif
...
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
  inline void InvokeDebugAssertOutputHandlerUPP(
      OSType componentSignature, UInt32 options, const char *assertionString,
      const char *exceptionString, const char *errorString, const char *fileName,
      long lineNumber, void *value, ConstStr255Param outputMsg,
      DebugAssertOutputHandlerUPP userUPP)
  {
    CALL_NINE_PARAMETER_UPP(userUPP, uppDebugAssertOutputHandlerProcInfo,
                            componentSignature, options, assertionString,
                            exceptionString, errorString, fileName, lineNumber,
                            value, outputMsg);
  }
#else
#define InvokeDebugAssertOutputHandlerUPP(                                    \
    componentSignature, options, assertionString, exceptionString,            \
    errorString, fileName, lineNumber, value, outputMsg, userUPP)             \
  CALL_NINE_PARAMETER_UPP((userUPP), uppDebugAssertOutputHandlerProcInfo,     \
                          (componentSignature), (options), (assertionString), \
                          (exceptionString), (errorString), (fileName),       \
                          (lineNumber), (value), (outputMsg))
#endif
#endif

#if CALL_NOT_IN_CARBON || OLDROUTINENAMES
/** support for pre-Carbon UPP routines: New...Proc and Call...Proc */
#define NewDebugComponentCallbackProc(userRoutine) \
  NewDebugComponentCallbackUPP(userRoutine)
#define NewDebugAssertOutputHandlerProc(userRoutine) \
  NewDebugAssertOutputHandlerUPP(userRoutine)
#define CallDebugComponentCallbackProc(userRoutine, optionSelectorNum,       \
                                       command, optionSetting)               \
  InvokeDebugComponentCallbackUPP(optionSelectorNum, command, optionSetting, \
                                  userRoutine)
#define CallDebugAssertOutputHandlerProc(                                 \
    userRoutine, componentSignature, options, assertionString,            \
    exceptionString, errorString, fileName, lineNumber, value, outputMsg) \
  InvokeDebugAssertOutputHandlerUPP(                                      \
      componentSignature, options, assertionString, exceptionString,      \
...
#define CapsLockDebugStr(str)                                \
  ...
  /** DEBUGGER and DEBUGSTR call the debugger if it is installed ONLY in DEBUG
   * builds. */
...
#define require_noerr_action_quiet(assertion, label, action) \
  require_noerr_action(assertion, label, action)
...
#define require_action_quiet(assertion, label, action) \
...
#define debug_string(string)                                           \
  ...
  {                                                                    \
    DEBUGASSERTMSG(COMPONENT_SIGNATURE, DEBUG_NO_OPTIONS,              \
                   QuoteExceptionString(string), NULL, NULL, __FILE__, \
                   __LINE__, 0);                                       \
  } while (false)
...
#define check(assertion)                                                    \
  ...
  {                                                                         \
    ...
    {                                                                       \
      DEBUGASSERTMSG(COMPONENT_SIGNATURE, DEBUG_NO_OPTIONS,                 \
                     QuoteExceptionString(assertion), NULL, NULL, __FILE__, \
                     __LINE__, 0);                                          \
    }                                                                       \
  } while (false)
...
#define check_string(assertion, cstring)                                       \
  ...
  {                                                                            \
    ...
    {                                                                          \
      DEBUGASSERTMSG(COMPONENT_SIGNATURE, DEBUG_NO_OPTIONS,                    \
                     QuoteExceptionString(assertion), cstring, NULL, __FILE__, \
                     __LINE__, 0);                                             \
    }                                                                          \
  } while (false)
  ...
  {                                                                   \
    ...
    {                                                                 \
      DEBUGASSERTMSG(COMPONENT_SIGNATURE, DEBUG_NO_OPTIONS,           \
                     ...
                     QuoteExceptionString(exception), NULL, __FILE__, \
                     __LINE__, 0);                                    \
      goto exception;                                                 \
    }                                                                 \
  } while (false)
...
#define require_string(assertion, exception, string)                          \
  ...
  {                                                                           \
    if (!(assertion))                                                         \
    {                                                                         \
      DEBUGASSERTMSG(COMPONENT_SIGNATURE, DEBUG_NO_OPTIONS, string,           \
                     QuoteExceptionString(assertion),                         \
                     QuoteExceptionString(exception), __FILE__, __LINE__, 0); \
      goto exception;                                                         \
    }                                                                         \
  } while (false)
...
/**    Define these in terms of the check() and require macros.  In non-debug
 builds, the check calls go away and the require macros are mostly checks and
 jumps.
*/
...
#define require_tasklevel0_string(label, string) \
  require_string(ATTASKLEVEL0(), label, string)
#define require_tasklevel0_action(label, string, action) \
  require_action(ATTASKLEVEL0(), label, action)
#define require_tasklevel0_action_string(label, string, action) \
  require_action_string(ATTASKLEVEL0(), label, action, string)
...
#define nrequire_quiet(assertion, exception) \
  require_quiet(!(assertion), exception)
#define nrequire_action_quiet(assertion, exception, action) \
  require_action_quiet(!(assertion), exception, action)
#define nrequire_string(assertion, exception, string) \
  require_string(!(assertion), exception, string)
