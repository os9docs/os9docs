/**
    ...
    \brief   QuickDraw GX object and constant definitions
...
#define gxAnyNumber 1
  ...
  enum
  {
    gxUnderlineAdvanceLayer =
        0x0001, /** a gxLine is drawn through the advances */
    gxSkipWhiteSpaceLayer =
        0x0002,                        /** except characters describing white space */
    gxUnderlineIntervalLayer = 0x0004, /** (+ gxStringLayer) a gxLine is drawn
                                          through the gaps between advances */
    gxUnderlineContinuationLayer =
        0x0008,            /** (+ gxStringLayer) join this underline with another face */
  ...
  enum
  {
    gxRejectSourceTransfer =
        0x0001, /** at least one component must be out of range       */
    gxRejectDeviceTransfer =
        0x0002,                        /** at least one component must be out of range       */
    gxSingleComponentTransfer = 0x0004 /** duplicate gxTransferComponent[0] for
                                          all components in transfer */
  };
  ...
  enum
  {
    gxOverResultComponent =
        0x01,                 /** & result gxColor with 0xFFFF before clamping   */
  ...
  struct gxTransferComponent
  {
    ...
    gxColorValue
        operand; /** ratio for blend, step for migrate, gxColor for highlight   */
  };
  typedef struct gxTransferComponent gxTransferComponent;
  struct gxTransferMode
  {
    gxColorSpace
        space; /** the gxColor-space the transfer mode is to operate in */
    gxColorSet set;
    gxColorProfile profile;
  ...
  enum
  {
    ...
    gxAlphaFirstPacking =
        0x1000 /** alpha channel is the first field in the packed space */
  };
  ...
  enum
  {
    ...
    gxAnyPart = gxBoundsPart | gxGeometryPart | gxPenPart | gxCornerPointPart |
                gxControlPointPart | gxEdgePart | gxJoinPart | gxStartCapPart |
  ...
  enum
  {
    ...
    gxAverageTint =
        2,             /** add all the components and divide by the number of components */
    gxMixtureTint = 3, /** find the closest gxColor on the axis between the
                          foreground and background */
    gxComponent1Tint =
        4,                /** use the value of the first component of the gxColor */
...
#define gxScreenViewDevices ((gxViewGroup)1)
  ...
  struct gxSpoolBlock
  {
    ...
    long bufferSize;                   /** how many bytes for the system to read (flatten) / write
                                          (unflatten) */
                                       /** these fields are written to (but are not read from) */
    long count;                        /** how many bytes for the caller to read (unflatten) /write
                                          (flatten) */
    long operationSize;                /** operation size (including operand byte) */
    long operationOffset;              /** the data offset, if any, within the current
                                          operation */
    gxGraphicsOpcode lastTypeOpcode;   /** type of last created object */
    gxGraphicsOpcode currentOperation; /** operation emitted by flatten, or
                                          intrepreted by last unflatten */
    gxGraphicsOpcode
        currentOperand;       /** e.g., gxTransformTypeOpcode, gxInkTagOpcode */
  ...
  enum
  {
    uppgxSpoolProcInfo = 0x000003F1
  }; /** 4_bytes Func(4_bytes, 4_bytes) */
#ifdef __cplusplus
  inline gxSpoolUPP NewgxSpoolUPP(gxSpoolProcPtr userRoutine)
  {
    return (gxSpoolUPP)NewRoutineDescriptor(
        (ProcPtr)(userRoutine), uppgxSpoolProcInfo, GetCurrentArchitecture());
  }
#else
#define NewgxSpoolUPP(userRoutine)   \
  (gxSpoolUPP) NewRoutineDescriptor( \
      (ProcPtr)(userRoutine), uppgxSpoolProcInfo, GetCurrentArchitecture())
#endif
#endif
...
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
  inline void DisposegxSpoolUPP(gxSpoolUPP userUPP)
  {
...
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
  inline long InvokegxSpoolUPP(gxSpoolCommand command, gxSpoolBlock *block,
                               gxSpoolUPP userUPP)
  {
    return (long)CALL_TWO_PARAMETER_UPP(userUPP, uppgxSpoolProcInfo, command,
                                        block);
  }
#else
...
#define CallgxSpoolProc(userRoutine, command, block) \
  InvokegxSpoolUPP(command, block, userRoutine)
#endif /** CALL_NOT_IN_CARBON */

  enum
  {
    gxFontListFlatten = 0x01,       /** if set, generate a gxTag containing list of each
                                       gxFont referenced */
    gxFontGlyphsFlatten = 0x02,     /** if set, generate a gxTag containing the list
                                       of glyphs referenced inside the gxFont */
    gxFontVariationsFlatten = 0x04, /** if set, append the gxTag with separate
                                       [variation] coordinates */
    gxBitmapAliasFlatten = 0x08     /** if set, open bitmap alias files and flatten
                                       out their image data */
  };
...
#define gxGraphicsSystemClient (gxGraphicsClient)(-1)
  ...
  /** The actual constants for feature types and selectors have been moved to a
   * library. */
  typedef unsigned short gxRunFeatureType;
  typedef unsigned short gxRunFeatureSelector;
  /** If tracking is not desired, specify the following value in the track field
     in the gxRunControls record (note that a track of 0 does *not* mean to turn
     tracking off; rather, it means to use normal tracking). */
  enum
  {
    gxNoTracking = gxNegativeInfinity
  };

  /** The special "gxNoStake" value is returned by the GXGetLayoutBreakOffset call
     to indicate the absence of a character offset that is stable with respect to
  ...
  /** A glyph's behavior with respect to other glyphs on its line is defined in
     part by its gxBaselineType. These types allow correct alignment of the
     baselines of all glyphs on the line. */
  enum
  {
    gxRomanBaseline = 0,
  ...
  /** gxJustificationPriority defines the point during the justification process
     at which a glyph will begin to receive deltas before and after itself. */
  enum
  {
    gxKashidaPriority = 0,
  ...
  /** gxJustificationFlags are used to control which fields of a
     gxWidthDeltaRecord are to be overridden and which are not if a
     gxPriorityJustificationOverride or gxGlyphJustificationOverride (qq.v.) is
     specified. */
  enum
  {
    ...
    gxAllJustificationFlags =
        (gxOverridePriority | gxOverrideLimits | gxOverrideUnlimited |
         gxUnlimitedGapAbsorption | gxJustificationPriorityMask)
  };

  typedef unsigned short gxJustificationFlags;
  /** The directional behavior of a glyph can be overridden using a
   * gxDirectionOverride. */
  enum
  {
  ...
  /** gxRunControlFlags describe the nonparametric layout options contained in a
   * gxStyle. */
  enum
  {
  ...
  enum
  {
    gxAllRunControlFlags =
        (gxNoLigatureSplits | gxNoCaretAngle | gxImposeWidth | gxNoCrossKerning |
         gxNoOpticalAlignment | gxForceHanging | gxNoSpecialJustification |
         gxDirectionOverrideMask)
  };

  typedef unsigned long gxRunControlFlags;
  /** gxHighlightType is used to distinguish various highlighting methods, both in
     terms of character offset based vs. visual based, and in terms of angled
     sides vs. non-angled sides. */
  enum
  {
  ...
  /** gxCaretType is used to control whether the caret that is returned from
     GXGetLayoutCaret is a split caret or a (keyboard-syncronized) single caret.
   */
  enum
  {
  ...
  /** gxLayoutOffsetState describes the characteristics of a given gxByteOffset in
     some layout. It is returned by the GXGetOffsetGlyphs call. Note that the
          gxOffsetInsideLigature value is returned in addition to the 8/16 (or
     invalid) indication. */
  enum
  {
    gxOffset8_8 = 0,
  ...
  /** gxLayoutOptionsFlags are single-bit flags contained in a gxLayoutOptions
     record. We also define here some utility constants that are useful in setting
     various fields in the gxLayoutOptions record. */
  enum
  {
    gxNoLayoutOptions = 0,
    ...
    gxAllLayoutOptionsFlags = gxLineIsDisplayOnly | gxKeepSpacesInMargin |
                              gxLimitReorderingToTwoLevels |
                              gxLineLeftEdgeNotAtMargin |
                              gxLineRightEdgeNotAtMargin,
    gxMaxRunLevel = 15,
    gxFlushLeft = 0,
  ...
  /** A gxWidthDeltaRecord contains all of the information needed to describe the
     behavior of one class of glyphs during the justification process. */
  ...
  /** A gxPriorityJustificationOverride contains an array of WidthDeltaRecords,
     one for each gxJustificationPriority. */
  ...
  /** A gxGlyphJustificationOverride contains a gxWidthDeltaRecord that is to be
     used for a specific glyph in a specific run (this limitation is because
     glyphcodes vary by font). */
  ...
  /** gxRunControls contains flags, shifts, imposed widths and overrides for a
   * run. */
  ...
  /** A gxGlyphSubstitution describes one client-provided substitution that occurs
     after all other automatic glyph changes have happened. */
  ...
  /** gxKerningAdjustmentFactors specify an adjustment to automatic kerning. The
     adjustment is ax + b where x is the automatic kerning value, a is
     scaleFactor, and b is adjustmentPointSizeFactor times the run's point size.
   */
  ...
  /** A value of gxResetCrossStreamFactor in
     crossStreamFactors.adjustmentPointSizeFactor will reset the cross-stream
     kerning to the baseline. */
  enum
  {
  ...
  /** A gxLineBaselineRecord contains the deltas from 0 to all the different
     baselines for the layout. It can be filled via a call to GetBaselineDeltas
     (q.v.). */
  ...
  enum
  {
    ...
    gxNextOpcode =
        0xFF /** used by currentOperand field to say opcode is coming */
  };
  ...
  {
    gxHeaderTypeOpcode =
        0x00, /** byte following new object uses bottom 6 bits for type */
  ...
  /**    format of top byte:
  xx yyyyyy   xx == 0x00, 0x40, 0x80, 0xC0: defines graphics operation (see
  gxGraphicsOperationOpcode) yyyyyy == size of operation in bytes if (yyyyyy ==
  0), byte size follows. If byte following == 0, word size follows; if == 0, long
  ...
  xx yyyyyy   xx == 0x00, 0x40, 0x80, 0xC0: defines compression level (0 == none,
  0xC0 == most) exact method of compression is defined by type of data yyyyyy ==
  data type selector (0 to 63): see gxGraphicsNewOpcode, __DataOpcode
  */
  enum
  {
    gxOpcodeShift = 6,
  ...
  {
    gxFont fontID;          /**** if we get rid of this, remove #include "font types.h",
                               above */
    unsigned char name;     /** gxFontName */
    unsigned char platform; /** gxFontPlatform */
    unsigned char script;   /** gxFontScript */
    ...
    unsigned short
        glyphCount;                /** CountFontGlyphs or 0 if gxFontGlyphsFlatten is false */
    unsigned short axisCount;      /** CountFontVariations or 0 if
                                      gxFontVariationsFlatten is false */
    unsigned short variationCount; /** number of bitsVariationPairs that follow
                                      the (optional) glyphBits */
  };
  ...
  enum
  {
    ...
    gxPSContinueNext = 2 /** next shape is continuation of this shape's PS -- only
                            obeyed if gxNoSave is true */
  };
  ...
  enum
  {
    ...
    gxCurveToFlag =
        0x0002, /** Draw a gxCurve through the 3 points following this flag */
    gxMoveToFlag =
        0x0003,              /** Start a new contour at the gxPoint following this flag */
  ...
  struct gxPatternSynonym
  {
    ...
    gxPoint anchorPoint; /** gxPoint with with respect to which pattern position
                            is calculated */
  };
