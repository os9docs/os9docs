...
/* CustomBadgeResource data structure you must clear the
...
/* You can specify routing information for a file by including a 'rout' 0
    resource in it and setting the kExtendedFlagHasRoutingInfo bit in the
   extended Finder flags. The 'rout' resource is an array of
   RoutingResourceEntry. Each entry is considered in turn. The first matching
   entry is used. If the creator and fileType match the file being dropped and
   targetFolder match the folder ID of the folder being dropped onto, then the
   file is rerouted into the specified destination folder. The only target
   folder currently supported is the system folder, kSystemFolderType = 'macs'.
*/
...
enum {
  kContainerFolderAliasType =
      FOUR_CHAR_CODE('fdrp'), /* type for folder aliases */
  kContainerTrashAliasType =
      FOUR_CHAR_CODE('trsh'), /* type for trash folder aliases */
  kContainerHardDiskAliasType =
      FOUR_CHAR_CODE('hdsk'), /* type for hard disk aliases */
  kContainerFloppyAliasType =
      FOUR_CHAR_CODE('flpy'), /* type for floppy aliases */
  kContainerServerAliasType =
      FOUR_CHAR_CODE('srvr'), /* type for server aliases */
  kApplicationAliasType =
      FOUR_CHAR_CODE('adrp'), /* type for application aliases */
  kContainerAliasType =
      FOUR_CHAR_CODE('drop'), /* type for all other containers */
  kDesktopPrinterAliasType =
      FOUR_CHAR_CODE('dtpa'), /* type for Desktop Printer alias */
  kContainerCDROMAliasType = FOUR_CHAR_CODE('cddr'), /* type for CD-ROM alias */
  kApplicationCPAliasType =
      FOUR_CHAR_CODE('acdp'), /* type for application control panel alias */
  kApplicationDAAliasType =
      FOUR_CHAR_CODE('addp'), /* type for application DA alias */
  kPackageAliasType = FOUR_CHAR_CODE('fpka'), /* type for plain package alias */
  kAppPackageAliasType =
      FOUR_CHAR_CODE('fapa') /* type for application package alias */
};
...
/* that the item is not a folder by checking ((ParamBlockRec.ioFlAttrib &
 * ioDirMask) == 0) */
enum {
...
enum {
  kExtendedFlagsAreInvalid =
      0x8000, /* If set the other extended flags are ignored */
  kExtendedFlagHasCustomBadge =
      0x0100, /* Set if the file or folder has a badge resource */
