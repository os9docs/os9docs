...
#pragma pack(2)
    ...
    */
    ...
    {
         /** language */
code values for Language Order    systemCurLang  =  -2   /** (itlbLang) */
lang for system script    systemDefLang  =  -3   /** (table) */
lang for system script    currentCurLang  =  -4  /** (itlbLang) */
lang for current script    currentDefLang  =  -5  /** lang */
for current script    scriptCurLang  =  -6  // current (itlbLang) lang for specified script    scriptDefLang = -7   // default language for a specified script
    };
 ...
 EqualString(strA, strB, FALSE, FALSE);  // returns TRUE
    If caseSens = FALSE then both strings are treated as if they had been
                      ...
                      UprString(though the original contents are not modified)
                          .The RelString function[128K ROMs] is more flexible in that its return code identifies which string is higher or
                  lower in the collating sequence.The IUEqualString and IUCompString functions take into consideration special spelling conventions used in foreign languages.</pre>
                  \copyright THINK Reference © 1991 - 1992 Symantec Corporation
                  \non_carbon_cfm in InterfaceLib 7.1 and later
                  \carbon_lib in CarbonLib 1.0 and later
                  \mac_os_x in version 10.0 and later
