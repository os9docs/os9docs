/**
    ...
    \brief   Definitions for Open Transport kernel code, such as drivers and
   protocol modules.
...
public:
  ...
  //
  const TStreamMessage *_MDECL GetBlockAt(OTByteCount &offset) const;
  const void *_MDECL GetPointerTo(OTByteCount offset, OTByteCount *len) const;
  const void *_MDECL GetPointerTo(OTByteCount offset, OTByteCount *len,
                                  void *bfr) const;
  //
...
inline void FreeTimerMsg(TTimerMessage *msg) { mi_timer_free(msg); }
