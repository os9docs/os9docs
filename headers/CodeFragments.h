...
*/
\**
        ame The Code Fragment Manager API * /
...
#pragma pack(2)
  ...
  enum
  {
    ...
    kDropInAdditionCFrag =
        2,                    /* Application or library private extension/plug-in*/
    kStubLibraryCFrag = 3,    /* Import library used for linking only*/
    kWeakStubLibraryCFrag = 4 /* Import library used for linking only and will be
  ...
  /**
     ...
     followed by a 16 bit size in bytes. Only one standard extension type is
     defined at present, it is used by SOM's searching mechanism.
  */
...
#define NextCFragResourceSearchQualifier(searchQualifierPtr) \
  ...
  enum
  {
    ...
    kReferenceCFrag =
        0x0001, /* Try to use existing copy, increment reference counts.*/
    kFindCFrag =
        0x0002, /* Try find an existing copy, do not increment reference counts.*/
    kPrivateCFragCopy =
...
#define CFragHasFileLocation(where)                                            \
  ...
  FindSymbol(
      ...
      CFragSymbolClass * symClass) /* can be NULL */ THREEWORDINLINE(0x3F3C,
                                                                     0x0005,
                                                                     0xAA5A);
  ...
  EXTERN_API(OSErr)
  GetIndSymbol(
      CFragConnectionID connID, long symIndex, Str255 symName, /* can be NULL */
      Ptr *symAddr,                                            /* can be NULL */
      CFragSymbolClass *symClass) /* can be NULL */ THREEWORDINLINE(0x3F3C,
                                                                    0x0007,
                                                                    0xAA5A);
