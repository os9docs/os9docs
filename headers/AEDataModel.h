/**
    ...
    \brief   AppleEvent Data Model Interfaces.
...
enum {
  ...
  keyEventSourceAttr = FOUR_CHAR_CODE(
      'esrc'), /** this attribute is read only - returned as typeShortInteger */
  keyMissedKeywordAttr =
      FOUR_CHAR_CODE('miss'), /** this attribute is read only */
...
enum {
  kAENormalPriority =
      0x00000000,              /** post message at the end of the event queue */
  kAEHighPriority = 0x00000001 /** post message at the front of the event queue
                                  (same as nAttnMsg) */
};
...
enum {
  ...
  kAEDontReconnect = 0x00000080, /** don't reconnect if there is a sessClosedErr
                                    from PPCToolbox */
  kAEWantReceipt =
      0x00000200, /** (nReturnReceipt) sender wants a receipt of message */
  ...
  kAEAlwaysInteract = 0x00000030, /** server should always interact with user
                                     where appropriate */
  kAECanSwitchLayer = 0x00000040, /** interaction may switch layer */
  kAEDontRecord = 0x00001000,  /** don't record this event - available only in
                                  vers 1.0.1 and greater */
  kAEDontExecute = 0x00002000, /** don't send the event for recording -
                                  available only in vers 1.0.1 and greater */
  kAEProcessNonReplyEvents =
      0x00008000 /** allow processing of non-reply events while awaiting
                    synchronous AppleEvent reply */
};
...
/** a AECoercionHandlerUPP is by default a AECoerceDescUPP.  If you are
   registering a Ptr based coercion handler you will have to add a cast to
...
\carbon_lib
    ...
    **************************************************************************/
     ...
     *    \mac_os_x         in version 10.0 and later
     */
    EXTERN_API(OSErr) AECreateList(const void *factoringPtr, Size factoredSize,
 ...
 */
EXTERN_API(OSErr)
AEPutPtr(AEDescList *theAEDescList, long index, DescType typeCode,
 ...
 */
EXTERN_API(OSErr)
AESizeOfNthItem(const AEDescList *theAEDescList, long index, DescType *typeCode,
 ...
 */
EXTERN_API(OSErr)
AEPutArray(AEDescList *theAEDescList, AEArrayType arrayType,
...
/***************************************************************************
  ...
  of t\carbon_libvent. Since AppleEvent is a subtype of AERecord, the calls
 ...
 */
EXTERN_API(OSErr)
AEPutParamDesc(Appl\carbon_lib theAppleEvent, AEKeyword theAEKeyword,
 ...
 */
EXTERN_API(OSErr)
AEGetParamPtr(cons\carbon_libnt *theAppleEvent, AEKeyword theAEKeyword,
 ...
 */
EXTERN_API(OSErr)
AESizeOfParam(const AppleEvent *theAppleEvent, AEKe\carbon_lib theAEKeyword,
 ...
 *
on_car\carbon_libin CarbonAccessors.o 1.0 and later
 *    \carbon_lib        in CarbonLib 1.0 and later
 *    \mac_os_x         in version 10.0 and later
 */
EXTERN_API(OSErr)
