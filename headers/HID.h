...
#pragma pack(2)
  ...
  */
  ...
  enum
  {
    ...
    /* thought of as version 1 of HIDButtonCapabilities and
...
/* these are the constants to be passed to HIDGetDeviceInfo*/
  ...
  typedef CALLBACK_API_C(OSStatus, HIDOpenDeviceProcPtr)(
      HIDDeviceConnectionRef *outConnectionRef, UInt32 permissions,
      UInt32 reserved);
  /* When finished, 'close' a device*/
  typedef CALLBACK_API_C(OSStatus, HIDCloseDeviceProcPtr)(
  ...
  /**
     ...
     recieved on the first interrupt in pipe. The supplied refcon is also passed
     thru.
  */
  typedef CALLBACK_API_C(OSStatus, HIDInstallReportHandlerProcPtr)(
  ...
  typedef CALLBACK_API_C(OSStatus, HIDRemoveReportHandlerProcPtr)(
      HIDDeviceConnectionRef inConnectionRef);
  ...
  */
  ...
  typedef CALLBACK_API_C(OSStatus, HIDControlDeviceProcPtr)(
  ...
  */
  ...
  /**
     ...
     work with) vendorID is who wrote this HID device driver (hi word of 0 and USB
     vendorID is valid)
  */
  struct HIDDeviceDispatchTable
  {
    ...
    HIDGetReportProcPtr
        pHIDGetReport; /* new for vers 2, was reserved in version 1*/
    HIDSetReportProcPtr
        pHIDSetReport; /* new for vers 2, was reserved in version 1*/
    HIDControlDeviceProcPtr pHIDControlDevice;
    HIDGetIndexedStringProcPtr pHIDGetIndexedString; /* new for vers 3*/
    HIDGetSizedReportProcPtr pHIDGetSizedReport;     /* new for vers 3*/
  };
  ...
  /* HID Usage Tables */
  /*                                                                                            */
  /* The following constants are from the USB 'HID Usage Tables' specification,
   * revision 1.1rc3 */
  /* ******************************************************************************************
   */
  ...
  enum
  {
    ...
    kHIDPage_PID = 0x0F,                 /* USB Physical Interface Device definitions for force
    ...
    /* Reserved 0x88 - 0x8B */
    kHIDPage_BarCodeScanner = 0x8C, /* (Point of Sale) USB Device Class Definition
                                       for Bar Code Scanner Devices */
    kHIDPage_Scale =
        0x8D, /* (Point of Sale) USB Device Class Definition for Scale Devices */
              /* ReservedPointofSalepages 0x8E - 0x8F */
    kHIDPage_CameraControl =
        0x90, /* USB Device Class Definition for Image Class Devices */
    kHIDPage_Arcade =
        0x91, /* OAAF Definitions for arcade and coinop related Devices */
   ...
   * language by simply printing different */
  /* keycaps. One example is the Y key on a North American keyboard. In Germany
   * this is typically Z. Rather than changing */
  /* the keyboard firmware to put the Z Usage into that place in the descriptor
   * list, the vendor should use the Y Usage on */
  /* both the North American and German keyboards. This continues to be the
   * existing practice in the industry, in order to */
  /* minimize the number of changes to the electronics to accommodate
   * otherlanguages. */
  enum
  {
  ...
  |* where a value of 1 will turn on the indicator, and a value of 0 will turn it
  off. The exceptions are described below. */
  enum
  {
   ...
   * selection controls. System graphical user */
  /* interfaces typically employ a pointer and a set of hierarchical selectors to
   * select, move and otherwise manipulate */
  /* their environment. For these purposes the following assignment of
   * significance can be applied to the Button usages: */
  /* � Button 1, Primary Button. Used for object selecting, dragging, and double
  click activation. On MacOS, this is the
  |*   only button. Microsoft operating systems call this a logical left button,
  because it */
   ...
   * browse object properties. Exposed by systems */
   ...
   * seldom assigned functionality due to prevalence */
  /*   of two- and one-button devices. */
  /* � Buttons 4 -55. As the button number increases, its significance as a
   * selector decreases. */
  /* In many ways the assignment of button numbers is similar to the assignment of
   * Effort in Physical descriptors. */
  /* Button 1 would be used to define the button a finger rests on when the hand
   * is in the �at rest� position, that is, virtually */
  /* no effort is required by the user to activate the button. Button values
   * increment as the finger has to stretch to reach a */
  /* control. See Section 6.2.3, �Physical Descriptors,� in the HID Specification
   ...
   * to be declared without requiring individual */
  /* enumeration in the native usage page. For example, it is not necessary to
   * declare usages of Pointer 1, Pointer 2, and so */
  /* forth on the Generic Desktop page. When parsed, the ordinal instance number
   * is, in essence, concatenated to the usages */
  /* attached to the encompassing collection to create Pointer 1, Pointer 2, and
   * so forth. */
  /* For an example, see Section A.5, �Multiple Instances of a Control,� in
   ...
   * Indicator and LED: Usage Selected Indicator */
  /* usages. For example, a message-indicator LED would be identified by a
   * Telephony: Message usage declared as a Feature */
