/**
    ...
    \brief   CoreGraphics direct display
...
#define kCGDirectMainDisplay ((CGDirectDisplayID)NULL)
     ...
     *
     * If the activeDspys array is NULL, maxDisplays is ignored, and *dspyCnt is
     * filled in with the number of displays meeting the function's requirements.
     *
    ...
    /**
     * Try to find a display mode of specified depth with dimensions equal or
     * greater than specified. If no depth match is found, try for the next larger
     * depth with dimensions equal or greater than specified.  If no luck, then just
     * return the current mode.
     *
     * exactmatch, if not NULL, is set to 'true' if an exact match in width, height,
     * and depth is found, and 'false' otherwise. Returns NULL if display is
     * invalid.
     */
    /**
     ...
     * automatically reverts to the permanent setting made by Preferences when the
     * program terminates. The mode dictionary passed in must be a dictionary vended
     * by other CGDirectDisplay APIs such as CGDisplayBestModeForParameters() and
     * CGDisplayAvailableModes().
     */
    /**
     ...
     * through 1: value = Min + ((Max - Min) * pow(index, Gamma)) The resulting
     * values are converted to a machine specific format and loaded into hardware.
     */
    ...
    CGSetDisplayTransferByFormula(CGDirectDisplayID display, CGGammaValue redMin,
                                  ...
                                  CGGammaValue greenGamma, CGGammaValue blueMin,
                                  CGGammaValue blueMax, CGGammaValue blueGamma);
     ...
     * 'tableSize' indicates the number of entries in each table. The tables are
     * interpolated as needed to generate the number of samples needed by hardware.
     */
    /**
    ...
    /** Restore gamma tables of system displays to the user's ColorSync specified
     * values */
    /**
    ...
    /**
     * Wait until the beam position is outside the range specified by upperScanLine
     * and lowerScanLine. Note that if upperScanLine and lowerScanLine encompass the
     * entire display height, the function returns an error. lowerScanLine must be
     * greater than or equal to upperScanLine.
     *
     * Some display systems may not conventional video vertical and horizontal sweep
     * in painting. These displays report a kCGDisplayRefreshRate of 0 in the
     * CFDictionaryRef returned by CGDisplayCurrentMode().  On such displays, this
     ...
     *
     * Returns CGDisplayNoErr on success, and an error if display or upperScanLine
     * and lowerScanLine are invalid.
     *
     * The app should set the values of upperScanLine and lowerScanLine to allow
     * enough lead time for the drawing operation to complete.  A common strategy is
     * to wait for the beam to pass the bottom of the drawing area, allowing almost
     * a full vertical sweep period to perform drawing. To do this, set
     * upperScanLine to 0, and set lowerScanLine to the bottom of the bounding box:
     *  lowerScanLine = (CGBeamPosition)(cgrect.origin.y + cgrect.size.height);
     *
     * IOKit may implement this as a spin-loop on the beam position call used for
     * CGDisplayBeamPosition(). On such system the function is CPU bound, and
     * subject to all the usual scheduling pre-emption. In particular, attempting to
     * wait for the beam to hit a specific scanline may be an exercise in
     * frustration.
     *
     * These functions are advisary in nature, and depend on IOKit and hardware
     * specific drivers to implement support. If you need extremely precise timing,
     * or access to vertical blanking interrupts, you should consider writing a
     * device driver to tie into hardware-specific capabilities.
     */
    /**
     ...
     * sweep in painting, or the driver does not implement this functionality, 0 is
     * returned.
     */
    /**
