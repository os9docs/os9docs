/**
    ...
    \brief   AppleEvents Registry Interface.
...
enum {
  ...
  typeQDRegion = FOUR_CHAR_CODE(
      'Qrgn'), /**  0x51447074  (data is actual region data, including rectangle
                  and size, _not_ region handle or ptr)*/
...
enum {
  ...
  kUpdateActiveInputArea =
      FOUR_CHAR_CODE('updt'), /** update the active input area */
  kShowHideInputWindow =
      FOUR_CHAR_CODE('shiw'), /** show or hide the input window */
  kPos2Offset = FOUR_CHAR_CODE(
      'p2st'), /** converting global coordinates to char position */
  kOffset2Pos = FOUR_CHAR_CODE(
      'st2p'), /** converting char position to global coordinates */
  kUnicodeNotFromInputMethod =
      FOUR_CHAR_CODE('unim'), /** Unicode text when event not handled by Input
                                 Method or no Input Method */
  kGetSelectedText =
      FOUR_CHAR_CODE('gtxt'), /** Get text for current selection */
  keyAETSMDocumentRefcon =
      FOUR_CHAR_CODE('refc'), /** TSM document refcon, typeLongInteger */
  ...
  keyAETSMEventRecord = FOUR_CHAR_CODE(
      'tevt'), /** Low level Event Record, typeLowLevelEventRecord */
  keyAETSMEventRef =
      FOUR_CHAR_CODE('tevr'), /** Carbon EventRef, typeEventRef */
  keyAETextServiceEncoding = FOUR_CHAR_CODE(
      'tsen'), /** Text Service encoding, mac or Unicode in
                  UpdateActiveInputArea or GetSelectedText events. */
  keyAETextServiceMacEncoding =
      FOUR_CHAR_CODE('tmen'), /** Target mac encoding for TSM conversion of text
                                 from Unicode text service. */
  ...
  typeLowLevelEventRecord =
      FOUR_CHAR_CODE('evtr'),            /** Low Level Event Record */
...
struct TextRangeArray {
  ...
  TextRange fRange[1]; /** when fNumOfRanges > 1, the size of this array has to
                          be calculated */
};
...
struct OffsetArray {
  ...
  long fOffset[1]; /** when fNumOfOffsets > 1, the size of this array has to be
                      calculated */
};
...
enum {
  keyAEHiliteRange =
      FOUR_CHAR_CODE('hrng'),             /** typeTextRangeArray for System 7,
                                             typeHiliteRangeArray for System 8 */
  keyAEPinRange = FOUR_CHAR_CODE('pnrg'), /** typeTextRange for System 7,
                                             typeTextRegionRange for System 8 */
  keyAEClauseOffsets =
      FOUR_CHAR_CODE('clau'), /** typeOffsetArray for System 7,
                                 typeClauseOffsetArray for System 8 */
  keyAEOffset = FOUR_CHAR_CODE(
      'ofst'), /** typeLongInteger for System 7, typeByteOffset for System 8  */
  keyAEPoint = FOUR_CHAR_CODE(
      'gpos'), /** typePoint for System 7, typeQDPoint for System 8 */
  keyAELeftSide = FOUR_CHAR_CODE('klef'),    /** typeBoolean */
  keyAERegionClass = FOUR_CHAR_CODE('rgnc'), /** typeShortInteger for System 7,
                                                typeRegionClass for System 8 */
  keyAEDragging = FOUR_CHAR_CODE('bool')     /** typeBoolean */
};

#if OLDROUTINENAMES
enum { keyAELeadingEdge = keyAELeftSide };
...
enum {
  ...
  kFAServerApp =
      FOUR_CHAR_CODE('ssrv'), /** Creator code for Folder Actions Server*/
  kDoFolderActionEvent = FOUR_CHAR_CODE(
      'fola'), /** Event the Finder sends to the Folder Actions FBA*/
  kFolderActionCode =
      FOUR_CHAR_CODE('actn'), /** Parameter that contains the Folder Action*/
  kFolderOpenedEvent =
      FOUR_CHAR_CODE('fopn'), /** Value of kFolderActionCode parameter; sent to
                                 script as event*/
  ...
  kItemList =
      FOUR_CHAR_CODE('flst'), /** List parameter for added and removed items*/
  kNewSizeParameter = FOUR_CHAR_CODE('fnsz'), /** Parameter for moved window*/
  kFASuiteCode =
      FOUR_CHAR_CODE('faco'), /** Suite code for the following events*/
  ...
  kFAIndexParam = FOUR_CHAR_CODE(
      'indx') /** Key for index (0-based) parameter for Remove and Edit*/
};
...
enum {
  ...
  pProtocol = FOUR_CHAR_CODE(
      'pprt'), /** cde 4/27/98 was 'ppro' conflicted with DB suite */
  ...
  pSCSILUN =
      FOUR_CHAR_CODE('pslu'), /** cde 5/22/98 per WWDC developer request */
  ...
  eAudioLineOut = FOUR_CHAR_CODE(
      'ecal'), /** cde 4/24/98 changed from 'ecao' to not conflict */
