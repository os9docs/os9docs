/**
    ...
    \brief   Definitions likely to be used by low-level protocol stack
   implementation.
...
/** STREAMS environments are expected to define these constants in a public
 * header file.*/
...
typedef CALLBACK_API_C(OTInt32, openp_t)(queue *q, dev_t *dev, OTInt32 foo,
                                         OTInt32 bar, cred_t *cred);
typedef CALLBACK_API_C(OTInt32, openOld_t)(queue *q, dev_t dev, OTInt32 foo,
                                           OTInt32 bar);
typedef CALLBACK_API_C(OTInt32, old_openp_t)(queue *q, dev_t dev, OTInt32 foo,
                                             OTInt32 bar);
...
enum {
  ...
  QHLIST = 0x0200,  /** The Stream head is doing something with this queue (Not
                       supported by MPS) */
...
enum {
  ...
  QB_WANTW =
      0x02, /** The stream should be back enabled when this band/queue drains */
...
enum {
  ...
  RECOPY = 0x02 /** perform I_STR copyin again this time using canonical format
                   specifier */
};
...
struct copyresp {
  ...
  caddr_t
      cp_rval; /** status of request; 0 for success; error value for failure */
  UInt32 cp_pad1;
  SInt32 cp_pad2;
...
enum {
  I_NREAD =
      ((MIOC_STREAMIO << 8) | 1), /** return the number of bytes in 1st msg */
  I_PUSH = ((MIOC_STREAMIO << 8) | 2), /** push module just below stream head */
  I_POP = ((MIOC_STREAMIO << 8) | 3),  /** pop module below stream head */
  I_LOOK =
      ((MIOC_STREAMIO << 8) | 4), /** retrieve name of first stream module */
  I_FLUSH =
      ((MIOC_STREAMIO << 8) | 5), /** flush all input and/or output queues */
  ...
  I_FDINSERT =
      ((MIOC_STREAMIO << 8) | 16), /** create a message and send downstream */
  I_SENDFD =
      ((MIOC_STREAMIO << 8) | 17), /** send an fd to a connected pipe stream */
  I_RECVFD = ((MIOC_STREAMIO << 8) | 18), /** retrieve a file descriptor */
  I_FLUSHBAND = ((MIOC_STREAMIO << 8) |
                 19), /** flush a particular input and/or output band */
  I_SWROPT = ((MIOC_STREAMIO << 8) | 20), /** set the write mode */
  I_GWROPT = ((MIOC_STREAMIO << 8) | 21), /** get the current write mode */
  I_LIST =
      ((MIOC_STREAMIO << 8) | 22), /** get a list of all modules on a stream  */
  I_ATMARK = ((MIOC_STREAMIO << 8) |
              23), /** check to see if the next message is "marked" */
  I_CKBAND = ((MIOC_STREAMIO << 8) |
              24), /** check for a message of a particular band */
  I_GETBAND = ((MIOC_STREAMIO << 8) |
               25), /** get the band of the next message to be read */
  I_CANPUT = ((MIOC_STREAMIO << 8) |
              26), /** check to see if a message may be passed on a stream */
  I_SETCLTIME = ((MIOC_STREAMIO << 8) | 27), /** set the close timeout wait */
  I_GETCLTIME =
      ((MIOC_STREAMIO << 8) | 28), /** get the current close timeout wait */
  I_PLINK = ((MIOC_STREAMIO << 8) |
             29), /** permanently connect a stream under a mux */
  ...
  I_RUN_QUEUES =
      ((MIOC_STREAMIO << 8) | 45),         /** sacrifice for the greater good */
  I_GETPMSG = ((MIOC_STREAMIO << 8) | 46), /** getpmsg() system call */
  I_PUTPMSG = ((MIOC_STREAMIO << 8) | 47), /** putpmsg() system call */
  I_AUTOPUSH = ((MIOC_STREAMIO << 8) |
                48), /** for systems that cannot do the autopush in open */
...
enum {
  RPROTNORM =
      0x10,        /** Normal handling of M_PROTO/M_PCPROTO messages, default */
...
enum {
  SNDZERO =
      0x01 /** Send a zero-length message downstream on a write of zero bytes */
};
...
enum {
  ...
  DL_UNBIND_PENDING =
      0x02,       /** Waiting ack of DL_UNBIND_REQ, LLI compatibility */
  ...
  DL_USER_RESET_PENDING =
      0x0C, /** user initiated reset, awaiting DL_RESET_CON */
  DL_PROV_RESET_PENDING =
      0x0D, /** provider initiated reset, awaiting DL_RESET_RES */
  DL_RESET_RES_PENDING = 0x0E, /** Waiting ack of DL_RESET_RES */
  DL_DISCON8_PENDING =
      0x0F, /** Waiting ack of DL_DISC_REQ when in DL_OUTCON_PENDING */
  DL_DISCON9_PENDING =
      0x10, /** Waiting ack of DL_DISC_REQ when in DL_INCON_PENDING */
  DL_DISCON11_PENDING =
      0x11, /** Waiting ack of DL_DISC_REQ when in DL_DATAXFER */
  DL_DISCON12_PENDING =
      0x12, /** Waiting ack of DL_DISC_REQ when in DL_USER_RESET_PENDING */
  DL_DISCON13_PENDING =
      0x13, /** Waiting ack of DL_DISC_REQ when in DL_DL_PROV_RESET_PENDING */
...
enum {
  ...
  DL_OUTSTATE =
      0x03,         /** Primitive issued in improper state, LLI compatibility */
  ...
  DL_NOTSUPPORTED =
      0x12,          /** Primitive is known but not supported by DLS provider */
  DL_TOOMANY = 0x13, /** limit exceeded */
  DL_NOTENAB = 0x14, /** Promiscuous mode not enabled */
  DL_BUSY =
      0x15, /** Other streams for a particular PPA in the post-attached state */
  DL_NOAUTO =
      0x16, /** Automatic handling of XID & TEST responses not supported */
...
enum {
  ...
  DL_MAXIMUM = 0x0B03  /** protection against modification, replay, addition, or
                          deletion */
};
...
struct dl_bind_req_t {
  ...
  UInt32 dl_xidtest_flg;  /** if set to 1 indicates automatic initiation of test
                             and xid frames */
};
...
struct PollRef {
  SInt32 filler; /** holds a file descriptor an a UNIX system, replaced by ref
                    (at end of structure) under OT*/
  SInt16 events;
  SInt16 revents;
  StreamRef ref;
};
...
#define POLLNVAL                                                               \
  0x020 /** This fd is bogus                                    */
#define POLLRDNORM                                                             \
  0x040 /** A non-priority message is available                 */
#define POLLRDBAND                                                             \
  0x080 /** A priority message (band > 0) message is available  */
#define POLLWRNORM                                                             \
  0x100 /** Same as POLLOUT                                     */
#define POLLWRBAND                                                             \
  0x200               /** A priority band exists and is writable              */
#define POLLMSG 0x400 /** A signal message has reached the front of the queue  \
                       */

/** OTReadInfo structure is used with the various functions that read and peek
 * at the stream head.*/
...
OTStreamOpen(const char *name, OTOpenFlags oFlags, OSStatus *errPtr);
 ...
 */
EXTERN_API(OSStatus)
OTAsyncStreamOpen(const char *name, OTOpenFlags oFlags, OTNotifyUPP proc,
                  void *contextPtr);
 ...
 */
EXTERN_API(StreamRef)
OTCreateStream(OTConfigurationRef cfig, OTOpenFlags oFlags, OSStatus *errPtr);
 ...
 */
EXTERN_API(OSStatus)
OTAsyncCreateStream(OTConfigurationRef cfig, OTOpenFlags oFlags,
 ...
 */
EXTERN_API(OTResult)
OTStreamPoll(PollRef *fds, UInt32 nfds, OTTimeout timeout);
 ...
 */
EXTERN_API(OTResult)
OTAsyncStreamPoll(PollRef *fds, UInt32 nfds, OTTimeout timeout,
                  OTNotifyUPP proc, void *contextPtr);
 ...
 */
EXTERN_API(OTResult)
OTStreamRead(StreamRef strm, void *buf, OTByteCount len);
 ...
 */
EXTERN_API(OTResult)
OTStreamWrite(StreamRef strm, void *buf, OTByteCount len);
 ...
 */
EXTERN_API(OTResult)
OTStreamIoctl(StreamRef strm, UInt32 cmd, void *data);
 ...
 */
EXTERN_API(OSStatus)
OTStreamInstallNotifier(StreamRef strm, OTNotifyUPP proc, void *contextPtr);
 ...
 */
EXTERN_API(OSStatus)
OTStreamUseSyncIdleEvents(StreamRef strm, Boolean useEvents);
 ...
 */
EXTERN_API(OTResult)
OTStreamGetMessage(StreamRef strm, strbuf *ctlbuf, strbuf *databuf,
                   OTFlags *flags);
 ...
 */
EXTERN_API(OTResult)
OTStreamGetPriorityMessage(StreamRef strm, strbuf *ctlbuf, strbuf *databuf,
 ...
 */
EXTERN_API(OSStatus)
OTStreamPutMessage(StreamRef strm, const strbuf *ctlbuf, const strbuf *databuf,
                   OTFlags flags);
 ...
 */
EXTERN_API(OSStatus)
OTStreamPutPriorityMessage(StreamRef strm, const strbuf *ctlbuf,
 ...
 */
EXTERN_API(void)
OTStreamSetControlMask(StreamRef strm, UInt32 mask, Boolean setClear);

/**
   Opening endpoints and mappers on a Stream - these calls are synchronous, and
   may only be used at System Task time. Once the stream has been installed into
   a provider or endpoint, you should not continue to use STREAMS APIs on it
*/
...
OTOpenProviderOnStream(StreamRef strm, OSStatus *errPtr);
...
OTOpenEndpointOnStream(StreamRef strm, OSStatus *errPtr);
 ...
 */
EXTERN_API(OSStatus)
OTPeekMessage(StreamRef strm, OTReadInfo *readInfo);
 ...
 */
EXTERN_API(OTBuffer *)
OTReadMessage(StreamRef strm, OTReadInfo *readInfo);
 ...
 */
EXTERN_API(void)
OTPutBackBuffer(StreamRef strm, OTBuffer *buffer);
 ...
 */
EXTERN_API(void)
OTPutBackPartialBuffer(StreamRef strm, OTBufferInfo *readInfo,
                       OTBuffer *buffer);
...
/**
   Returns a buffer containing all of the clients that refused to yield the
   port. "size" is the total number of bytes @ buffer, including the fNumClients
   field.
*/
#if CALL_NOT_IN_CARBON
/**
...
/**
   Under Carbon, OTCreateTimerTask takes a client context pointer.  Applications
 ...
 */
EXTERN_API(long)
OTCreateTimerTaskInContext(OTProcessUPP upp, void *arg,
...
OTCreateTimerTask(OTProcessUPP proc, void *arg);
 ...
 */
EXTERN_API(Boolean)
OTScheduleTimerTask(OTTimerTask timerTask, OTTimeout milliSeconds);
...
#endif
/**
   Typedef for the OTCanConfigure function, and the enum for which pass we're
   doing. The first (kOTSpecificConfigPass) is to give configurators a shot at
   the configuration before we start allowing the generic configurators to get
   into the act.
*/
...
typedef CALLBACK_API_C(OSStatus,
                       OTCreateConfiguratorProcPtr)(TOTConfiguratorRef *cfigor);
/**
   Typedef for the "OTSetupConfigurator" function that your configurator library
   must export. The enum is for the type of configurator that it is.
*/
...
typedef CALLBACK_API_C(OSStatus, OTSetupConfiguratorProcPtr)(
    OTCanConfigureProcPtr *canConfigure,
    OTCreateConfiguratorProcPtr *createConfigurator, UInt8 *configuratorType);
/**
...
*/
typedef CALLBACK_API_C(OSStatus,
                       OTCFConfigureProcPtr)(TOTConfiguratorRef cfigor,
                                             OTConfigurationRef cfig);
typedef CALLBACK_API_C(OSStatus, OTCFCreateStreamProcPtr)(
    TOTConfiguratorRef cfigor, OTConfigurationRef cfig, OTOpenFlags oFlags,
    OTNotifyUPP proc, void *contextPtr);
typedef CALLBACK_API_C(void, OTCFHandleSystemEventProcPtr)(
    TOTConfiguratorRef cfigor, OTEventCode code, OTResult result, void *cookie);
/**
...
/**
   ...
   OTSMCreateControlStream(OTStateMachine*, OTConfigurationRef,
...
struct OTStateMachine {
  ...
  Boolean GetMessage(StreamRef ref, strbuf *ctl, strbuf *data,
                     OTFlags *flagPtr);
...
/**
   ...
   be used (unless you pass in NULL).  The callDepth is the depth level of
   nested calls using OTSMCallStateProc.
*/
#if CALL_NOT_IN_CARBON
/**
...
inline Boolean OTStateMachine::CallStateProc(OTStateProcPtr proc,
                                             UInt32 state) {
...
inline Boolean OTStateMachine::CreateStream(OTConfigurationRef cfig,
                                            OTOpenFlags flags) {
...
inline Boolean OTStateMachine::SendIoctl(StreamRef ref, UInt32 type,
                                         void *data) {
...
inline Boolean OTStateMachine::PutMessage(StreamRef ref, struct strbuf *ctl,
                                          struct strbuf *data, OTFlags flags) {
  return OTSMPutMessage(this, ref, ctl, data, flags);
}
inline Boolean OTStateMachine::GetMessage(StreamRef ref, struct strbuf *ctl,
                                          struct strbuf *data,
                                          OTFlags *flagPtr) {
...
enum {
  ...
  I_SAD_VML = ((MIOC_SAD << 8) |
               3) /** Validate a list of modules (uses str_list structure) */
};
...
/**
   ...
   includePort is true, a port distinguishing suffix (eg " port X") should be
   added for multiport cards.
*/
...
/**
   This entry point returns the location of the icon for the port.  Return false
...
typedef CALLBACK_API_C(Boolean,
                       OTGetPortIconProcPtr)(OTPortRecord *port,
...
struct OTHashList {
  ...
  OTLink *Remove(OTHashSearchProcPtr proc, const void *refPtr,
                 UInt32 hashValue);
...
inline OTLink *OTHashList::Remove(OTHashSearchProcPtr proc, const void *refPtr,
                                  UInt32 hashValue) {
...
inline OTLink *OTHashList::FindLink(OTHashSearchProcPtr proc,
...
/**
   A list of CFMLibraryInfo structures is returned by the OTFindCFMLibraries
   routine. The list is created out of the data that is passed to the function.
...
public:
  OTConfigurationRef Clone() { return OTCfigCloneConfiguration(this); }
  //
  ...
  OSStatus PopChild(OTItemCount index) { return OTCfigPopChild(this, index); }

  OTConfigurationRef GetChild(OTItemCount index = 0) {
    return OTCfigGetChild(this, index);
  }
  OTConfigurationRef GetParent() { return OTCfigGetParent(this); }
  ...
  OSStatus SetPath(const char *path) { return OTCfigSetPath(this, path); }

  Boolean HasOptions() { return OTCfigGetOptionNetbuf(this)->len != 0; }
};

/**  -------------------------------------------------------------------------
    Class TOTConfigurator

    This class is subclassed to do configuration for a protocol or protocol
   stack. Of course, you can also use OTNewConfigurator to do it from C.
...
public:
  ...
  void operator delete(void *mem) { OTFreeSharedClientMem(mem); };
