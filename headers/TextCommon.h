...
/* (values 0-32 correspond to the Script Codes defined in Inside Macintosh: Text
 * pages 6-52 and 6-53 */
...
enum {
  ...
  kTextEncodingMacUkrainian = 0x98, /* Meta-value in TEC 1.5 & later; maps to
                                       kTextEncodingMacCyrillic variant    */
  /* The following use script code 28, smEthiopic*/
  kTextEncodingMacInuit =
      0xEC, /* The following use script code 32, smUnimplemented*/
  kTextEncodingMacVT100 = 0xFC /* VT100/102 font from Comm Toolbox: Latin-1
...
enum {
  ...
  kTextEncodingUnicodeV2_1 =
      0x0103, /* We treat both Unicode 2.0 and Unicode 2.1 as 2.1*/
  kTextEncodingUnicodeV3_0 = 0x0104,
  kTextEncodingUnicodeV3_1 =
      0x0105, /* Adds characters requiring surrogate pairs in UTF-16*/
...
enum {
  ...
  kTextEncodingISOLatin9 =
      0x020F /* ISO 8859-15, 8859-1 changed for EURO & CP1252 letters  */
};
...
enum {
  ...
  kTextEncodingDOSJapanese =
      0x0420, /* code page 932, also for Windows; Shift-JIS with additions*/
  kTextEncodingDOSChineseSimplif =
      0x0421, /* code page 936, also for Windows; was EUC-CN, now GBK (EUC-CN
                 extended)*/
  kTextEncodingDOSKorean = 0x0422, /* code page 949, also for Windows; Unified
                                      Hangul Code (EUC-KR extended)*/
  kTextEncodingDOSChineseTrad =
      0x0423, /* code page 950, also for Windows; Big-5*/
...
enum {
  ...
  kTextEncodingJIS_X0201_76 =
      0x0620, /* JIS Roman and 1-byte katakana (halfwidth)*/
  ...
  kTextEncodingGBK_95 =
      0x0631, /* annex to GB 13000-93; for Windows 95; EUC-CN extended*/
...
enum {
  kMacRomanDefaultVariant =
      0, /* meta value, maps to 1 or 2 depending on System */
  kMacRomanCurrencySignVariant =
      1,                       /* Mac OS version < 8.5, 0xDB is CURRENCY SIGN*/
...
enum {
  kMacCyrillicDefaultVariant =
      0, /* meta value, maps to 1, 2, or 3 depending on System*/
  ...
  kMacCyrillicEuroSignVariant =
      3 /* Mac OS >= 9.0, 0xFF is EURO SIGN, 0xA2/0xB6 = GHE WITH UPTURN*/
};

/* Variants of kTextEncodingMacIcelandic */
enum {
  kMacIcelandicStdDefaultVariant =
      0, /* meta value, maps to 2 or 4 depending on System */
  kMacIcelandicTTDefaultVariant =
      1, /* meta value, maps to 3 or 5 depending on System */
         /* The following are for Mac OS version < 8.5, 0xDB is CURRENCY SIGN        */
  kMacIcelandicStdCurrSignVariant =
      2, /* 0xBB/0xBC are fem./masc. ordinal indicators*/
  ...
  kMacIcelandicStdEuroSignVariant =
      4, /* 0xBB/0xBC are fem./masc. ordinal indicators*/
...
enum {
  kMacArabicStandardVariant =
      0, /* 0xC0 is 8-spoke asterisk, 0x2A & 0xAA are asterisk (e.g. Cairo)*/
  kMacArabicTrueTypeVariant =
      1, /* 0xC0 is asterisk, 0x2A & 0xAA are multiply signs (e.g. Baghdad)*/
  kMacArabicThuluthVariant =
      2, /* 0xC0 is Arabic five-point star, 0x2A & 0xAA are multiply signs*/
  kMacArabicAlBayanVariant =
      3 /* 8-spoke asterisk, multiply sign, Koranic ligatures & parens*/
};

/* Variants of kTextEncodingMacFarsi*/
enum {
  kMacFarsiStandardVariant =
      0, /* 0xC0 is 8-spoke asterisk, 0x2A & 0xAA are asterisk (e.g. Tehran)*/
  kMacFarsiTrueTypeVariant =
      1 /* asterisk, multiply signs, Koranic ligatures, geometric shapes*/
};
...
enum {
  ...
  kBig5_StandardVariant =
      1, /* 0xC6A1-0xC7FC: kana, Cyrillic, enclosed numerics*/
  kBig5_ETenVariant =
...
enum {
  kMacRomanLatin1DefaultVariant =
      0, /* meta value, maps to others depending on System*/
  ...
  kMacRomanLatin1IcelandicVariant =
      11, /* permuted MacIcelandic, StdEuroSignVariant*/
...
enum {
  ...
  kUnicodeMaxDecomposedVariant =
      2, /* replaced by kUnicodeCanonicalDecompVariant*/
  /* The following Japanese variant options were never supported and are now
     deprecated.*/
  /* In TEC 1.4 and later their functionality is replaced by the Unicode
     Converter options listed.*/
  kJapaneseNoOneByteKanaOption =
      0x20, /* replaced by UnicodeConverter option kUnicodeNoHalfwidthCharsBit*/
...
struct TECInfo {
  ...
  UInt32 tecUnicodeConverterFeatures; /* bitmask indicating UnicodeConverter
                                         features/fixes*/
  UInt32
      tecTextCommonFeatures; /* bitmask indicating TextCommon features/fixes*/
  Str31 tecTextEncodingsFolderName; /* localized name of Text Encodings folder
                                       (pascal string)*/
  Str31
      tecExtensionFileName; /* localized name of TEC extension (pascal string)*/
  UInt16 tecLowestTEFileVersion;  /* Lowest version (BCD) of all files in Text
                                     Encodings folder*/
...
enum {
  kTECInfoCurrentFormat =
      2 /* any future formats will just add fields at the end*/
};
...
/**
   ...
   Bit:                             Meaning if set:
   ----                             ---------------
   kTECKeepInfoFixBit               Unicode Converter no longer ignores other
   control flags if kUnicodeKeepInfoBit is set. Bug fix in TEC Manager 1.2.1.
   ...
   *destConvLen returned by caller-supplied fallback handler for any status it
                                    returns except for kTECUnmappableElementErr
   ...
   encoding was incorrect). Bug fix in TEC Manager 1.3.
   ...
   mapping. This has several effects:
   ...
   the control flags kUnicodeKeepInfoMask and kUnicodeStringUnterminatedMask.
                                    Bug fix and enhancement in TEC Manager 1.3.
   kTECAddForceASCIIChangesBit      Define new control flag bits
   kUnicodeForceASCIIRangeBit and kUnicodeNoHalfwidthCharsBit for use with
   ...
   functions fix a problem that occurred when a preferred encoding was specified
   that did not match the System script; the preferred script was not actually
   placed first in the ordered list of encodings to use. Bug fix in TEC
   Manager 1.4. kTECAddTextRunHeuristicsBit      Define new control flag bit
   kUnicodeTextRunHeuristicsBit for use with ConvertFromUnicodeToTextRun.
   kTECAddFallbackInterruptBit      Define new option
   kUnicodeFallbackInterruptSafeMask for use with SetFallbackUnicodeToText. If a
   ...
   handler moves memory); otherwise, it won't.
*/
...
enum {
  ...
  kUnicodeReplacementChar =
      0xFFFD, /* Unicode replacement for unconvertable input char*/
  kUnicodeSwappedByteOrderMark =
      0xFFFE, /* not a Unicode char; byte-swapped version of FEFF*/
