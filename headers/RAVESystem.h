/**
    ...
    \brief   Interfaces needed when building RAVE engines
...
#pragma options(!pack_enums)
     ...
     * format */
    /** long                 transparentIndex    boolean, false means no
    ...
    /**  TQATexture          **newTexture        (Out) Newly created TQATexture, or
     * NULL on error */
    ...
    typedef CALLBACK_API_C(TQAError,
                           TQATextureBindColorTable)(TQATexture *texture,
    ...
    /**  TQABitmap           **newBitmap         (Out) Newly created TQABitmap, or
     * NULL on error */
     ...
     * QAColorTableNew() */
    typedef CALLBACK_API_C(TQAError,
                           TQABitmapBindColorTable)(TQABitmap *bitmap,
                                                    TQAColorTable *colorTable);
    /*************************************************************************************************
     *
     * Typedefs of private (system-only) functions provided by the drawing engine.
     *
     * The TQADrawPrivateNew function returns a TQADrawPrivate *, which points to
     *the engine-specific private data created for the context. (TQADrawPrivate is a
     *dummy type which is then cast to the correct engine-specific datatype by the
     *engine code.)
     *
     * The TQADrawPrivateDelete function deletes the engine-specific private data.
     *
     * TQAStorePrivateNew and TQAStorePrivateDelete provide the same function as
     *QADrawPrivateNew and TQADrawPrivateDelete, but for the texture and bitmap
     *storage context.
     *
     * TQADrawMethodGet and TQAStoreMethodGet are called by the RAVE manager to
     *retrieve the method pointers for a drawing engine.
     *
     * The TQAEngineCheckDevice function returns TRUE if the engine can render to
     *the indicated GDevice.
     *
     ***********************************************************************************************/
    ...
    /**  const TQARect       *rect               Target rectangle (device
     * coordinates) */
    /**  const TQAClip       *clip               2D clip region (or NULL) */
    /**  unsigned long       flags               Mask of kQAContext_xxx */
    typedef CALLBACK_API_C(TQAError, TQADrawPrivateNew)(
        TQADrawContext *newDrawContext, const TQADevice *device,
        const TQARect *rect, const TQAClip *clip, unsigned long flags);
    ...
    typedef CALLBACK_API_C(TQAError, TQAAccessBitmap)(TQABitmap *bitmap, long flags,
                                                      TQAPixelBuffer *buffer);
     ...
     * parameter to QAEngineGetMethod). TQAEngineMethodTag identifies which method
     *is being requested.
     *
     ***********************************************************************************************/
    union TQAEngineMethod
    {
        ...
        TQADrawPrivateDelete
            drawPrivateDelete; /** Method: Delete a private draw context */
        TQAEngineCheckDevice
            engineCheckDevice;          /** Method: Check a device for drawing */
        TQAEngineGestalt engineGestalt; /** Method: Gestalt */
        TQATextureNew
            textureNew; /** Method: Create a texture (load is non-blocking) */
        TQATextureDetach
            textureDetach;              /** Method: Complete load of a texture (blocking) */
        TQATextureDelete textureDelete; /** Method: Delete a texture */
        TQABitmapNew bitmapNew;         /** Method: Create a bitmap (load is non-blocking)  */
        TQABitmapDetach
            bitmapDetach;                     /** Method: Complete load of a bitmap (blocking) */
        ...
        TQATextureBindColorTable
            textureBindColorTable; /** Method: Bind a CLUT to a texture */
        TQABitmapBindColorTable
            bitmapBindColorTable; /** Method: Bind a CLUT to a bitmap */
    ...
    /*************************************************************************************************
     *
     * QARegisterEngine() registers a new engine. This is called at boot time by the
     *drawing engine initialization code to register itself with the system. This
     *call takes only one parameter, the engine's function that allows the manager
     *to request the other methods.
     *
     ***********************************************************************************************/
    ...
    typedef CALLBACK_API_C(TQAError,
                           TQAEngineGetMethod)(TQAEngineMethodTag methodTag,
...
/*************************************************************************************************
 *
 * The TQADrawMethod union is used to represent a single draw context method
 *(it's a parameter to QARegisterDrawMethod). TQADrawMethodTag identifies which
 ...
 ***********************************************************************************************/
    ...
    union TQADrawMethod
    {
        ...
        TQADrawTriGouraud
            drawTriGouraud; /** Method: Draw a Gouraud shaded triangle */
        TQADrawTriTexture
            drawTriTexture;           /** Method: Draw a texture mapped triangle */
...
/*************************************************************************************************
 *
 * System call to register a new method for an engine. This is called during the
 *engine's draw private new functions (to set the initial value of the draw
 *methods), and possibly at other times when the engine needs to change a draw
 *method.
 *
 ***********************************************************************************************/
/** QARegisterDrawMethod parameter descriptions */
/**  TQADrawContext          *drawContext            Draw context in which to
 * set method */
