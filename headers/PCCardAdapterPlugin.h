/**
    ...
    \brief   PC Card Socket Service Plug-In Programming Interface
...
#pragma pack(2)
  ...
  enum
  {
    kSCBatteryDead = 0x01, /** Dead */
    Status Change kSCBatteryLow = 0x02
    /** Warning */
    Status Change kSCReady = 0x04
    /** Status */
    Change kSCCardDetect = 0x08
    /** Detect */
    Status Change kSCCardEjected = 0x10
    /** Ejected */
    kSCStatusChange = 0x20
    // PC Card Status Change Signal Asserted  kSCRingIndicate = 0x40
    // PC Card Ring Indicate Signal Asserted
  };
  // Battery Dead Status Change
  //  IRQ bits
  enum
  {                       // Battery Warning Status Change
...
// battery status, busy status, and XIP
 /** shared */
for all sockets  SS_ADPT_FLG_PWR  =  0x02
// if set indicates that the sockets
 /** the */
same power control  SS_ADPT_FLG_DBW  =  0x04
// all windows on the adapter must use
 /** same */
Data Bus Width  SS_ADPT_FLG_CARDBUS  =  0x08
 /** sockets */
are CardBus PC Card capable  SS_ADPT_FLG_DMA  =  0x10
// the adapter has DMA capability
 
 bits for adapter power characteristics  SS_ADPT_FLG_V33  =  0x20
 /** supports */
3.3 volt power to socket  SS_ADPT_FLG_V50  =  0x40
// adapter supports 5.0 volt power to socket  SS_ADPT_FLG_V12 = 0x80
// adapter supports 12.0 volt power to socket
};
...
typedef CALLBACK_API_C(OSStatus, SSInquireAdapterProc)(
    ItemCount *numberOfSockets, ItemCount *numberOfWindows,
    ItemCount *numberOfBridgeWindow, PCCardAdapterCapabilities *capabilities);
typedef CALLBACK_API_C(OSStatus, SSInquireSocketProc)(
    PCCardSocket socket, ItemCount *numberOfWindows,
    PCCardSocketStatus *supportedSocketStatus,
    PCCardSCEvents *supportedStatusChange);
typedef CALLBACK_API_C(OSStatus, SSGetSocketProc)(
    PCCardSocket socket, PCCardVoltage *Vcc, PCCardVoltage *Vpp,
    PCCardVoltage *Vs, PCCardInterfaceType *socketIF,
    PCCardCustomInterfaceID *customIFID, PCCardSocketStatus *socketStatus,
...
typedef CALLBACK_API_C(OSStatus,
                       SSGetStatusProc)(PCCardSocket socket,
                                        PCCardSocketStatus *currentStatus,
                                        PCCardSocketStatus *changedStatus);
typedef CALLBACK_API_C(OSStatus, SSInquireWindowProc)(
    PCCardSocket *socket, PCCardWindow window, PCCardWindowState *windowState,
    PCCardWindowSize *windowSize, PCCardWindowAlign *windowAlign);
typedef CALLBACK_API_C(OSStatus, SSGetWindowProc)(
    PCCardSocket *socket, PCCardWindow window, PCCardWindowState *windowState,
    PhysicalAddress *startAddress, PCCardWindowSize *windowSize,
    PCCardWindowOffset *windowOffset, PCCardAccessSpeed *memSpeed);
typedef CALLBACK_API_C(OSStatus, SSSetWindowProc)(
    PCCardSocket socket, PCCardWindow window, PCCardWindowState windowState,
    PhysicalAddress startAddress, PCCardWindowSize windowSize,
    PCCardWindowOffset windowOffset, PCCardAccessSpeed memSpeed);
typedef CALLBACK_API_C(OSStatus,
                       SSGetWindowOffsetProc)(PCCardSocket *socket,
                                              PCCardWindow window,
                                              PCCardWindowState *windowState,
                                              PCCardWindowOffset *windowOffset);
typedef CALLBACK_API_C(OSStatus,
                       SSSetWindowOffsetProc)(PCCardSocket socket,
                                              PCCardWindow window,
                                              PCCardWindowState windowState,
                                              PCCardWindowOffset windowOffset);
typedef CALLBACK_API_C(OSStatus, SSInquireBridgeWindowProc)(
    PCCardSocket *socket, PCCardWindow window, PCCardWindowState *windowState,
    PCCardWindowSize *windowSize, PCCardWindowAlign *windowAlign);
typedef CALLBACK_API_C(OSStatus, SSGetBridgeWindowProc)(
    PCCardSocket *socket, PCCardWindow window, PCCardWindowState *windowState,
    PhysicalAddress *startAddress, PCCardWindowSize *windowSize);
typedef CALLBACK_API_C(OSStatus, SSSetBridgeWindowProc)(
    PCCardSocket socket, PCCardWindow window, PCCardWindowState windowState,
    PhysicalAddress startAddress, PCCardWindowSize windowSize);
typedef CALLBACK_API_C(OSStatus, SSGetInterruptSetMemberProc)(
    PCCardSocket socket, InterruptSetMember *interruptSetMember);
...
typedef struct PCCardAdapterPluginDispatchTable
    PCCardAdapterPluginDispatchTable;
