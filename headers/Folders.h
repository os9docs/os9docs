/**
    ...
    \brief   Folder Manager Interfaces.
...
#pragma pack(2)
    ...
    enum
    {
        kOnSystemDisk = -32768L, /** previously was 0x8000 but that is an unsigned
                                    value whereas vRefNum is signed*/
        kOnAppropriateDisk =
            -32767, /** Generally, the same as kOnSystemDisk, but it's clearer that
                       this isn't always the 'boot' disk.*/
        /** Folder Domains - Carbon only.  The constants above can continue to be
           used, but the folder/volume returned will*/
        /** be from one of the domains below.*/
        kSystemDomain = -32766,  /** Read-only system hierarchy.*/
        kLocalDomain = -32765,   /** All users of a single machine have access to these
                                    resources.*/
        kNetworkDomain = -32764, /** All users configured to use a common network
                                    server has access to these resources.*/
        kUserDomain =
            -32763,             /** Read/write. Resources that are private to the user.*/
        kClassicDomain = -32762 /** Domain referring to the currently configured
    ...
    enum
    {
        ...
        kDesktopFolderType =
            FOUR_CHAR_CODE('desk'), /** the desktop folder; objects in this folder
                                       show on the desk top. */
        kSystemDesktopFolderType = FOUR_CHAR_CODE(
            'sdsk'), /** the desktop folder at the root of the hard drive, never the
                        redirected user desktop folder */
        kTrashFolderType =
            FOUR_CHAR_CODE('trsh'), /** the trash folder; objects in this folder show
                                       up in the trash */
        kSystemTrashFolderType =
            FOUR_CHAR_CODE('strs'), /** the trash folder at the root of the drive,
                                       never the redirected user trash folder */
        kWhereToEmptyTrashFolderType =
            FOUR_CHAR_CODE('empt'), /** the "empty trash" folder; Finder starts empty
                                       from here down */
        kPrintMonitorDocsFolderType =
            FOUR_CHAR_CODE('prnt'), /** Print Monitor documents */
        kStartupFolderType =
            FOUR_CHAR_CODE('strt'), /** Finder objects (applications, documents, DAs,
                                       aliases, to...) to open at startup go here */
        kShutdownFolderType =
            FOUR_CHAR_CODE('shdf'), /** Finder objects (applications, documents, DAs,
                                       aliases, to...) to open at shutdown go here */
        kAppleMenuFolderType = FOUR_CHAR_CODE(
        ...
        kSystemControlPanelFolderType = FOUR_CHAR_CODE(
            'sctl'), /** System control panels folder - never the redirected one,
                        always "Control Panels" inside the System Folder */
        kExtensionFolderType =
            FOUR_CHAR_CODE('extn'),                /** System extensions go here */
        kFontsFolderType = FOUR_CHAR_CODE('font'), /** Fonts go here */
        kPreferencesFolderType =
            FOUR_CHAR_CODE('pref'), /** preferences for applications go here */
        kSystemPreferencesFolderType = FOUR_CHAR_CODE(
            'sprf'), /** System-type Preferences go here - this is always the system's
                        preferences folder, never a logged in user's */
        kTemporaryFolderType =
            FOUR_CHAR_CODE('temp') /** temporary files go here (deleted periodically,
     ...
     *    \non_carbon_cfm   in FoldersLib 1.0 and later
     *    \carbon_lib        in CarbonLib 1.0 and later
     *    \mac_os_x         in version 10.0 and later
     */
    OSErr
 ...
 */
    ...
    enum
    {
        ...
        kALMPreferencesFolderType = FOUR_CHAR_CODE(
            'trip'), /** EAS for Location Manager Preferences (within
        ...
        kLauncherItemsFolderType =
            FOUR_CHAR_CODE('laun'), /** Launcher Items folder */
        kRecentApplicationsFolderType =
            FOUR_CHAR_CODE('rapp'), /** Recent Applications folder */
        kRecentDocumentsFolderType =
            FOUR_CHAR_CODE('rdoc'), /** Recent Documents folder */
        kRecentServersFolderType =
            FOUR_CHAR_CODE('rsvr'), /** Recent Servers folder */
        kSpeakableItemsFolderType =
            FOUR_CHAR_CODE('spki'),                   /** Speakable Items folder */
        kKeychainFolderType = FOUR_CHAR_CODE('kchn'), /** Keychain folder */
        kQuickTimeExtensionsFolderType = FOUR_CHAR_CODE(
            'qtex'), /** QuickTime Extensions Folder (in Extensions folder) */
        kDisplayExtensionsFolderType = FOUR_CHAR_CODE(
            'dspl'), /** Display Extensions Folder (in Extensions folder) */
        kMultiprocessingFolderType = FOUR_CHAR_CODE(
            'mpxf'), /** Multiprocessing Folder (in Extensions folder) */
        kPrintingPlugInsFolderType = FOUR_CHAR_CODE(
            'pplg') /** Printing Plug-Ins Folder (in Extensions folder) */
    };
    ...
    enum
    {
        kDomainTopLevelFolderType = FOUR_CHAR_CODE(
            'dtop'), /** The top-level of a Folder domain, e.g. "/System"*/
        kDomainLibraryFolderType =
            FOUR_CHAR_CODE('dlib'), /** the Library subfolder of a particular domain*/
        kColorSyncFolderType =
            FOUR_CHAR_CODE('sync'),                       /** Contains ColorSync-related folders*/
        kColorSyncCMMFolderType = FOUR_CHAR_CODE('ccmm'), /** ColorSync CMMs*/
        kColorSyncScriptingFolderType =
            FOUR_CHAR_CODE('cscr'), /** ColorSync Scripting support*/
        kPrintersFolderType =
            FOUR_CHAR_CODE('impr'), /** Contains Printing-related folders*/
        kSpeechFolderType =
            FOUR_CHAR_CODE('spch'), /** Contains Speech-related folders*/
        kCarbonLibraryFolderType =
            FOUR_CHAR_CODE('carb'), /** Contains Carbon-specific file*/
        kDocumentationFolderType = FOUR_CHAR_CODE(
            'info'), /** Contains Documentation files (not user documents)*/
        kDeveloperDocsFolderType = FOUR_CHAR_CODE(
            'ddoc'), /** Contains Developer Documentation files and folders*/
        kDeveloperHelpFolderType =
            FOUR_CHAR_CODE('devh'), /** Contains Developer Help related files*/
        kISSDownloadsFolderType = FOUR_CHAR_CODE(
            'issd'), /** Contains Internet Search Sites downloaded from the Internet*/
        kUserSpecificTmpFolderType =
            FOUR_CHAR_CODE('utmp'), /** Contains temporary items created on behalf of
                                       the current user*/
        kCachedDataFolderType = FOUR_CHAR_CODE(
            'cach'), /** Contains various cache files for different clients*/
        kFrameworksFolderType =
            FOUR_CHAR_CODE('fram'), /** Contains MacOS X Framework folders     */
        kPrivateFrameworksFolderType =
            FOUR_CHAR_CODE('pfrm'), /** Contains MacOS X Private Framework folders */
        kClassicDesktopFolderType =
            FOUR_CHAR_CODE('sdsk'), /** MacOS 9 compatible desktop folder - same as */
        /** kSystemDesktopFolderType but with a more appropriate*/
        /** name for Mac OS X code.*/
        kDeveloperFolderType =
            FOUR_CHAR_CODE('devf'), /** Contains MacOS X Developer Resources*/
        kSystemSoundsFolderType =
            FOUR_CHAR_CODE('ssnd'), /** Contains Mac OS X System Sound Files*/
        kComponentsFolderType =
            FOUR_CHAR_CODE('cmpd'), /** Contains Mac OS X components*/
        kQuickTimeComponentsFolderType =
            FOUR_CHAR_CODE('wcmp'), /** Contains QuickTime components for Mac OS X*/
        kCoreServicesFolderType = FOUR_CHAR_CODE(
            'csrv'), /** Refers to the "CoreServices" folder on Mac OS X*/
        kPictureDocumentsFolderType = FOUR_CHAR_CODE(
            'pdoc'), /** Refers to the "Pictures" folder in a users home directory*/
        ...
        kPublicFolderType = FOUR_CHAR_CODE(
            'pubb'), /** Refers to the "Public" folder in a users home directory*/
        kAudioSupportFolderType = FOUR_CHAR_CODE(
            'adio'), /** Refers to the Audio support folder for Mac OS X*/
        kAudioSoundsFolderType = FOUR_CHAR_CODE(
            'asnd'), /** Refers to the Sounds subfolder of Audio Support*/
        kAudioSoundBanksFolderType = FOUR_CHAR_CODE(
            'bank'), /** Refers to the Banks subfolder of the Sounds Folder*/
        kAudioAlertSoundsFolderType = FOUR_CHAR_CODE(
            'alrt'), /** Refers to the Alert Sounds subfolder of the Sound Folder*/
        kAudioPlugInsFolderType = FOUR_CHAR_CODE(
            'aplg'), /** Refers to the Plug-ins subfolder of the Audio Folder   */
        kAudioComponentsFolderType =
            FOUR_CHAR_CODE('acmp'), /** Refers to the Components subfolder of the
                                       Audio Plug-ins Folder    */
        kKernelExtensionsFolderType = FOUR_CHAR_CODE(
            'kext'), /** Refers to the Kernel Extensions Folder on Mac OS X*/
        kDirectoryServicesFolderType = FOUR_CHAR_CODE(
            'dsrv'), /** Refers to the Directory Services folder on Mac OS X*/
        kDirectoryServicesPlugInsFolderType =
            FOUR_CHAR_CODE('dplg'), /** Refers to the Directory Services Plug-Ins
                                       folder on Mac OS X */
        kInstallerReceiptsFolderType =
            FOUR_CHAR_CODE('rcpt'), /** Refers to the "Receipts" folder in Mac OS X*/
        kFileSystemSupportFolderType =
            FOUR_CHAR_CODE('fsys'), /** Refers to the [domain]/Library/Filesystems
                                       folder in Mac OS X*/
        kAppleShareSupportFolderType = FOUR_CHAR_CODE(
            'shar'), /** Refers to the [domain]/Library/Filesystems/AppleShare folder
                        in Mac OS X*/
        kAppleShareAuthenticationFolderType =
            FOUR_CHAR_CODE('auth'), /** Refers to the
                                       [domain]/Library/Filesystems/AppleShare/Authentication
                                       folder in Mac OS X*/
        kMIDIDriversFolderType =
            FOUR_CHAR_CODE('midi') /** Refers to the MIDI Drivers folder on Mac OS X*/
    };
    ...
    enum
    {
        kUsersFolderType = FOUR_CHAR_CODE(
            'usrs'), /** "Users" folder, contains one folder for each user. */
        kCurrentUserFolderType = FOUR_CHAR_CODE(
            'cusr'), /** The folder for the currently logged on user. */
        kCurrentUserRemoteFolderLocation = FOUR_CHAR_CODE(
            'rusf'), /** The remote folder for the currently logged on user */
        kCurrentUserRemoteFolderType =
            FOUR_CHAR_CODE('rusr'), /** The remote folder location for the currently
                                       logged on user */
        kSharedUserDataFolderType =
            FOUR_CHAR_CODE('sdat'), /** A Shared "Documents" folder, readable &
                                       writeable by all users */
        kVolumeSettingsFolderType =
            FOUR_CHAR_CODE('vsfd') /** Volume specific user information goes here */
    };
    ...
    enum
    {
        kCurrentUserFolderLocation =
            FOUR_CHAR_CODE('cusf') /**    the magic 'Current User' folder location*/
    };
    ...
    /** routing constants */
    /**  These are bits in the .flags field of the FindFolderUserRedirectionGlobals
     * struct*/
    enum
    {
        /**    Set this bit to 1 in the .flags field of a
        ...
        kFindFolderRedirectionFlagUseDistinctUserFoldersBit =
            0, /**    Set this bit to 1 and the currentUserFolderVRefNum and
                  currentUserFolderDirID*/
        /**    fields of the user record will get used instead of finding the user
           folder*/
        /**    with the userName field.*/
        kFindFolderRedirectionFlagUseGivenVRefAndDirIDAsUserFolderBit =
            1, /**    Set this bit to 1 and the remoteUserFolderVRefNum and
                  remoteUserFolderDirID*/
        /**    fields of the user record will get used instead of finding the user
           folder*/
        /**    with the userName field.*/
    ...
    typedef struct FindFolderUserRedirectionGlobals
        FindFolderUserRedirectionGlobals;
    ...
    /**
        These are passed into FindFolderExtended(), FindFolderInternalExtended(),
       and FindFolderNewInstallerEntryExtended() in the flags field.
    */
    enum
    {
    ...
    typedef CALLBACK_API(OSStatus,
                         FolderManagerNotificationProcPtr)(OSType message,
                                                           void *arg,
                                                           void *userRefCon);
    typedef STACK_UPP_TYPE(FolderManagerNotificationProcPtr)
    ...
    enum
    {
        uppFolderManagerNotificationProcInfo = 0x00000FF0
    }; /** pascal 4_bytes Func(4_bytes, 4_bytes, 4_bytes) */
#ifdef __cplusplus
    inline FolderManagerNotificationUPP
    NewFolderManagerNotificationUPP(FolderManagerNotificationProcPtr userRoutine)
    {
        return (FolderManagerNotificationUPP)NewRoutineDescriptor(
...
#define DisposeFolderManagerNotificationUPP(userUPP) \
                                       ...
                                       FolderManagerNotificationUPP userUPP)
    {
        return (OSStatus)CALL_THREE_PARAMETER_UPP(
            userUPP, uppFolderManagerNotificationProcInfo, message, arg, userRefCon);
    }
#else
...
#define NewFolderManagerNotificationProc(userRoutine) \
    NewFolderManagerNotificationUPP(userRoutine)
#define CallFolderManagerNotificationProc(userRoutine, message, arg, \
                                          userRefCon)                \
    InvokeFolderManagerNotificationUPP(message, arg, userRefCon, userRoutine)
#endif /** CALL_NOT_IN_CARBON */

    enum
    {
        kFolderManagerNotificationMessageUserLogIn =
            FOUR_CHAR_CODE('log+'), /**    Sent by system & third party software after
                                       a user logs in.  arg should point to a valid
                                       FindFolderUserRedirectionGlobals structure or
                                       nil for the owner*/
        kFolderManagerNotificationMessagePreUserLogIn =
            FOUR_CHAR_CODE('logj'), /**    Sent by system & third party software
                                       before a user logs in.  arg should point to a
                                       valid FindFolderUserRedirectionGlobals
                                       structure or nil for the owner*/
        kFolderManagerNotificationMessageUserLogOut =
            FOUR_CHAR_CODE('log-'), /**    Sent by system & third party software
                                       before a user logs out.  arg should point to a
                                       valid FindFolderUserRedirectionGlobals
                                       structure or nil for the owner*/
        kFolderManagerNotificationMessagePostUserLogOut =
            FOUR_CHAR_CODE('logp'), /**    Sent by system & third party software after
                                       a user logs out.  arg should point to a valid
                                       FindFolderUserRedirectionGlobals structure or
                                       nil for the owner*/
        kFolderManagerNotificationDiscardCachedData = FOUR_CHAR_CODE(
            'dche'), /**    Sent by system & third party software when the entire
                        Folder Manager cache should be flushed*/
        kFolderManagerNotificationMessageLoginStartup =
            FOUR_CHAR_CODE('stup') /**    Sent by 'Login' application the first time
                                      it starts up after each boot*/
    };

    /**  These get used in the options parameter of
    ...
    enum
    {
        ...
        kDoNotRemoveWheCurrentApplicationQuitsBit =
            kDoNotRemoveWhenCurrentApplicationQuitsBit /**    Going away soon, use
                                                          kDoNotRemoveWheCurrentApplicationQuitsBit*/
    };

    /**  These get used in the options parameter of
     * FolderManagerCallNotificationProcs()*/
    enum
    {
    ...
    /**
       ...
       Ease 1.0.  The structure has been expanded (and developers should assume that
       it will continue this way into the future).  Older fields have been obsoleted
       as the features have changed in newer versions of the code.
    */
    ...
    struct MultiUserGestalt
    {
        ...
        short giReserved0;  /** [OBSOLETE with v3] giIsActive: if true then At Ease is
                               currently running*/
        short giReserved1;  /** [OBSOLETE] if true then auto create alias*/
        short giReserved2;  /** [OBSOLETE with v6]  if true then request floppy on new
                               saves*/
        short giReserved3;  /** [OBSOLETE] if true then hypercard stacks are shown on
                               Applications panel*/
        FSSpec giReserved4; /** [OBSOLETE with v6] location of At Ease Items folder*/

        /**    Version 2 fields.*/
        short giDocsVRefNum;  /** vrefnum of user's documents location (only valid if
                                 not on floppy)*/
        long giDocsDirID;     /** directory id of user's documents folder (only valid if
                                 not on floppy)*/
        short giForceSaves;   /** true if user is forced to save to their documents
                                 folder*/
        short giForceOpens;   /** true if user is forced to open from their documents
                                 folder*/
        ...
        short giReserved5;    /** [OBSOLETE with v6] true if user has Go To Finder menu
                                 item*/
        ...
        /**    Version 4 fields.*/
        short giUserLoggedInType;   /** 0 = normal user, 1 = workgroup admin, 2 = global
                                       admin*/
        char giUserEncryptPwd[16];  /** encrypted user password (our digest form)*/
        short giUserEnvironment;    /** 0 = panels, 1 = normal Finder, 2 =
                                       limited/restricted Finder*/
        long giReserved6;           /** [OBSOLETE]*/
        long giReserved7;           /** [OBSOLETE]*/
        Boolean giDisableScrnShots; /** true if screen shots are not allowed*/

        /**    Version 5 fields.*/
        Boolean giSupportsAsyncFSCalls; /** Finder uses this to tell if our patches
                                           support async trap patches*/
        short giPrefsVRefNum;           /** vrefnum of preferences*/
        long giPrefsDirID;              /** dirID of the At Ease Items folder on preferences
                                           volume*/
        unsigned long giUserLogInTime;  /** time in seconds we've been logged in (0 or
                                           1 mean not logged in)*/
        Boolean
            giUsingPrintQuotas;    /** true if logged in user is using printer quotas*/
        ...
        Boolean giInSystemAccess;    /** true if system is in System Access (i.e., owner
                                        logged in)*/
        Boolean giUserFolderEnabled; /** true if FindFolder is redirecting folders
                                        (uses giUserName for user)*/
        short giReserved8;
        long giReserved9;
        Boolean giInLoginScreen; /** true if no user has logged in (including owner)*/

        /** May have more fields added in future, so never check for
         * sizeof(GestaltRec)*/
    };
