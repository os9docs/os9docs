/**
    ...
    \brief   Balloon Help Package Interfaces.
...
#pragma pack(2)
  ...
  /**
     Carbon clients should use MacHelp. The definitions below will NOT work for
  ...
  */
  ...
  enum
  {
    ...
    hmMatchInTitleBit = 4,
    hmUseSubIDMask =
        (1 << hmUseSubIDBit), /* treat resID's in resources as subID's of driver
                                 base ID (for Desk Accessories) */
    hmAbsoluteCoordsMask =
        (1 << hmAbsoluteCoordsBit), /* ignore window port origin and treat
                                       rectangles as absolute coords (local to
                                       window) */
    hmSaveBitsNoWindowMask =
        (1 << hmSaveBitsNoWindowBit), /* don't create a window, just blast bits on
                                         screen. No update event is generated */
    hmSaveBitsWindowMask =
        (1 << hmSaveBitsWindowBit), /* create a window, but restore bits behind
                                       window when window goes away & generate
                                       update event */
    hmMatchInTitleMask =
        (1 << hmMatchInTitleBit) /* for hwin resources, match string anywhere in
                                    window title string */
  };
  ...
  {
    return (TipFunctionUPP)NewRoutineDescriptor(
        (ProcPtr)(userRoutine), uppTipFunctionProcInfo, GetCurrentArchitecture());
  }
#else
...
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
  inline void DisposeTipFunctionUPP(TipFunctionUPP userUPP)
  {
...
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
  inline OSErr InvokeTipFunctionUPP(Point tip, RgnHandle structure, Rect *r,
                                    BalloonVariant *balloonVariant,
                                    TipFunctionUPP userUPP)
  {
    return (OSErr)CALL_FOUR_PARAMETER_UPP(userUPP, uppTipFunctionProcInfo, tip,
...
#define HMGetHelpMenuRef HMGetHelpMenuHandle
   ...
   */
  OSErr
  HMGetIndHelpMsg(ResType inWhichResType, SInt16 inWhichResID,
