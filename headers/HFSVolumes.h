/**
    ...
    \brief   On-disk data structures for HFS and HFS Plus volumes.
...
enum {
  ...
  kHFSPlusVersion = 0x0004, /** will change as format changes (version 4 shipped
                               with Mac OS 8.1) */
  kHFSPlusMountVersion =
      FOUR_CHAR_CODE('8.10') /** will change as implementations change ('8.10'
                                in Mac OS 8.1) */
};
...
struct HFSPlusAttrInlineData {
  ...
  UInt8 userData[2];  /**    variable length; space allocated is a multiple of 2
                         bytes*/
};
...
enum {
  ...
  kHFSPlusCatalogKeyMinimumLength =
      kHFSPlusCatalogKeyMaximumLength - sizeof(HFSUniStr255) + sizeof(UInt16),
  kHFSCatalogKeyMaximumLength = sizeof(HFSCatalogKey) - sizeof(UInt8),
  kHFSCatalogKeyMinimumLength =
      kHFSCatalogKeyMaximumLength - sizeof(Str31) + sizeof(UInt8),
...
struct HFSMasterDirectoryBlock {
  ...
  HFSExtentDescriptor drEmbedExtent; /** embedded volume location and size
                                        (formerly drVBMCSize and drCtlCSize) */
...
struct HFSPlusVolumeHeader {
  ...
  UInt32 lastMountedVersion; /** implementation version which last mounted
                                volume */
  UInt32 reserved;           /** reserved - set to zero */
  ...
  UInt32 totalBlocks; /** number of allocation blocks in volume (includes this
                         header and VBM*/
