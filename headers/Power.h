...
#pragma pack(2)
  ...
  enum
  {
    // Bit positions for ModemByte     modemOnBit = 0,
    ringWakeUpBit = 2,
    modemInstalledBit = 3,
    ringDetectBit = 4,
    modemOnHookBit = 5
  ...
  enum
  {
    // masks for ModemByte     modemOnMask = 0x01,
    // masks for ModemByte
  ...
  enum
  {
    // bit positions for BatteryByte     chargerConnBit = 0,
    hiChargeBit = 1,
  ...
  enum
  {
    // masks for BatteryByte     chargerConnMask = 0x01,
    hiChargeMask = 0x02,
  ...
  enum
  {
    // bit positions for SoundMixerByte     MediaBaySndEnBit = 0,
    PCISndEnBit = 1,
    ZVSndEnBit = 2,
    PCCardSndEnBit = 3
  ...
  enum
  {
    // masks for SoundMixerByte     MediaBaySndEnMask = 0x01,
    PCISndEnMask = 0x02,
    ZVSndEnMask = 0x04,
  ...
  enum
  {
    // commands to SleepQRec sleepQProc     kSleepRequest = 1,
    kSleepDemand = 2,
    kSleepWakeUp = 3,
    kSleepRevoke = 4,
    kSleepUnlock = 4,
    kSleepDeny = 5, // A non-zero value clients can use to deny requests    kSleepNow = 6,
    // commands to SleepQRec sleepQProc 
    kDozeWakeUp = 8,
    kDozeRequest = 9,   // additional messages for Power Mgr 2.0    kEnterStandby = 10, // Idle Queue Only    kEnterRun = 11,     // Idle Queue Only    kSuspendRequest = 12,
    kSuspendDemand = 13,
    kSuspendRevoke = 14,
    kSuspendWakeUp = 15,
    kGetPowerLevel =// A non-zero value clients can use to deny requests
    kSetPowerLevel = 17,
    kDeviceInitiatedWake = 18,
    kWakeToDoze = 19,
    kDozeToFullWakeUp = // additional messages for Power Mgr 2.0
    kGetPowerInfo = 21,// Idle Queue Only
    kGetWakeOnNetInfo = // Idle Queue Only
    kSuspendWakeToDoze = 23,
    kEnterIdle = 24, // Idle Queue Only    kStillIdle = 25, // Idle Queue Only    kExitIdle = 26   // Idle Queue Only  };

  enum
  {
    // depreciated commands to SleepQRec sleepQProc     sleepRequest = kSleepRequest,
    ...
    sleepNow = kSleepNow,
    dozeDemand = kDoz// Idle Queue Only
    dozeWakeUp = kDoz// Idle Queue Only
    dozeRequest = kDo// Idle Queue Only
  ...
  // Power Handler func messages   typedef UInt32 PowerLevel;
  /* Power levels corresponding to PCI Bus Power Management Interface Spec (PMIS)
   */
  enum
  {
    kPMDevicePowerLevel_On = 0, // fully-powered 'On' state (D0 state)        kPMDevicePowerLevel_D1 = 1, // not used by Apple system SW             kPMDevicePowerLevel_D2 = 2, // not used by Apple system SW             kPMDevicePowerLevel_Off = 3 /* main PCI bus power 'Off', but PCI standby power
                                   available (D3cold state) */
  };

  // PowerHandlerProc definition   typedef CALLBACK_API(OSStatus, PowerHandlerProcPtr)(UInt32 message, void *param,
                                                      UInt32 refCon,
  ...
  PowerHandlerUPP// not used by Apple system SW         
  NewPowerHandlerUPP(PowerHandle// not used by Apple system SW
#if !OPAQUE_UPP_TYPES
  enum
  {
    uppPowerHandlerProcInfo = 0x00003FF0
        // PowerHandlerProc definition
        inline PowerHandlerUPP
...
// support for pre-Carbon UPP routines: New...Proc and Call...Proc #define NewPowerHandlerProc(userRoutine) NewPowerHandlerUPP(userRoutine)
...
#define kIdleQueueDeviceType "idle-queue"
  ...
  enum
  {
    kUseDefaultMinimumWakeTime = 0, // Defaults to 5 minutes    kPowerSummaryVersion = 1,       // Version of PowerSummary structure.    kDevicePowerInfoVersion = 1     // Version of DevicePowerInfo structure.  };

  enum
  {
    // PowerSummary flags    kPCIPowerOffAllowed = (1L << 0) // PCI power off is allowed.  };

  enum// CALL_NOT_IN_CARBON 
  {
    // DevicePowerInfo flags    kDevicePCIPowerOffAllowed =
// support for pre-Carbon UPP routines: New...Proc and Call...Proc 
        (1L << 1), // Device supports Power Mgt Interface Spec.    kDeviceCanAssertPMEDuringSleep =
        (1L << 2),                           // Device can assert PME# during sleep.    kDeviceUsesCommonLogicPower = (1L << 3), // Device uses common-logic power    kDeviceDriverPresent = (1L << 4),        // Driver present for device.    kDeviceDriverSupportsPowerMgt =
        (1L << 5) // Driver installed a power handler.  };
// CALL_NOT_IN_CARBON 
  struct DevicePowerInfo
  {
    UInt32 version;          // Version of this structure.    RegEntryID regID;        // RegEntryID for device.    OptionBits flags;        // Flags    UInt32 minimumWakeTime;  // Minimum seconds before sleeping again.    UInt32 sleepPowerNeeded; // Milliwatts needed in the sleep state.  };
  typedef struct DevicePowerInfo DevicePowerInfo;
  struct PowerSummary
  {
    UInt32 version;             // Version of this structure.    OptionBits flags;           // Flags    UInt32 sleepPowerAvailable; // Milliwatts available during sleep.    UInt32 sleepPowerNeeded;    // Milliwatts needed during sleep.    UInt32 minimumWakeTime;     // Minimum seconds before sleeping again.    ItemCount deviceCount;      // Number of device power info records.    DevicePowerInfo devices[1]; // Array of device power info records.  };
  // PCI power management support
  enum
  {
    // SleepQRec.sleepQFlags     noCalls = 1,
    noRequest = 2,// Defaults to 5 minutes
    slpQType = 16,// Version of PowerSummary structure.
    sleepQType = 16// Version of DevicePowerInfo structure.
  };

  // Power Mgt Apple Event types and errors   enum
  {
    // PowerSummary flags
    kAEMacLowPowerSaveData = FOUR_CH// PCI power off is allowed.
  ...
  /*// DevicePowerInfo flags
     ...
     by the Power M// PCI power off is allowed for device.
     woke the machine.
  */// Device supports Power Mgt Interface Spec.
  enum
  {// Device can assert PME# during sleep.
    kDeviceDidNotWakeMachine = 0, // device d// Device uses common-logic power
        1, // device did wake machine and req// Driver present for device.
        2 // device did wake machine and requests partial wakeup  };
// Driver installed a power handler.
  // bits in bitfield returned by PMFeatures   enum
  {
    hasWakeupTimer = 0,      // 1=wakeup timer is supported                        hasSharedModemPort = 1,  // 1=modem port shared by SCC and internal modem      hasProcessorCycling = 2, // 1=processor cycling is supported                    mustProcessorCycle = 3,  // 1=processor cycling should not be turned off      hasReducedSpeed = 4,     // 1=processor can be started up at reduced speed         dynamicSpeedChange = 5,  // 1=processor speed can be switched dynamically      hasSCSIDiskMode = 6,     // 1=SCSI Disk Mode is supported                     canGetBatteryTime = 7,   // 1=battery time can be calculated                    canWakeupOnRing = 8,     // 1=can wakeup when the modem detects a ring              hasDimmingSupport =
        9,                      // 1=has dimming support built in (DPMS standby by default)       hasStartupTimer = 10,       // 1=startup timer is supported                        hasChargeNotification = 11, /* 1=client can determine of charge connect status
                             // Version of this structure.
    hasDimSuspendSupport =// RegEntryID for device.
        12,                  // Flags
        15,            // 1=h// Minimum seconds before sleeping again.
        18, // 1=hardware sup// Milliwatts needed in the sleep state.
        19, // 1=hardware support UPS integration and reporting          hasAggressiveIdling =
        20,                // 1=Power Manager only resets OverallAct on UsrActvity         supportsIdleQueue = 21 // 1=Power Manager supports the idle queue   };

  // bits in bitfield returned by GetIntModemInfo and set by SetIntModemState   enum
  {// Version of this structure.
    hasInternalModem = 0,    // // Flags
// Milliwatts available during sleep.
  // bits in BatteryInfo.flags  // Milliwatts needed during sleep.
  {// Minimum seconds before sleeping again.
    batteryInstalled = 7, // 1=b// Number of device power info records.
// Array of device power info records.
  enum
  {
    HDPwrQType = 0x4844,    // 'HD' hard disk spindown queue element type         PMgrStateQType = 0x504D // 'PM' Power Manager state queue element type   };
  ...
  // Power Mgt Apple Event types and errors 
    ...
    // power mgt class
    pmWakeupTimerChanged = 9,// event ids
  ...
  enum
  {
    ...
    pmDimmingTimeoutChangedMask = // device did NOT wake machine
    pmDimmingEnableChangedMask = (1 << pmDimmingEnableChanged),
    pmDiskM// device did wake machine and requests full wakeup
    pmProcessorCyclingChangedMask = (1 << pmProcessorCyclingChanged),
    pmProc// device did wake machine and requests partial wakeup
  ...
  // bits in bitfield returned by PMFeatures 
    ...
    pmWakeOnNetActivityChange// 1=wakeup timer is supported                    
  };// 1=modem port shared by SCC and internal modem  
// 1=processor cycling is supported                
  // System Activity Selector// 1=processor cycling should not be turned off  
   * hasAggressiveIdling PMFe// 1=processor can be started up at reduced speed     
  /*         Use IdleActivity// 1=processor speed can be switched dynamically  
   * OverallAct will only    // 1=SCSI Disk Mode is supported                 
  /*         delay power cycl// 1=battery time can be calculated                
   * amount when             // 1=can wakeup when the modem detects a ring          
  /*         hasAggressiveIdling is set.  Don't use IdleActivity unless
   * hasAggressiveIdling is set;// 1=has dimming support built in (DPMS standby by default)   
  /*         hasAggressiveIdling// 1=startup timer is supported                    
   * and well do different      */
  //         things depending on which Power Manager is currently running.   enum
  {
    OverallAct = 0,  // Delays // 1=supports dimming LCD and CRT to DPMS suspend state     
        3,           // Delays // 1=hardware supports wake on network activity 
// 1=hardware can wake when opened                   
  // Storage Media sleep mode defines   enum
  {// 1=hardware can power off PCI bus during sleep if cards allow 
    kMediaModeOn = 0,  // 1=hardware supports deep sleep (hibernation) mode   
...
// 1=Power Manager supports the idle queue 
  // definitions for HDQueueElement.hdFlags     enum
  {
  // bits in bitfield returned by GetIntModemInfo and set by SetIntModemState 
  };

  struct ActivityInfo// 1=internal modem installed               
  {// 1=internal modem has detected a ring          
    short ActivityType; /* Ty// 1=internal modem is off hook               
                           Up// 1=wakeup on ring is enabled                 
    unsigned long// 1=external modem selected             
        ActivityTime; // Time// 1=set bit, 0=clear bit (SetIntModemState)   
  typedef struct ActivityInfo ActivityInfo;
  // information returned by GetScaledBatteryInfo   struct BatteryInfo
  // bits in BatteryInfo.flags                                    
  // ("chargerConnected" doesn't mean the charger is plugged in)  
  typedef struct BatteryInfo BatteryInfo;

  typedef SInt8 ModemByte;// 1=battery is currently connected             
  typedef SInt8 BatteryByt// 1=battery is being charged               
  typedef SInt8 SoundMixer// 1=charger is connected to the PowerBook         
  typedef long PMResultCod// 1=there is a UPS connected               
  typedef struct SleepQRec// 1=UPS is source of power                
  ...
  typedef CALLBACK_API(void,// 'PM' Power Manager state queue element type 
  ...
  struct SleepQRec
  {
    SleepQRecPtr sleepQLink; // pointer to next queue element              short sleepQType;        // queue element type (must be SleepQType)           SleepQUPP sleepQProc;    // pointer to sleep universal proc ptr             short sleepQFlags;       // flags                         };

  struct HDQueueElement
  {
    struct HDQueueElement *hdQLink; // pointer to next queue element              short hdQType;                  // queue element type (must be HDPwrQType)           short hdFlags;                  // miscellaneous flags                       HDSpindownUPP hdProc;           // pointer to routine to call               long hdUser;                    // user-defined (variable storage, etc.)     };

  struct PMgrQueueElement
  {
    struct PMgrQueueElement *pmQLink; // pointer to next queue element              short pmQType;                    // queue element type (must be PMgrStateQType)        short pmFlags;                    // miscellaneous flags                       long pmNotifyBits;                // bitmap of which changes to be notified for     PMgrStateChangeUPP pmProc;        // pointer to routine to call               long pmUser;                      // user-defined (variable storage, etc.)     };

  struct BatteryTimeRec
  {
    unsigned long
        expectedBatteryTime; // estimated battery time remaining (seconds)     unsigned long
        minimumBatteryTime; // minimum battery time remaining (seconds)         unsigned long
        maximumBatteryTime; // maximum battery time remaining (seconds)         unsigned long
        timeUntilCharged; // time until battery is fully charged (seconds)  };
  typedef struct BatteryTimeRec BatteryTimeRec;

  struct WakeupTime
  {
    unsigned long wakeTime; // wakeup time (same format as current time)       Boolean wakeEnabled;    // 1=enable wakeup timer, 0=disable wakeup timer      SInt8 filler;
  };
  typedef struct WakeupTime WakeupTime;

  struct StartupTime
  {
    unsigned long startTime; // startup time (same format as current time)         Boolean startEnabled;    // 1=enable startup timer, 0=disable startup timer        SInt8 filler;
  };
  typedef struct StartupTime StartupTime;
  // PowerSource version  enum
  {
  ...
  enum
  {
    bSourceIsBattery = 0, // power source is battery    bSourceIsAC = 1,         // power source is AC    bSourceCanBeCharged = 2, // power source can be charged    bSourceIsUPS = 3,        /* power source is UPS. NOTE: software should set
    bSourceIsBattery and bSourceIsAC also,
    as appropriate * /
        bSourceProvidesWarnLevels =
        4, // power source provides low power and dead battery warning levels    kSourceIsBatteryMask = (1 << bSourceIsBattery),
...
// Delays idle sleep and dimming by timeout time          
  // PowerSourceFlags// Delays idle sleep and power cycling by small amount 
  enum
  {// Delays hard drive spindown and idle sleep by small amount  
    bSourceIsAvailabl// Delays idle sleep by timeout time                 
  ...
  enum// Media active (Drive spinning and at full power)    
  {// Media standby (not implemented)    
    kCapacityIsActual = 0, // Media Idle (not implemented)   
                              same units as max*/
    kCapacityIsPercentOfMax =
        1 // current capacity is expressed as a percentage of maximumCapacity  };

  // Net Activity Wake Options  enum
  {
  ...
  // definitions for HDQueueElement.hdFlags   
    ...
    kWakeOnAllNetAccesses// 1 = call this routine on the second pass     
    kUnmountServersBeforeSleepingMask = (1 << kUnmountServersBeforeSleepingBit)
  };

  // Power Source capacity usage types  enum
  {
    kCurrentCapacityIsActualValue =
        0, // currentCapacity is a real value in same units as maxCapacity    kCurrentCapacityIsPercentOfMax =
        1 // currentCapacity is expressed as a percentage of maxCapacity.  };
// Time of last activity (in ticks) of specified type. 
  typedef SInt16 PowerSourceID;
  struct PowerSourceParamBlock
  // information returned by GetScaledBatteryInfo 
    PowerSourceID sourceID;     // unique id assigned by Power Mgr    UInt16 sourceCapacityUsage; // how currentCapacity is used    UInt32 sourceVersion;       // version of this record    OptionBits sourceAttr;      // attribute flags (see below)    OptionBits sourceState;     // state flags (see below)    UInt32 currentCapacity;     // current capacity, in                                //   milliwatts or %    UInt32 maxCapacity;         // full capacity, in milliwatts    UInt32 timeRemaining;       // time left to deplete,                                 //   in milliwatt-hours    UInt32 timeToFullCharge;    // time to charge,                                 //   in milliwatt-hours    UInt32 voltage;             // voltage in millivolts    SInt32 current;             // current in milliamperes                                 //  (negative if consuming,                                 //   positive if charging)    UInt32 lowWarnLevel;        /* low warning level in milliwatts (or % if
                                   sourceCapacityUsage is %)*/
    UInt32 deadWarnLevel// misc flags (see below)                  
                        // scaled warning level (0-255)               
    UInt32 reserved[16];// reserved for internal use             
  typedef struct PowerSo// scaled battery level (0-255)               
...
updates the real-time clock a// pointer to next queue element          
second. When the real-time cl// queue element type (must be SleepQType)       
the Power Manager returns the// pointer to sleep universal proc ptr         
The WUTime parameter specifie// flags                       
...
timer to a time earlier than the cur// pointer to next queue element          
effectively disable the wakeup timer// queue element type (must be HDPwrQType)       
</pre>// miscellaneous flags                   
* \copyright THINK Reference © 1991-// pointer to routine to call           
   *    \non_carbon_cfm   in Interfa// user-defined (variable storage, etc.)   
*    \carbon_lib        in CarbonLib 1.0 and later
*    \mac_os_x         in version 10.0 and later
*/
  OSErr
  SetWUTime(long wuTime);// pointer to next queue element          
// queue element type (must be PMgrStateQType)    
  /**// miscellaneous flags                   
  \brief Get the current wakeup time s// bitmap of which changes to be notified for 
// pointer to routine to call           
  <pre>The GetWUTime function returns // user-defined (variable storage, etc.)   
...
</pre>// estimated battery time remaining (seconds) 
* \copyright THINK Reference © 1991-1992 Symantec Corporation
   *    \non_carbon_cfm   in// minimum battery time remaining (seconds)     
*    \carbon_lib        in CarbonLib 1.0 and later
*    \mac_os_x         in ve// maximum battery time remaining (seconds)     
*/
  OSErr// time until battery is fully charged (seconds)
  GetWUTime(long *wuTime, Byte *wuFlag);

  /**
  \brief Get status of battery charger and voltage level

  <pre>The BatteryStatus fun// wakeup time (same format as current time)   
the voltage level of the bat// 1=enable wakeup timer, 0=disable wakeup timer  
follows:
BitMeaning
7Reserved.
6Reserved.
...
4The battery warning. If 1, t// startup time (same format as current time)     
3The dead battery indicator. // 1=enable startup timer, 0=disable startup timer    
...
Due to the nature of lead-aci// power source is battery
difficult to measure accurate// power source is AC
the measured voltage by 30 pe// power source can be charged
...
When the ba// power source provides low power and dead battery warning levels
...
*/// power source is installed
  OSErr// power source being charged
  BatteryStatus(Byte *status// a charger is connected
...
3The modem// current capacity is expressed as a percentage of maximumCapacity
2The state of the ring-wakeup feature. If 1, the ring-wakeup feature
is enabled.
1R// Net Activity Wake Options
...
* \copyright THINK Reference © 1991-1992 Symantec Corporation
   *    \non_carbon_cfm   in InterfaceLib 7.1 and later
*    \carbon_lib        in CarbonLib 1.0 and later
*    \mac_o// currentCapacity is a real value in same units as maxCapacity
*/
  OSErr// currentCapacity is expressed as a percentage of maxCapacity.
  ModemStatus(Byte *status);

    /**
    \brief Reset the activity timer

    <pre>IdleUpdate is equivalent // unique id assigned by Power Mgr
  </pre>// how currentCapacity is used
  * \copyright THINK Reference © 1// version of this record
     *    \non_carbon_cfm   in Int// attribute flags (see below)
  *    \carbon_lib        in Carbo// state flags (see below)
  *    \mac_os_x         in versio// current capacity, in
  */                             //   milliwatts or %
#if TARGET_OS_MAC && TARGET_CPU_ // full capacity, in milliwatts
#pragma parameter __D0 IdleUpdat // time left to deplete,
#endif                           //   in milliwatt-hours
  long// time to charge, 
  IdleUpdate(void) ONEWORDINLINE//   in milliwatt-hours
// voltage in millivolts
  /**// current in milliamperes 
  \brief Return the current effe//  (negative if consuming, 
...
* \copyright THINK Reference © 1// for future expansion
   *    \non_carbon_cfm   in InterfaceLib 7.1 and later
*    \carbon_lib        in CarbonLib 1.0 and later
*    \mac_os_x         in version 10.0 and later
*/
...
#pragma parameter __D0 RemovePowerSource(__D0)
  ...
  RemovePowerSource(PowerSourceID inSourceID)
      FOURWORDINLINE(0x4840, 0x303C, 0x0031, 0xA09E);
...
#pragma parameter EnableServerMode(__D0)
  ...
  EnableServerMode(Boolean inEnable)
      FOURWORDINLINE(0x4840, 0x303C, 0x0034, 0xA09E);
...
#pragma parameter __D0 NumBatteriesInstalled
  ...
  }; // register 4_bytes:D0 Func(4_bytes:D0, 4_bytes:A0) #ifdef __cplusplus
...
#if ! // register 4_bytes:D0 Func(4_bytes:D0, 4_bytes:A0)
#ifdef __cplusplus
...
#pragma pack(pop)
#elif PRAGMA_STRUCT_PACK
#prag // pascal no_return_value Func(4_bytes, 4_bytes)
#endif
...
#endif // __POWER__ */ueElement *theElement, long stateBits,
                         PMgrStateChangeUPP userUPP);
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
                         inline void InvokePMgrStateChangeUPP(PMgrQueueElement * theElement,
...
#endif // __POWER__ * /*/*/ * /*/*/ * /*/*/ * /*/*/ * /*/*/// support for pre-Carbon UPP routines: New...Proc and Call...Proc
  // CALL_NOT_IN_CARBON
  // __POWER__
  // support for pre-Carbon UPP routines: New...Proc and Call...Proc
  // CALL_NOT_IN_CARBON
