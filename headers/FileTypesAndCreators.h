/**
    ...
    \brief   Symbolic constants for FileTypes and signatures of popular
   documents.
...
have file types (or creators). Another problem is some Macintosh applications
keep the same file type for a file after changing the file format from one
version to the next.  Thus, the HFS file type cannot be used to distinguish
between formats.

    This file is an effort to define FileTypes and creator codes for
non-Macintosh file formats and define FileTypes that distinguish between
formats.


There are two main categories of constants defined in FileTypesAndCreators.h.
One is creator signatures of applications the other is file types.  To make this
easier to use, some style guidelines are provided.
...
U.S. version is always used. It is optional to make symbolic constants for
localized names. If a U.S. is not available, another Roman name is used (this is
for ease of editing in MPW.)  Example:
...
different, then the version number is added to the end of the symbolic name, for
the first version in which the signature changed.  Example:
...
Mac, then the creator for a file from the other platform should be the same as
the creator of the Macintosh application.   Example:
...
of the U.S. version is always used. It is optional to make symbolic constants
for localized kind strings.  If a U.S. is not available, another Roman kind
string is used (this is for ease of editing in MPW.)  Example:

        ftSurfWriterDocument =  'SRFD,
        ftSkiWriterDocument =   'SRFD'  // optional Canadian name

    If multiple versions of an application exist, and the document for files
created by each are different (meaning one version of the app can not read the
documents of a newer version), then the version number is added to the end of
the symbolic name, for the first version in which the format changed.  Example:

       // version 1 and 2 docs have same format, version 3 has a different
format ftDirtWriterDocument =          'DDRT', ftDirtWriter3Document = '3DRT'

    If multiple versions of an application exist, and the document for files
created by each are different, but the catInfo file type has always been the
same, then constants are made for each unique format and one is made for the
catInfo type actually used.
...
and the file formats are the same (binary interchangeable), then the file type
for a file from the other platform should be the same as the file type of the
Macintosh document.   Example:
...
up.  If the program is used on two different non-Mac platforms and their file
formats are not the same, the platform name is added to the name.  Typically
after the program name and before document.  Example:
...
enum {
  ...
  sigTheWritingCenter = 0x0A1A5750, /** this 'unprintable unprintable WP' One of
                                       the unprintables is a line feed.  */
...
enum {
  /** graphics */
  sigIllustrator = FOUR_CHAR_CODE('ART5'),
  ...
  sigAdobePremiere =
      FOUR_CHAR_CODE('PrMr'), /** was previously misspelled as sigAdobePremier*/
  ftAdobePremiereMovie = FOUR_CHAR_CODE(
      'MooV'), /** was previously misspelled as ftAdobePremierMovie*/
...
enum {
  ...
  sigAppleSearchToo = FOUR_CHAR_CODE(
      'hobs'), /** the following are files types for system files */
...
enum {
  ftApplicationName =
      FOUR_CHAR_CODE('apnm'), /** this is the type used to define the
                                 application name in a kind resource */
  sigIndustryStandard = FOUR_CHAR_CODE(
      'istd'), /** this is the creator used to define a kind string in a kind
                  resource for a FileType that has many creators  */
  ftXTND13TextImport =
      FOUR_CHAR_CODE('xt13') /** this is a pseduo-format used by "XTND for
                                Apps". The taDstIsAppTranslation bit is set */
};
...
enum {
  ...
  ftStereoLithographyGraphicPC =
      FOUR_CHAR_CODE('STL '), /**    3D Systems     - not registered */
  ftZoomGraphicPC =
      FOUR_CHAR_CODE('ZOOM'), /**    Abvent          - not registered */
  ftFocusGraphicPC =
      FOUR_CHAR_CODE('FOCS'), /**    Abvent          - not registered */
  ftWaveFrontGraphicPC =
      FOUR_CHAR_CODE('WOBJ'), /**    WaveFront      - not registered */
  ftSculpt4DGraphicPC =
      FOUR_CHAR_CODE('Scn2'), /**    Byte By Byte   - not registered */
  ftMiniPascal3GraphicPC =
      FOUR_CHAR_CODE('MPT3'), /**    Graphsoft      - not registered */
  ftMiniPascal4GraphicPC =
      FOUR_CHAR_CODE('MPT4'), /**    Graphsoft      - not registered */
  ftWalkThroughGraphicPC =
      FOUR_CHAR_CODE('VWLK'), /**    Virtus          - not registered */
