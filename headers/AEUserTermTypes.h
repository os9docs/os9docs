/**
    ...
    \brief   AppleEvents AEUT resource format Interfaces.
...
enum {
  ...
  kAEUTTightBindingFunction =
      12, /** if this is a tight-binding precedence function. */
  /** AppleScript 1.3: new bits for reply, direct parameter, parameter, and
     property flags */
  kAEUTEnumsAreTypes =
      11, /** if the enumeration is a list of types, not constants */
  kAEUTEnumListIsExclusive =
      10,                    /** if the list of enumerations is a proper set */
  kAEUTReplyIsReference = 9, /** if the reply is a reference, not a value */
  kAEUTDirectParamIsReference =
      9, /** if the direct parameter is a reference, not a value */
  kAEUTParamIsReference = 9, /** if the parameter is a reference, not a value */
  kAEUTPropertyIsReference =
      9, /** if the property is a reference, not a value */
  kAEUTNotDirectParamIsTarget =
      8, /** if the direct parameter is not the target of the event */
...
enum {
  kLaunchToGetTerminology =
      (1 << 15), /**    If kLaunchToGetTerminology is 0, 'aete' is read directly
                    from res file.  If set to 1, then launch and use 'gdut' to
                    get terminology. */
  kDontFindAppBySignature =
      (1 << 14), /**    If kDontFindAppBySignature is 0, then find app with
                    signature if lost.  If 1, then don't */
  kAlwaysSendSubject =
      (1 << 13) /**    If kAlwaysSendSubject 0, then send subject when
                   appropriate. If 1, then every event has Subject Attribute */
};
...
enum {
  ...
  kOSIZDontOpenResourceFile =
      15, /** If set, resource file is not opened when osax is loaded */
  kOSIZdontAcceptRemoteEvents = 14, /** If set, handler will not be called with
                                       events from remote machines */
  kOSIZOpenWithReadPermission =
      13, /** If set, file will be opened with read permission only */
  kOSIZCodeInSharedLibraries = 11 /** If set, loader will look for handler in
                                     shared library, not osax resources */
};
