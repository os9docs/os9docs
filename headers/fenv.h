...
/**
    ...
    \brief   Floating-Point environment for PowerPC and 68K
 ...
 * *
 * fexcept_t is a type for representing the floating-point * exception
 *flag state collectively. *
 * *
 ********************************************************************************/
#if TARGET_CPU_PPC
  typedef long fenv_t;
  ...
  enum
  {
    ...
    FE_ALL_EXCEPT = 0x3E000000 /* FE_INEXACT | FE_DIVBYZERO | FE_UNDERFLOW |
...
#define FE_DFL_ENV &_FE_DFL_ENV /* pointer to default environment */
   ...
   * "*flagp" must have been set by a previous call to "fegetexcept". * This
   *function does not raise exceptions; it just sets the state of * the
   *flags. *
   *******************************************************************************/
  EXTERN_API_C(void)
   ...
   * "feholdexcept", by an implementation-defined macro of type "fenv_t", * or
   *by the use of the pointer macro FE_DFL_ENV as the argument. *
   *******************************************************************************/
  EXTERN_API_C(void)
   ...
   * pointer argument "envp", and then re-raises the saved exceptions. * This
   *function, which supersedes the SANE function "procexit", can be * used in
   *conjunction with "feholdexcept" to write routines which hide * spurious
...
#if TARGET_CPU_68K
/*******************************************************************************
 * The following functions provide control of rounding precision. * Because
 *the PowerPC does not provide this capability, these functions * are
 *available only for the 68K Macintosh. Rounding precision values * are
 *defined by the rounding precision macros. These functions are *
