/**
    ...
    \brief   URL Access Interfaces.

    \introduced_in  URLAccess 2.0
...
enum {
  ...
  kURLExpandFileFlag =
      1 << 2, /** Use StuffIt engine to expand file if necessary*/
  kURLDisplayProgressFlag = 1 << 3,
  kURLDisplayAuthFlag =
      1 << 4,              /** Display auth dialog if guest connection fails*/
  ...
  kURLDoNotTryAnonymousFlag =
      1 << 7, /** Don't try to connect anonymously before getting logon info*/
  kURLDirectoryListingFlag =
      1 << 8, /** Download the directory listing, not the whole directory*/
  kURLExpandAndVerifyFlag =
      1 << 9, /** Expand file and then verify using signature resource*/
  kURLNoAutoRedirectFlag = 1
                           << 10, /** Do not automatically redirect to new URL*/
  kURLDebinhexOnlyFlag =
      1 << 11, /** Do not use Stuffit Expander - just internal debinhex engine*/
  kURLDoNotDeleteOnErrorFlag = 1 << 12, /** Do not delete the downloaded file if
                                           an error or abort occurs.*/
...
enum {
  ...
  kURLAllBufferEventsMask =
      kURLDataAvailableEventMask + kURLTransactionCompleteEventMask,
  kURLAllNonBufferEventsMask =
      kURLInitiatedEventMask + kURLDownloadingMask + kURLUploadingMask +
      kURLAbortInitiatedMask + kURLCompletedEventMask +
      kURLErrorOccurredEventMask + kURLPercentEventMask +
      kURLPeriodicEventMask + kURLPropertyChangedEventMask,
 ...
 *    \non_carbon_cfm   in URLAccessLib 1.0 and later
...
#else
#define URLAccessAvailable()                                                   \
  ((URLGetURLAccessVersion != (void *)kUnresolvedCFragSymbolAddress))
#endif
...
#if !OPAQUE_UPP_TYPES
enum {
  uppURLNotifyProcInfo = 0x00000FF0
}; /** pascal 4_bytes Func(4_bytes, 4_bytes, 4_bytes) */
#ifdef __cplusplus
inline URLNotifyUPP NewURLNotifyUPP(URLNotifyProcPtr userRoutine) {
  return userRoutine;
}
#else
...
#if !OPAQUE_UPP_TYPES
enum {
  uppURLSystemEventProcInfo = 0x000003F0
}; /** pascal 4_bytes Func(4_bytes, 4_bytes) */
#ifdef __cplusplus
inline URLSystemEventUPP
NewURLSystemEventUPP(URLSystemEventProcPtr userRoutine) {
  return userRoutine;
}
#else
...
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
inline OSStatus InvokeURLNotifyUPP(void *userContext, URLEvent event,
                                   URLCallbackInfo *callbackInfo,
                                   URLNotifyUPP userUPP) {
  return (*userUPP)(userContext, event, callbackInfo);
}
#else
#define InvokeURLNotifyUPP(userContext, event, callbackInfo, userUPP)          \
  (*userUPP)(userContext, event, callbackInfo)
#endif
#endif
...
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
inline OSStatus InvokeURLSystemEventUPP(void *userContext, EventRecord *event,
                                        URLSystemEventUPP userUPP) {
  return (*userUPP)(userContext, event);
}
#else
#define InvokeURLSystemEventUPP(userContext, event, userUPP)                   \
  (*userUPP)(userContext, event)
#endif
#endif
 ...
 */
EXTERN_API(OSStatus)
URLSimpleDownload(const char *url, FSSpec *destination, /** can be NULL */
 ...
 */
EXTERN_API(OSStatus)
URLDownload(URLReference urlRef, FSSpec *destination, /** can be NULL */
            Handle destinationHandle,                 /** can be NULL */
            URLOpenFlags openFlags,
            URLSystemEventUPP eventProc, /** can be NULL */
            void *userContext);          /** can be NULL */
 ...
 */
EXTERN_API(OSStatus)
URLSimpleUpload(const char *url, const FSSpec *source, URLOpenFlags openFlags,
                URLSystemEventUPP eventProc, /** can be NULL */
                void *userContext);          /** can be NULL */
 ...
 */
EXTERN_API(OSStatus)
URLUpload(URLReference urlRef, const FSSpec *source, URLOpenFlags openFlags,
          URLSystemEventUPP eventProc, /** can be NULL */
          void *userContext);          /** can be NULL */
 ...
 */
EXTERN_API(OSStatus)
URLNewReference(const char *url, URLReference *urlRef);
 ...
 */
EXTERN_API(OSStatus)
URLOpen(URLReference urlRef, FSSpec *fileSpec,           /** can be NULL */
        URLOpenFlags openFlags, URLNotifyUPP notifyProc, /** can be NULL */
        URLEventMask eventRegister, void *userContext);  /** can be NULL */
 ...
 */
EXTERN_API(OSStatus)
URLGetDataAvailable(URLReference urlRef, Size *dataSize);
 ...
 */
EXTERN_API(OSStatus)
URLGetBuffer(URLReference urlRef, void **buffer, Size *bufferSize);
 ...
 */
EXTERN_API(OSStatus)
URLReleaseBuffer(URLReference urlRef, void *buffer);
 ...
 */
EXTERN_API(OSStatus)
URLGetProperty(URLReference urlRef, const char *property, void *propertyBuffer,
               Size bufferSize);
 ...
 */
EXTERN_API(OSStatus)
URLGetPropertySize(URLReference urlRef, const char *property,
                   Size *propertySize);
 ...
 */
EXTERN_API(OSStatus)
URLSetProperty(URLReference urlRef, const char *property, void *propertyBuffer,
               Size bufferSize);
 ...
 */
EXTERN_API(OSStatus)
URLGetCurrentState(URLReference urlRef, URLState *state);
 ...
 */
EXTERN_API(OSStatus)
URLGetError(URLReference urlRef, OSStatus *urlError);
 ...
 */
EXTERN_API(OSStatus)
URLGetFileInfo(StringPtr fName, OSType *fType, OSType *fCreator);
