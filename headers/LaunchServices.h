/**
    ...
    \brief   Public interfaces for LaunchServices.framework
...
enum { kLSInitializeDefaults = 0x00000001 };

enum {
  kLSMinCatInfoBitmap =
      (kFSCatInfoNodeFlags | kFSCatInfoParentDirID | kFSCatInfoFinderInfo |
       kFSCatInfoFinderXInfo) /* minimum info needed to avoid a FSGetCatalogInfo
                                 call when fetching item information */
};

enum {
  kLSInvalidExtensionIndex =
      (unsigned long)0xFFFFFFFF /* Index returned from LSGetExtensionInfo when
...
enum {
  ...
  kLSRequestBasicFlagsOnly =
      0x00000004, /* all but type of application and extension flags - safe to
                     use from threads*/
  ...
  kLSRequestAllInfo =
      (unsigned long)0xFFFFFFFF /* NOT SAFE to use from threads*/
};
...
enum {
  ...
  kLSItemInfoIsInvisible =
      0x00000040, /* does not include '.' files or '.hidden' entries*/
  ...
  kLSItemInfoAppPrefersNative =
      0x00000200, /* Carbon app that prefers to be launched natively*/
  kLSItemInfoAppPrefersClassic =
      0x00000400, /* Carbon app that prefers to be launched in Classic*/
...
enum {
  kLSRolesNone =
      0x00000001, /* no claim is made about support for this type/scheme*/
...
enum {
  kLSLaunchDefaults =
      0x00000001, /* default = open, async, use Info.plist, start Classic*/
  ...
  kLSLaunchInhibitBGOnly =
      0x00000080, /* causes launch to fail if target is background-only.*/
  kLSLaunchDontAddToRecents =
      0x00000100, /* do not add app or documents to recents menus.*/
  kLSLaunchDontSwitch = 0x00000200, /* don't bring new app to the foreground.*/
  kLSLaunchNoParams =
      0x00000800, /* Use Info.plist to determine launch parameters*/
  kLSLaunchAsync =
      0x00010000, /* launch async; obtain results from kCPSNotifyLaunch.*/
  kLSLaunchStartClassic =
      0x00020000, /* start up Classic environment if required for app.*/
  kLSLaunchInClassic =
      0x00040000, /* force app to launch in Classic environment.*/
  kLSLaunchNewInstance =
      0x00080000, /* Instantiate app even if it is already running.*/
  kLSLaunchAndHide =
      0x00100000, /* Send child a "hide" request as soon as it checks in.*/
  kLSLaunchAndHideOthers =
      0x00200000 /* Hide all other apps when child checks in.*/
};
...
struct LSLaunchFSRefSpec {
  ...
  const AEDesc *
      passThruParams; /* passed untouched to application as optional parameter*/
 ...
 */
EXTERN_API(OSStatus)
LSCopyItemInfoForRef(const FSRef *inItemRef, LSRequestedInfo inWhichInfo,
 ...
 */
EXTERN_API(OSStatus)
LSSetExtensionHiddenForRef(const FSRef *inRef, Boolean inHide);
 ...
 */
EXTERN_API(OSStatus)
LSSetExtensionHiddenForURL(CFURLRef inURL, Boolean inHide);
 ...
 */
EXTERN_API(OSStatus)
LSGetApplicationForItem(const FSRef *inItemRef, LSRolesMask inRoleMask,
 ...
 */
EXTERN_API(OSStatus)
LSGetApplicationForInfo(OSType inType, OSType inCreator,
 ...
 */
EXTERN_API(OSStatus)
LSGetApplicationForURL(CFURLRef inURL, LSRolesMask inRoleMask, FSRef *outAppRef,
                       CFURLRef *outAppURL);
 ...
 */
EXTERN_API(OSStatus)
LSFindApplicationForInfo(OSType inCreator,
 ...
 */
EXTERN_API(OSStatus)
LSCanRefAcceptItem(const FSRef *inItemFSRef, const FSRef *inTargetRef,
                   LSRolesMask inRoleMask, LSAcceptanceFlags inFlags,
 ...
 */
EXTERN_API(OSStatus)
LSCanURLAcceptURL(CFURLRef inItemURL, CFURLRef inTargetURL,
                  LSRolesMask inRoleMask, LSAcceptanceFlags inFlags,
                  Boolean *outAcceptsItem);
 ...
 */
EXTERN_API(OSStatus)
LSOpenFSRef(const FSRef *inRef, FSRef *outLaunchedRef); /* can be NULL */
 ...
 */
EXTERN_API(OSStatus)
LSOpenCFURLRef(CFURLRef inURL, CFURLRef *outLaunchedURL); /* can be NULL */
 ...
 */
EXTERN_API(OSStatus)
LSOpenFromRefSpec(const LSLaunchFSRefSpec *inLaunchSpec,
 ...
 */
EXTERN_API(OSStatus)
LSOpenFromURLSpec(const LSLaunchURLSpec *inLaunchSpec,
