/**
    ...
    \brief   Apple Speech Recognition Toolbox Interfaces.

    \introduced_in  PlainTalk 1.5
...
enum {
  kSRNotAvailable =
      -5100, /** the service requested is not avail or applicable */
  ...
  kSROutOfMemory =
      -5103, /** an out of memory error occurred in the toolbox memory space */
  kSRNotASpeechObject =
      -5104, /** the object specified is no longer or never was valid */
  ...
  kSRFeedbackNotAvail =
      -5110, /** there is no feedback window associated with SRRecognizer */
  ...
  kSRCantSetDuringRecognition =
      -5113, /** the property can't be set while recognition is in progress --
                do before or between utterances. */
  kSRAlreadyListening = -5114,  /** in response to SRStartListening */
  kSRNotListeningState = -5115, /** in response to SRStopListening */
  kSRModelMismatch =
      -5116, /** no acoustical models are avail to match request */
  kSRNoClientLanguageModel =
      -5117, /** trying to access a non-specified SRLanguageModel */
  ...
  kSRRecognitionDone =
      -5120, /** search has finished, but nothing was recognized */
  kSROtherRecAlreadyModal =
      -5121, /** another recognizer is modal at the moment, so can't set this
                recognizer's kSRBlockModally property right now */
  kSRHasNoSubItems = -5122, /** SRCountItems or related routine was called on an
                               object without subelements -- e.g. a word --
                               rather than phrase, path, or LM. */
  kSRSubItemNotFound = -5123,     /** returned when accessing a non-existent sub
                                     item of a container */
  kSRLanguageModelTooBig = -5124, /** Cant build language models so big */
  kSRAlreadyReleased =
      -5125, /** this object has already been released before */
  ...
  kSRNotFinishedWithRejection = -5128, /** property not found because the LMObj
                                          is not finished with rejection */
  kSRExpansionTooDeep = -5129, /** Language model is left recursive or is
                                  embedded too many levels */
  kSRTooManyElements = -5130,  /** Too many elements added to phrase or path or
                                  other langauge model object */
  kSRCantAdd = -5131,          /** Can't add given type of object to the base
                                  SRLanguageObject (e.g.in SRAddLanguageObject)   */
  kSRSndInSourceDisconnected = -5132, /** Sound input source is disconnected */
  kSRCantReadLanguageObject =
      -5133, /** An error while trying to create new Language object from file
                or pointer -- possibly bad format */
...
#if !OPAQUE_UPP_TYPES
enum {
  uppSRCallBackProcInfo = 0x000000C0
}; /** pascal no_return_value Func(4_bytes) */
#ifdef __cplusplus
inline SRCallBackUPP NewSRCallBackUPP(SRCallBackProcPtr userRoutine) {
  return (SRCallBackUPP)NewRoutineDescriptor(
      (ProcPtr)(userRoutine), uppSRCallBackProcInfo, GetCurrentArchitecture());
}
#else
#define NewSRCallBackUPP(userRoutine)                                          \
  (SRCallBackUPP) NewRoutineDescriptor(                                        \
      (ProcPtr)(userRoutine), uppSRCallBackProcInfo, GetCurrentArchitecture())
#endif
#endif
...
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
inline void DisposeSRCallBackUPP(SRCallBackUPP userUPP) {
...
#if !OPAQUE_UPP_TYPES
#ifdef __cplusplus
inline void InvokeSRCallBackUPP(SRCallBackStruct *param,
                                SRCallBackUPP userUPP) {
  CALL_ONE_PARAMETER_UPP(userUPP, uppSRCallBackProcInfo, param);
}
#else
#define InvokeSRCallBackUPP(param, userUPP)                                    \
  CALL_ONE_PARAMETER_UPP((userUPP), uppSRCallBackProcInfo, (param))
#endif
#endif
...
#define CallSRCallBackProc(userRoutine, param)                                 \
  InvokeSRCallBackUPP(param, userRoutine)
...
enum {
  kSRFeedbackAndListeningModes = FOUR_CHAR_CODE(
      'fbwn'), /** short: one of kSRNoFeedbackHasListenModes,
                  kSRHasFeedbackHasListenModes, kSRNoFeedbackNoListenModes */
  kSRRejectedWord =
      FOUR_CHAR_CODE('rejq'), /** the SRWord used to represent a rejection */
  kSRCleanupOnClientExit = FOUR_CHAR_CODE(
      'clup') /** Boolean: Default is true. The rec system and everything it
                 owns is disposed when the client application quits */
};

enum {
  kSRNoFeedbackNoListenModes = 0, /** next allocated recognizer has no feedback
                                     window and doesn't use listening modes   */
  kSRHasFeedbackHasListenModes = 1, /** next allocated recognizer has feedback
                                       window and uses listening modes */
  kSRNoFeedbackHasListenModes = 2 /** next allocated recognizer has no feedback
                                     window but does use listening modes  */
};
...
enum {
  ...
  kSRLiveDesktopSpeechSource =
      FOUR_CHAR_CODE('dklv'), /** live desktop sound input */
  kSRCanned22kHzSpeechSource = FOUR_CHAR_CODE(
      'ca22') /** AIFF file based 16 bit, 22.050 KHz sound input */
};
...
enum {
  kSRNotifyRecognitionBeginning =
      1L << 0, /** recognition can begin. client must now call
                  SRContinueRecognition or SRCancelRecognition */
  kSRNotifyRecognitionDone =
      1L << 1 /** recognition has terminated. result (if any) is available. */
};

/** Apple Event selectors */
/** AppleEvent message class  */
enum { kAESpeechSuite = FOUR_CHAR_CODE('sprc') };
...
enum {
  kSRNotificationParam =
      FOUR_CHAR_CODE('noti'),                /** see notification flags below */
  kSRCallBackParam = FOUR_CHAR_CODE('call'), /** type SRCallBackParam */
  kSRSearchStatusParam = FOUR_CHAR_CODE('stat'), /** see status flags below */
  kSRAutoFinishingParam = FOUR_CHAR_CODE(
      'afin'), /** automatic finishing applied on LM for search */
  kSRForegroundOnly =
      FOUR_CHAR_CODE('fgon'), /** Boolean. Default is true. If true, client
                                 recognizer only active when in foreground.   */
  kSRBlockBackground = FOUR_CHAR_CODE(
      'blbg'), /** Boolean. Default is false. If true, when client recognizer in
                  foreground, rest of LMs are inactive.    */
  kSRBlockModally = FOUR_CHAR_CODE(
      'blmd'), /** Boolean. Default is false. When true, this client's LM is
                  only active LM; all other LMs are inactive. Be nice, don't be
                  modal for long periods! */
  kSRWantsResultTextDrawn =
      FOUR_CHAR_CODE('txfb'), /** Boolean. Default is true. If true, search
                                 results are posted to Feedback window */
  kSRWantsAutoFBGestures = FOUR_CHAR_CODE(
      'dfbr'), /** Boolean. Default is true. If true, client needn't call
                  SRProcessBegin/End to get default feedback behavior */
  kSRSoundInVolume =
      FOUR_CHAR_CODE('volu'), /** short in [0..100] log scaled sound input
                                 power. Can't set this property */
  kSRReadAudioFSSpec = FOUR_CHAR_CODE(
      'aurd'), /** *FSSpec. Specify FSSpec where raw audio is to be read (AIFF
                  format) using kSRCanned22kHzSpeechSource. Reads until EOF */
  kSRCancelOnSoundOut = FOUR_CHAR_CODE(
      'caso'), /** Boolean: Default is true.  If any sound is played out during
                  utterance, recognition is aborted. */
  kSRSpeedVsAccuracyParam =
      FOUR_CHAR_CODE('sped') /** SRSpeedSetting between 0 and 100 */
};
...
enum {
  kSRListenKeyMode = FOUR_CHAR_CODE(
      'lkmd'), /** short: either kSRUseToggleListen or kSRUsePushToTalk */
  kSRListenKeyCombo =
      FOUR_CHAR_CODE('lkey'), /** short: Push-To-Talk key combination; high byte
                                 is high byte of event->modifiers, the low byte
                                 is the keycode from event->message */
  kSRListenKeyName =
      FOUR_CHAR_CODE('lnam'), /** Str63: string representing ListenKeyCombo */
  kSRKeyWord = FOUR_CHAR_CODE('kwrd'), /** Str255: keyword preceding spoken
                                          commands in kSRUseToggleListen mode */
  kSRKeyExpected =
      FOUR_CHAR_CODE('kexp') /** Boolean: Must the PTT key be depressed or the
                                key word spoken before recognition can occur? */
};
...
enum {
  ...
  kSRSearchWaitForAllClients =
      1L << 2, /** search is suspended waiting on all clients' input */
  kSRMustCancelSearch =
      1L << 3, /** something has occurred (sound played, non-speech detected)
                  requiring the search to abort */
...
enum {
  kSRTEXTFormat =
      FOUR_CHAR_CODE('TEXT'), /** raw text in user supplied memory */
  kSRPhraseFormat =
      FOUR_CHAR_CODE('lmph'), /** SRPhrase containing result words */
  kSRPathFormat =
      FOUR_CHAR_CODE('lmpt'), /** SRPath containing result phrases or words */
  kSRLanguageModelFormat =
      FOUR_CHAR_CODE('lmfm') /** top level SRLanguageModel for post parse */
};

/** SRLanguageObject Family Properties */
enum {
  kSRSpelling =
      FOUR_CHAR_CODE('spel'), /** spelling of a SRWord or SRPhrase or SRPath, or
                                 name of a SRLanguageModel */
  kSRLMObjType = FOUR_CHAR_CODE(
      'lmtp'), /** Returns one of SRLanguageObject Types listed below */
  kSRRefCon = FOUR_CHAR_CODE('refc'), /** 4 bytes of user storage */
  kSROptional = FOUR_CHAR_CODE(
      'optl'), /** Boolean -- true if SRLanguageObject is optional    */
  kSREnabled =
      FOUR_CHAR_CODE('enbl'), /** Boolean -- true if SRLanguageObject enabled */
  kSRRepeatable = FOUR_CHAR_CODE(
      'rptb'), /** Boolean -- true if SRLanguageObject is repeatable */
  kSRRejectable = FOUR_CHAR_CODE(
      'rjbl'), /** Boolean -- true if SRLanguageObject is rejectable
                  (Recognition System's kSRRejectedWord */
  /**       object can be returned in place of SRLanguageObject with this
     property)   */
  kSRRejectionLevel =
      FOUR_CHAR_CODE('rjct') /** SRRejectionLevel between 0 and 100 */
};

/** LM Object Types -- returned as kSRLMObjType property of language model
 * objects */
enum {
 ...
 *    \non_carbon_cfm   in SpeechRecognitionLib 1.0 and later
