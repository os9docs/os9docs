/**
    ...
    \brief   Mixed Mode Manager Interfaces.
...
#define BUILD_FAT_ROUTINE_DESCRIPTOR(m68kProcInfo, m68kProcPtr,                \
                                     powerPCProcInfo, powerPCProcPtr)          \
  {                                                                            \
    _MixedModeMagic,               /** Mixed Mode A-Trap */                    \
        kRoutineDescriptorVersion, /** version */                              \
        kSelectorsAreNotIndexable, /** RD Flags - not dispatched */            \
...
enum {
  ...
  kStackParameterMask =
      (long)0xFFFFFFC0, /** Register Result Location offsets & widths */
  kRegisterResultLocationWidth = 5,
  kRegisterResultLocationPhase =
      (kCallingConventionWidth +
       kResultSizeWidth), /** Register Parameter offsets & widths */
  ...
  kRegisterParameterWhichWidth =
      3, /** Dispatched Stack Routine Selector offsets & widths */
  kDispatchedSelectorSizeWidth = 2,
  kDispatchedSelectorSizePhase =
      (kCallingConventionWidth +
       kResultSizeWidth), /** Dispatched Stack Routine Parameter offsets */
  kDispatchedParameterPhase =
      (kCallingConventionWidth + kResultSizeWidth +
       kDispatchedSelectorSizeWidth), /** Special Case offsets & widths */
...
/**
    ...
    When calling these routine from classic 68k code there are two possible
   intentions.

    The first is source compatibility with code ported to CFM (either PowerPC or
   68k CFM). When the code is compiled for CFM the functions create routine
   descriptors that can be used by the mixed mode manager operating on that
   machine. When the code is compiled for classic 68k these functions do nothing
   so that the code will run on Macintoshes that do not have a mixed mode
   manager. The dual nature of these functions is achieved by turning the CFM
   calls into "no-op" macros for classic 68k: You can put "NewRoutineDescriptor"
   in your source, compile it for any runtime or instruction set architecture,
   and it will run correctly on the intended runtime/instruction platform. All
   without source changes and/or conditional source.

    The other intention is for code that "knows" that it is executing as classic
   68k runtime and is specifically trying to call code of another architecture
   using mixed mode. Since the routines were designed with classic <-> CFM
   source compatibility in mind this second case is treated special. For classic
   68k code to create routines descriptors for use by mixed mode it must call
   the "Trap" versions of the routines (NewRoutineDescriptorTrap). These
   versions are only available to classic 68k callers: rigging the interfaces to
   allow calling them from CFM code will result in runtime failure because no
   shared library implements or exports the functions.


    This almost appears seamless until you consider "fat" routine descriptors
   and the advent of CFM-68K. What does "fat" mean? CFM-68K is not emulated on
   PowerPC and PowerPC is not emulated on CFM-68K. It makes no sense to create a
   routine descriptor having both a CFM-68K routine and a PowerPC native routine
   pointer. Therefore "fat" is defined to be a mix of classic and CFM for the
   hardware's native instruction set: on PowerPC fat is classic and PowerPC
   native, on a 68k machine with CFM-68K installed fat is classic and CFM-68K.

    By definition fat routine descriptors are only constructed by code that is
   aware of the architecture it is executing as and that another architecture
   exists. Source compatibility between code intented as pure classic and pure
   CFM is not an issue and so NewFatRoutineDescriptor is not available when
   building pure classic code.

    NewFatRoutineDescriptorTrap is available to classic code on both PowerPC and
   CFM-68K. The classic code can use the code fragment manager routine
   "FindSymbol" to obtain the address of a routine in a shared library and then
   construct a routine descriptor with both the CFM routine and classic routine.
*/
 ...
 *    \non_carbon_cfm   in InterfaceLib 7.1 and later or as macro/inline
...
#ifdef __cplusplus
inline UniversalProcPtr NewRoutineDescriptor(ProcPtr theProc, ProcInfoType,
                                             ISAType) {
  return (UniversalProcPtr)theProc;
}
#else
#define NewRoutineDescriptor(theProc, theProcInfo, theISA)                     \
...
#if !TARGET_OS_MAC || !TARGET_RT_MAC_CFM
#ifdef __cplusplus
inline void DisposeRoutineDescriptor(UniversalProcPtr) {}
#else
...
NewFatRoutineDescriptor(ProcPtr theM68kProc, ProcPtr thePowerPCProc,
...
/**
    The "Trap" versions of the MixedMode calls are only for classic 68K clients
   that want to load and use CFM based code.
*/
#if CALL_NOT_IN_CARBON
/**
...
#if !TARGET_CPU_68K || TARGET_RT_MAC_CFM
/**
    CallUniversalProc is defined for all targets except classic 68k code.  This
   will catch accidental calls from classic 68K code that previously only showed
   up as linker errors.
*/
#if CALL_NOT_IN_CARBON
/**
 ...
 * SIZE_CODE(sizeof(Boolean))) | REGISTER_ROUTINE_PARAMETER(2, kRegisterA0,
...
/** * * * * * * * * * * * * *
 *   SIZE_CODE -     Return the size code for an object, given its size in
 * bytes. size - size of an object in bytes
 */
...
/** * * * * * * * * * * * * *
 *   STACK_ROUTINE_PARAMETER -   Return a parameter field of a ProcInfo, for a
 * simple, non-dispatched, stack based routine. whichParam - which parameter
 *     sizeCode - size code
 */
...
/** * * * * * * * * * * * * *
 *   DISPATCHED_STACK_ROUTINE_PARAMETER -    Return a parameter field of a
 * ProcInfo, for a dispatched, stack based routine.  The same macro is used
 * regardless of the type of dispatching. whichParam - which parameter sizeCode
 * - size code
 */
...
/** * * * * * * * * * * * * *
 *   DISPATCHED_STACK_ROUTINE_SELECTOR_SIZE -    Return a the selector size
 * field of a ProcInfo for a dispatched, stack based routine.  The same macro is
 * used regardless of the type of dispatching. sizeCode - size code
 */
#define DISPATCHED_STACK_ROUTINE_SELECTOR_SIZE(sizeCode)                       \
  ((ProcInfoType)(sizeCode) << kDispatchedSelectorSizePhase)

/** * * * * * * * * * * * * *
 *   REGISTER_RESULT_LOCATION -      Return the Result Location field of a
 * ProcInfo, given the location. whichReg - which register
 */
#define REGISTER_RESULT_LOCATION(whichReg)                                     \
  ((ProcInfoType)(whichReg) << kRegisterResultLocationPhase)

/** * * * * * * * * * * * * *
 *   REGISTER_ROUTINE_PARAMETER -    Return a parameter field of a ProcInfo for
 * a register based routine.
 */
...
/** * * * * * * * * * * * * *
 *
 *    SPECIAL_CASE_PROCINFO -     Returns the procInfo constant for the
 * following special cases:
 *
 *     High Hook & Caret Hook -  (see I-379)
 *            C calling conventions, Rect on stack, pointer in A3, no return
 * value EOL Hook - (see VI-15-26) Register-based; inputs in D0, A3, A4; output
 * is Z flag of status register Width Hook - (see VI-15-27) Register-based;
 * inputs in D0, D1, A0, A3, A4; output in D1 NWidth Hook - (see VI-15-27)
 ...
 *         Register-based; inputs in D0, D1, D2, A0, A3, A4; outputs in D0, D1,
 * D2 FindWord -  (see VI-15-30) Register-based; inputs in D0, D2, A3, A4;
 * outputs in D0, D1 ADBRoutines - (see V-371) Register-based; inputs in A0, A1,
 * A2, D0; no outputs ProtocolHandler - (see II-326) Register-based; inputs in
 * A0, A1, A2, A3, A4, D1.w; output in Z SocketListener - (see II-329)
 *            Register-based; inputs in A0, A1, A2, A3, A4, D0.b, D1.w; output
 * in Z Reclac - (see I-391) Register-based; inputs in A3, D7; outputs in D2,
 * D3, D4 DoText - (see I-391) Register-based; inputs in A3, D3, D4, D7; outputs
 * in A0, D0 GNEFilterProc - (see tech note 85) Register & Stack Based; inputs
 * in A1, D0 & on the stack; outputs on the stack MenuBarHook - (see I-356)
...
#define CALL_ONE_PARAMETER_UPP(upp, procInfo, p1)                              \
  CallUniversalProc(upp, procInfo, (p1))
#define CALL_TWO_PARAMETER_UPP(upp, procInfo, p1, p2)                          \
  CallUniversalProc(upp, procInfo, (p1), (p2))
#define CALL_THREE_PARAMETER_UPP(upp, procInfo, p1, p2, p3)                    \
  CallUniversalProc(upp, procInfo, (p1), (p2), (p3))
#define CALL_FOUR_PARAMETER_UPP(upp, procInfo, p1, p2, p3, p4)                 \
  CallUniversalProc(upp, procInfo, (p1), (p2), (p3), (p4))
#define CALL_FIVE_PARAMETER_UPP(upp, procInfo, p1, p2, p3, p4, p5)             \
  CallUniversalProc(upp, procInfo, (p1), (p2), (p3), (p4), (p5))
#define CALL_SIX_PARAMETER_UPP(upp, procInfo, p1, p2, p3, p4, p5, p6)          \
  CallUniversalProc(upp, procInfo, (p1), (p2), (p3), (p4), (p5), (p6))
#define CALL_SEVEN_PARAMETER_UPP(upp, procInfo, p1, p2, p3, p4, p5, p6, p7)    \
  CallUniversalProc(upp, procInfo, (p1), (p2), (p3), (p4), (p5), (p6), (p7))
#define CALL_EIGHT_PARAMETER_UPP(upp, procInfo, p1, p2, p3, p4, p5, p6, p7,    \
                                 p8)                                           \
  CallUniversalProc(upp, procInfo, (p1), (p2), (p3), (p4), (p5), (p6), (p7),   \
                    (p8))
#define CALL_NINE_PARAMETER_UPP(upp, procInfo, p1, p2, p3, p4, p5, p6, p7, p8, \
                                p9)                                            \
  CallUniversalProc(upp, procInfo, (p1), (p2), (p3), (p4), (p5), (p6), (p7),   \
                    (p8), (p9))
#define CALL_TEN_PARAMETER_UPP(upp, procInfo, p1, p2, p3, p4, p5, p6, p7, p8,  \
                               p9, p10)                                        \
  CallUniversalProc(upp, procInfo, (p1), (p2), (p3), (p4), (p5), (p6), (p7),   \
                    (p8), (p9), (p10))
#define CALL_ELEVEN_PARAMETER_UPP(upp, procInfo, p1, p2, p3, p4, p5, p6, p7,   \
                                  p8, p9, p10, p11)                            \
  CallUniversalProc(upp, procInfo, (p1), (p2), (p3), (p4), (p5), (p6), (p7),   \
                    (p8), (p9), (p10), (p11))
#define CALL_TWELVE_PARAMETER_UPP(upp, procInfo, p1, p2, p3, p4, p5, p6, p7,   \
                                  p8, p9, p10, p11, p12)                       \
  CallUniversalProc(upp, procInfo, (p1), (p2), (p3), (p4), (p5), (p6), (p7),   \
                    (p8), (p9), (p10), (p11), (p12))
#define CALL_THIRTEEN_PARAMETER_UPP(upp, procInfo, p1, p2, p3, p4, p5, p6, p7, \
...
#define CALL_FIVE_PARAMETER_UPP(upp, procInfo, p1, p2, p3, p4, p5)             \
  (*(upp))((p1), (p2), (p3), (p4), (p5))
#define CALL_SIX_PARAMETER_UPP(upp, procInfo, p1, p2, p3, p4, p5, p6)          \
  (*(upp))((p1), (p2), (p3), (p4), (p5), (p6))
#define CALL_SEVEN_PARAMETER_UPP(upp, procInfo, p1, p2, p3, p4, p5, p6, p7)    \
  (*(upp))((p1), (p2), (p3), (p4), (p5), (p6), (p7))
#define CALL_EIGHT_PARAMETER_UPP(upp, procInfo, p1, p2, p3, p4, p5, p6, p7,    \
                                 p8)                                           \
  (*(upp))((p1), (p2), (p3), (p4), (p5), (p6), (p7), (p8))
#define CALL_NINE_PARAMETER_UPP(upp, procInfo, p1, p2, p3, p4, p5, p6, p7, p8, \
                                p9)                                            \
  (*(upp))((p1), (p2), (p3), (p4), (p5), (p6), (p7), (p8), (p9))
#define CALL_TEN_PARAMETER_UPP(upp, procInfo, p1, p2, p3, p4, p5, p6, p7, p8,  \
                               p9, p10)                                        \
  (*(upp))((p1), (p2), (p3), (p4), (p5), (p6), (p7), (p8), (p9), (p10))
#define CALL_ELEVEN_PARAMETER_UPP(upp, procInfo, p1, p2, p3, p4, p5, p6, p7,   \
                                  p8, p9, p10, p11)                            \
