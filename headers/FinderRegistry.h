...
*/
enum { kAEFinderSuite = FOUR_CHAR_CODE('fndr') };
...
enum {
  cInternalFinderObject =
      FOUR_CHAR_CODE('obj ') /** cReference - used to distinguish objects used
                                inside the Finder only*/
};
...
*/
enum {
  /** We do not use class cItem from AERegistry.r. Instead our class Item is a
     cObject*/
  /**         cItem                        = 'citm',   // defined in
     AERegistry.r*/
  ...
  cContentSpace = FOUR_CHAR_CODE('dwnd'), /**          cWindow = 'cwin', //
                                             defined in AERegistry.r*/
  ...
  cUser = FOUR_CHAR_CODE('cuse'), /**         cApplication                  =
                                     'capp',     // defined in AERegistry.r*/
  ...
  cSpecialFolders =
      FOUR_CHAR_CODE('spfl'), /** For use by viewer search engines:*/
  cOnlineDisk = FOUR_CHAR_CODE('cods'),
  cOnlineLocalDisk = FOUR_CHAR_CODE('clds'),
  cOnlineRemoteDisk =
      FOUR_CHAR_CODE('crds'), /** Miscellaneous class definitions*/
...
enum {
  ...
  /**    pIsStationeryPad               = 'pspd',         // defined in
     AERegistry.r                */
  /**    pVersion                    = 'vers',       // defined in
     AERegistry.r*/
  pProductVersion = FOUR_CHAR_CODE('ver2')
};

/** Properties of class cAliasFile (subclass of cFile)*/
enum { pOriginalItem = FOUR_CHAR_CODE('orig') };
...
enum { pInternetLocation = FOUR_CHAR_CODE('iloc') };

/** Properties of class cSoundFile (subclass of cFile)*/
enum { pSound = FOUR_CHAR_CODE('snd ') };
...
/**
   ...
   compatability. To set the full range of Finder Preferences, use the
   Preferences object.
*/
enum {
  pShowFolderSize = FOUR_CHAR_CODE(
      'sfsz'), /** Moved to a per-folder basis in Finder 8.0 HIS*/
  pShowComment = FOUR_CHAR_CODE(
      'scom'), /** Moved to a per-folder basis in Finder 8.0 HIS*/
  pShowDate = FOUR_CHAR_CODE(
      'sdat'), /** Moved to a per-folder basis in Finder 8.0 HIS*/
  pShowCreationDate = FOUR_CHAR_CODE(
      'scda'), /** Moved to a per-folder basis in Finder 8.0 HIS*/
  pShowKind = FOUR_CHAR_CODE(
      'sknd'), /** Moved to a per-folder basis in Finder 8.0 HIS*/
  pShowLabel = FOUR_CHAR_CODE(
      'slbl'), /** Moved to a per-folder basis in Finder 8.0 HIS*/
  pShowSize = FOUR_CHAR_CODE(
      'ssiz'), /** Moved to a per-folder basis in Finder 8.0 HIS*/
  pShowVersion = FOUR_CHAR_CODE(
      'svrs'), /** Moved to a per-folder basis in Finder 8.0 HIS*/
  ...
  pListViewIconSize = FOUR_CHAR_CODE(
      'lvis'), /** Moved to a per-folder basis in Finder 8.0 HIS*/
  pGridIcons = FOUR_CHAR_CODE(
      'fgrd'), /** Moved to a per-folder basis in Finder 8.0 HIS*/
  pStaggerIcons =
      FOUR_CHAR_CODE('fstg'), /** No longer part of the Finder 8.0 HIS*/
...
enum {
  ...
  pPreviousView =
      FOUR_CHAR_CODE('svew'), /**    pSelection                    = 'sele', //
                                 defined in AERegistry.r*/
...
enum { pStartupDisk = FOUR_CHAR_CODE('sdsk'), pTrash = FOUR_CHAR_CODE('trsh') };
...
enum { pWarnOnEmpty = FOUR_CHAR_CODE('warn') };
...
enum {
  /**    pShowFolderSize                   = 'sfsz',         // defined above
     for Views CP*/
  /**    pShowComment                = 'scom',      // defined above for Views
     CP*/
  pShowModificationDate = pShowDate, /** pShowDate defined above for Views CP*/
  /**    pShowKind                    = 'sknd',        // defined above for
     Views CP*/
  /**    pShowLabel                    = 'slbl',         // defined above for
     Views CP*/
  /**    pShowSize                    = 'ssiz',        // defined above for
     Views CP*/
  /**    pShowVersion                = 'svrs',      // defined above for Views
     CP*/
  /**    pShowCreationDate             = 'scda',      // Removed from Finder 8.0
     HIS*/
  /**    pShowFileType                 = 'sfty',       // Removed from
     Finder 8.0 HIS*/
  /**    pShowFileCreator               = 'sfcr',         // Removed from
     Finder 8.0 HIS*/
  /**    pListViewIconSize             = 'lvis',      // defined above for Views
     CP*/
  /**    pGridIcons                    = 'fgrd',         // defined above for
     Views CP*/
  /**    pStaggerIcons                 = 'fstg',       // defined above for
     Views CP*/
  /**    pViewFont                    = 'vfnt',        // defined above for
     Views CP*/
  /**    pViewFontSize                 = 'vfsz',       // defined above for
     Views CP*/
  pUseRelativeDate = FOUR_CHAR_CODE(
      'urdt'), /** Moved to a per-folder basis in Finder 8.0 HIS*/
  ...
  pDefaultListViewIconSize =
      FOUR_CHAR_CODE('lisz'), /** old use of this name is now pIconSize*/
...
/**  #define pObject                                 cObject         // defined
 * above*/
...
enum {
  /**    pName                      = 'pnam',         // defined in
     AERegistry.r*/
  pFile = cFile, /**    pCreatorType                = 'fcrt',      // defined
                    above*/
  /**    pFileType                    = 'asty',        // defined above*/
  /**    pIsFrontProcess                   = 'pisf',         // defined in
     AERegistry.r*/
  /**    pAppPartition                 = 'appt',       // defined above*/
  pPartitionSpaceUsed = FOUR_CHAR_CODE('pusd'), /**    pIsScriptable = 'isab',
                                                   // defined in AERegistry.r*/
  /**    pVisible                    = 'pvis'      // defined in AERegistry.r*/
...
enum { pDeskAccessoryFile = cDeskAccessoryFile };

/** Properties of class cApplicationProcess (subclass of cProcess)*/
enum { pApplicationFile = cApplicationFile };
...
/**
   ...
   NOTE: properties for the special folders must match their respective
   kXXXFolderType constants
*/
enum {
  pAboutMacintosh = FOUR_CHAR_CODE('abbx'),
  pAppleMenuItemsFolder = FOUR_CHAR_CODE('amnu'), /** kAppleMenuFolderType*/
  /**    pClipboard                    = 'pcli',         // defined in
     AERegistry.r*/
  ...
  pFontsFolderPreAllegro =
      FOUR_CHAR_CODE('ffnt'), /** DO NOT USE THIS - FOR BACKWARDS COMPAT ONLY*/
  /**    pIsFrontProcess                   = 'pisf',         // defined in
     AERegistry.r*/
  /**    pInsertionLoc                 = 'pins',       // defined in
     AERegistry.r*/
  ...
  /**    pUserSelection                  = 'pusl',        // defined in
     AERegistry.r*/
  ...
  /**    pVersion                    = 'vers',       // defined in
     AERegistry.r*/
  pViewPreferences = FOUR_CHAR_CODE('pvwp'), /**    pVisible = 'pvis',       //
                                                defined in AERegistry.r*/
  pStartingUp = FOUR_CHAR_CODE('awak') /** private property to tell whether the
                                          Finder is fully up and running*/
};
...
/**
  ...
  enum {
    pShowFolderSize                         = 'sfsz',           // defined above
  for Views CP pShowComment                            = 'scom',           //
  defined above for Views CP pShowModificationDate                   =
  pShowDate,            // pShowDate defined above for Views CP pShowKind =
  'sknd',           // defined above for Views CP pShowLabel = 'slbl', //
  defined above for Views CP pShowSize                               = 'ssiz',
  // defined above for Views CP pShowVersion                            =
  'svrs',           // defined above for Views CP pShowCreationDate = 'scda', //
  defined in cPreferencesWindow pShowFileType                           =
  'sfty',           // defined in cPreferencesWindow pShowFileCreator = 'sfcr',
  // defined in cPreferencesWindow pListViewIconSize                       =
  'lvis',           // defined above for Views CP pGridIcons = 'fgrd', //
  defined above for Views CP pStaggerIcons                           = 'fstg',
  // defined above for Views CP pViewFont                               =
  'vfnt',           // defined above for Views CP pViewFontSize = 'vfsz', //
  defined above for Views CP pUseRelativeDate                        = 'urdt',
  // defined in cPreferencesWindow pDelayBeforeSpringing                   =
  'dela',           // defined in cPreferencesWindow pShowMacOSFolder = 'sosf',
  // defined in cPreferencesWindow pUseShortMenus                          =
  'usme',           // defined in cPreferencesWindow pUseCustomNewMenu = 'ucnm',
  // defined in cPreferencesWindow pShowDesktopInBackground                =
  'sdtb',           // defined in cPreferencesWindow pActivateDesktopOnClick =
  'adtc',           // defined in cPreferencesWindow pLabel1 = 'lbl1', //
  defined in cPreferencesWindow pLabel2                                 =
  'lbl2',           // defined in cPreferencesWindow pLabel3 = 'lbl3', //
  defined in cPreferencesWindow pLabel4                                 =
  'lbl4',           // defined in cPreferencesWindow pLabel5 = 'lbl5', //
  defined in cPreferencesWindow pLabel6                                 =
  'lbl6',           // defined in cPreferencesWindow pLabel7 = 'lbl7', //
  defined in cPreferencesWindow pWindow                                 =
  cWindow           // defined above
  };
*/
...
enum { enumArrangement = FOUR_CHAR_CODE('earr') };
...
enum {
  typeIconFamily =
      cIconFamily, /** An AEList of typeIconAndMask, type8BitIcon, & c.*/
...
enum { formAlias = typeAlias, formCreator = pFileCreator };
...
enum {
  ...
  errFinderAppFolderProtected =
      -15276, /** used by General controls when folder protection is on*/
  errFinderSysFolderProtected =
      -15277, /** used by General controls when folder protection is on*/
  ...
  errFinderVolumeNotFound =
      -15283, /** more descriptive than what we get with nsvErr*/
  errFinderLockedItemsInTrash =
      -15284, /** there are some locked items in the trash*/
  errFinderOnlyLockedItemsInTrash =
      -15285, /** all the items (except folders) in the trash are locked*/
  errFinderProgramLinkingMustBeOn = -15286,
  errFinderWindowMustBeButtonView = -15287,
  errFinderBadPackageContents =
      -15288, /** something is wrong within the package   */
  errFinderUnsupportedInsidePackages =
      -15289, /** operation cannot be used on items within a package     */
  errFinderCorruptOpenFolderList =
      -15290, /** was -15276 in Finder 8.6 and earlier, but that conflicted with
                 General Controls*/
  errFinderNoInvisibleFiles =
      -15291, /** was -15277 in Finder 8.6 and earlier, but that conflicted with
                 General Controls*/
  errFinderCantDeleteImmediately =
      -15292, /** cannot delete immediately via scripting*/
