...
/**
   ...
   drives that depend on a network connection. � A drive that should never be
   written to but is safe for read-only file mapping should return
   kAllowVMReadOnlyMask. Examples of this are WORM drives where each write eats
   ...
   that don't allow manual eject and don't use a network connection.
   ...
   kAllowVMNoneMask, kAllowVMReadOnlyMask, and kAllowVMReadWriteMask (i.e.,
   setting only kAllowVMWriteBit is not valid).

    Important: All bits not defined here are reserved and should be set to zero
   ...
   the kdgVMOptions response. If a driver supports the kdcVMOptions Driver
   Configure _Control call, but is asked to set an option bit that it doesn't
   support (for example, if a read-only device is asked to set the
   kAllowVMWriteBit), it should return paramErr.
*/
...
If driver supports Driver Gestalt, the Alias Mgr holds the drive information
...
/**
   ...
   to a kdgMediaInfo DriverGestalt request. This allows a disk driver to tell
   callers the physical block size, the number of blocks that are of that size,
   and the media type for a given device.
...
/**
   ...
   floppy disks and large capacity removables, the driver would return that 2
...
/**
   ...
   boot support for the driver/device.  The kdgOpenFirmwareBootingSupport is
   used during the boot process to allow booting from partitions that Open
   Firmware may not be able to read directly (i.e. Disk Array partitions,
   encrypted partitions, compressed partitions, etc..)

    The driver will return the bootPartitionQualifier value in the
   driverGestaltResponse field of the DriverGestaltParam and will return the
   exact partition map entry for the boot partition in the
   driverGestaltResponse1 field of the DriverGestaltParam if
