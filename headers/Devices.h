...
#pragma pack(2)
  ...
  {
      chooserInitMsg = 11,                                                            /** user */
      selected this device package newSelMsg = 12                                     /** user */
      made new device selections fillListMsg = 13                                     /** the */
      device list with choices getSelMsg = 14                                         /** one */
                                           or more choices as selected selectMsg = 15 /** user */
      made a selection deselectMsg = 16                                               /** user */
      canceled a selection terminateMsg = 17                                          // allows device package to clean up     buttonMsg = 19       // the user selected a button
  };
  ...
  {
      goodbye = -1,                  /** being */
      reinitialized killCode = 1     /** requested */
      accEvent = 64                  /** an */
      event accRun = 65              /** for */
      periodic action accCursor = 66 /** cursor */
      shape accMenu = 67             /** menu */
      item accUndo = 68              /** undo */
      command accCut = 70            /** cut */
      command accCopy = 71           /** copy */
      command accPaste = 72          // handle paste command     accClear = 73   // handle clear command
  };
  ...
  {
        ioInProgress = 1,  /** value */
of ioResult while I/O is pending     aRdCmd  =  2        /** byte */
of ioTrap for Read calls     aWrCmd  =  3        /** byte */
of ioTrap for Write calls     asyncTrpBit  =  10 // trap word modifier     noQueueBit = 9    // trap word modifier
  };
  ...
  {                 // KillIO requested
                    ...
                    // run-time flags // handle paste command
    {
        ...
        dVMImmuneBit = 0,                  /** does */
        not need VM protection dOpened = 5 /** is */
        open dRAMBased = 6                 // dCtlDriver is a handle (1) or pointer (0)     drvrActive = 7    // driver is currently processing a request
    };

    // Control/Status Call Codes
    // drvStsCode, ejectCode and tgBuffCode are now defined in Disks.h/p/a
    dVMImmuneMask = 0x0001,                     /** does */
    not need VM protection dOpenedMask = 0x0020 /** is */
    open dRAMBasedMask = 0x0040                 // dCtlDriver is a handle (1) or pointer (0)     drvrActiveMask = 0x0080 // driver is currently processing a request
  };
  ...
  typedef struct DRVRHeader DRVRHeader;    // set if driver responds to write requests
  typedef DRVRHeader *DRVRHeaderPtr;       // set if driver responds to control requests
  typedef DRVRHeaderPtr *DRVRHeaderHandle; // set if driver responds to status requests
  /**
  ...
  */
  ...
  enum
  {
    ...
    kStatusCommand = 5,
    kKillIOCommand = 6,
    kInitializeCommand = 7,                              /** driver */
        and device kFinalizeCommand = 8                  /** driver */
            and device kReplaceCommand = 9               /** an */
        old driver kSupersededCommand = 10               /** to */
        be replaced by a new driver kSuspendCommand = 11 // prepare driver to go to sleep    kResumeCommand = 12      // wake up sleeping driver
  };

  enum
  {
    /** more */
    IOCommandCode kPowerManagementCommand = 13 /* power management command supercedes
...
#endif
  OSErr
  PBOpenAsync(ParmBlkPtr paramBlock);
...
#endif
  OSErr
  PBOpenImmed(ParmBlkPtr paramBlock);
...
#endif
  OSErr
  PBCloseImmed(ParmBlkPtr paramBlock);
...
#endif
  OSErr
  PBReadImmed(ParmBlkPtr paramBlock);
...
#endif
  OSErr
  PBWriteImmed(ParmBlkPtr paramBlock);
...
#endif
  OSErr
  DriverInstall(DRVRHeaderPtr drvrPtr, short refNum);
...
#endif
  void
  IODone(DCtlPtr dce, OSErr ioResult);
...
#endif
  OSErr
  PBControlAsync(ParmBlkPtr paramBlock);
...
#endif
  OSErr
  PBControlImmed(ParmBlkPtr paramBlock);
...
#endif
  OSErr
  PBStatusSync(ParmBlkPtr paramBlock);
...
#endif
  OSErr
  PBStatusAsync(ParmBlkPtr paramBlock);
...
#endif
  OSErr
  PBStatusImmed(ParmBlkPtr paramBlock);
...
#endif
  OSErr
  PBKillIOSync(ParmBlkPtr paramBlock);
...
#endif
  OSErr
  PBKillIOAsync(ParmBlkPtr paramBlock);
...
#endif
  OSErr
  PBKillIOImmed(ParmBlkPtr paramBlock);
