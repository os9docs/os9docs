...
#pragma pack(2)
  ...
  enum
  {
    ...
    FOUR_CHAR_CODE('thme'),                         /** changed */
    (e.g.platinum to hi - tech) kAESystemFontChanged = FOUR_CHAR_CODE('sysf')
    // system font changed     kAESmallSystemFontChanged =
    FOUR_CHAR_CODE('ssfn'), // small system font changed     kAEViewsFontChanged = FOUR_CHAR_CODE('vfnt')
                            // views font changed
  };
...
#define kThemeAppearanceAquaGraphite \
  ...
  enum
  {
    ...
    kThemeBrushAlertBackgroundActive = 3 kThemeBrushAlertBackgroundInactive = 4,
    ...
    kThemeBrushListViewBackground = 10 kThemeBrushIconLabelBackground = 11,
  ...
  // theme to theme. You can use them instead of using direct RGB values.   enum
  {
  ...
  enum
  {
    ...
    kThemeCrossCursor = 5,
    kThemePlusCursor = 6,
    kThemeWatchCursor = 7, /** Animate */
    kThemeClosedHandCursor = 8 kThemeOpenHandCursor = 9,
    kThemePointingHandCursor = 10,
    kThemeCountingUpHandCursor = 11, /** Animate */
    ...
    /** Animate */
    kThemeResizeLeftCursor = 15 kThemeResizeRightCursor = 16,
  ...
  *at present.*/
  ...
  kThemeVariantNameTag  =  FOUR_CHAR_CODE('varn')
                                                // Str255    kThemeVariantBaseTintTag =
        FOUR_CHAR_CODE('tint'),                  /** (10.1 */
and later)    kThemeHighlightColorTag  =  FOUR_CHAR_CODE('hcol')
                                                // RGBColor    kThemeScrollBarArrowStyleTag =
        FOUR_CHAR_CODE('sbar'),                 // ThemeScrollBarArrowStyle    kThemeScrollBarThumbStyleTag =
        FOUR_CHAR_CODE('sbth'),                  
  kThemeSoundsEnabledTag  =  FOUR_CHAR_CODE('snds')
                                                // Boolean    kThemeDblClickCollapseTag = FOUR_CHAR_CODE('coll')
                                                // Boolean
      };
  ...
  kThemeDesktopPatternNameTag  =  FOUR_CHAR_CODE('patn')
                                                          // Str255    kThemeDesktopPatternTag =
    FOUR_CHAR_CODE('patt'),                                /** data> */
(flattened pattern)    kThemeDesktopPictureNameTag  =  FOUR_CHAR_CODE('dpnm')
                                                           
  kThemeDesktopPictureAliasTag  =  FOUR_CHAR_CODE('dpal')
                                                          // <alias handle>    kThemeDesktopPictureAlignmentTag =
    FOUR_CHAR_CODE('dpan'),                                /** (see */
the Picture Alignments below)    kThemeHighlightColorNameTag  =  FOUR_CHAR_CODE('hcnm')
  ...
  enum
  {
    ...
    kFitToScreen = 3,    /**< shrinks if necessary    kFillScreen = 4,     // messes up aspect ratio if necessary    kUseBestGuess = 5    /* heuristically determines the best way to display the picture based on picture and monitor sizes */
  };
  ...
  enum
  {
    kThemeCheckBoxClassicX = 0, // check box with an 'X'    kThemeCheckBoxCheckMark = 1
                                // check box with a real check mark
  };

  typedef UInt16 ThemeCheckBoxStyle;
  enum
  {
    kThemeScrollBarArrowsSingle = 0, // single arrow on each end    kThemeScrollBarArrowsLowerRight = 1
                                     // double arrows only on right or bottom
  };

  typedef UInt16 ThemeScrollBarArrowStyle;
  enum
  {
    kThemeScrollBarThumbNormal = 0, // normal, classic thumb size    kThemeScrollBarThumbProportional = 1
                                    // proportional thumbs
  };
  ...
  // NOTE ON TAB HEIGHT   /* Use the kThemeSmallTabHeightMax and kThemeLargeTabHeightMax when calculating the rects */
  ...
  {
      kThemeSmallTabHeight = 16, /** small */
      tabs protrude from frame.kThemeLargeTabHeight = 21
                                                      /** large */
                                                      tabs protrude from frame.kThemeTabPaneOverlap = 3
                                                                                                      /** tabs */
                                                                                                      overlap frame.kThemeSmallTabHeightMax = 19
      // small tab height + overlap    kThemeLargeTabHeightMax = 24
      // large tab height + overlap
  };
  ...
  enum
  { /** states */
    kThemeTrackActive = 0 kThemeTrackDisabled = 1,
  ...
  enum
  { /** states */
    (ignored unless track is active)kThemeLeftOutsideArrowPressed = 0x01 kThemeLeftInsideArrowPressed = 0x02,
  ...
  enum
  { /** direction */
    kThemeThumbPlain = 0 kThemeThumbUpward = 1,
  ...
  enum
  {
    kThemeTrackHorizontal = (1 << 0), /** is */
    drawn horizontally kThemeTrackRightToLeft = (1 << 1)
    /** progresses */
    from right to left kThemeTrackShowThumb = (1 << 2)
    // track's thumb should be drawn    kThemeTrackThumbRgnIsNotGhost =
    (1 << 3), // the provided thumbRgn should be drawn opaque, not as a ghost    kThemeTrackNoScrollBarArrows =
  ...
  {
    SInt32 viewsize; // current view range size     ThemeTrackPressState pressState;
                     // pressed parts state
  };
  typedef struct ScrollBarTrackInfo ScrollBarTrackInfo;
  struct SliderTrackInfo
  {
    ThemeThumbDirection thumbDir; // thumb direction     ThemeTrackPressState pressState;
                                  // pressed parts state
  };
  ...
  {
    ThemeTrackKind kind; // what kind of track this info is for     Rect bounds;
                         // track basis rectangle     SInt32 min;
                         // min track value     SInt32 max;
                         // max track value     SInt32 value;
                         // current thumb value     UInt32 reserved;

    ThemeTrackAttributes attributes; // various track attributes     ThemeTrackEnableState enableState;
                                     // enable state     UInt8 filler1;
  ...
  enum
  {
    ...
    (1 << 3),     /** can */
    zoom only horizontally kThemeWindowHasVerticalZoom = (1 << 4)
                                                         // window can zoom only vertically     kThemeWindowHasFullZoom =
                                                         kThemeWindowHasHorizontalZoom +
                                                         kThemeWindowHasVerticalZoom, /** zooms */
    in all directions kThemeWindowHasCloseBox = (1 << 5)
    /** has */
    a close box kThemeWindowHasCollapseBox = (1 << 6)
    /** has */
    a collapse box kThemeWindowHasTitleText = (1 << 7)
                                              /** has */
                                              a title /
                                              title icon kThemeWindowIsCollapsed = (1 << 8)
    // window is in the collapsed state     kThemeWindowHasDirty = (1 << 9)
  };
  ...
  enum
  {
    kThemeGrowLeft = (1 << 0), /** grow */
    to the left kThemeGrowRight = (1 << 1)
    /** grow */
    to the right kThemeGrowUp = (1 << 2)
    // can grow up     kThemeGrowDown = (1 << 3)
    // can grow down
  };
  ...
  enum
  {
    ...
    kThemeCheckBox = 1,
    kThemeRadioButton = 2,
    kThemeBevelButton = 3,  /** button */
(obsolete)     kThemeArrowButton  =  4 /* popup button without text (no label). See
                              ThemeButtonAdornment for glyphs. */
    kThemePopupButton = 5,  /** button */
    kThemeDisclosureButton  =  6
    kThemeIncDecButton = 7,         /** buttons */
 (no label)     kThemeSmallBevelButton  =  8   
 /** bevel */
button     kThemeMediumBevelButton  =  3  
 /** bevel */
button     kThemeLargeBevelButton  =  9   
 /** bevel */
button     kThemeListHeaderButton  =  10  
 /** button */
for top of list     kThemeRoundButton  =  11       
 /** button */
    kThemeLargeRoundButton  =  12  
 /** round */
button     kThemeSmallCheckBox  =  13     
 /** checkbox */
    kThemeSmallRadioButton  =  14  
 /** radio */
button     kThemeRoundedBevelButton  =  15
 /** bevel */
button     kThemeNormalCheckBox  =  kThemeCheckBox
  ...
  enum
  {
    ...
    kThemeAdornmentFocus = (1 << 2),   /** set, */
    draw focus kThemeAdornmentRightToLeft = (1 << 4)
    // if set, draw right to left label     kThemeAdornmentDrawIndicatorOnly =
...
{
  kThemeSoundNone = 0,
  kThemeSoundMenuOpen = FOUR_CHAR_CODE('mnuo'), /** sounds */
  ...
  kThemeSoundWindowClosePress = FOUR_CHAR_CODE('wclp'), /** sounds */
  ...
  kThemeSoundUtilWinClosePress =
      FOUR_CHAR_CODE('uclp'), /** window */
  sounds kThemeSoundUtilWinCloseEnter = FOUR_CHAR_CODE('ucle')
  ...
  kThemeSoundWindowOpen =
      FOUR_CHAR_CODE('wopn'), /** close */
      and zoom action kThemeSoundWindowClose = FOUR_CHAR_CODE('wcls')
  ...
  kThemeSoundDefaultButtonPress = FOUR_CHAR_CODE('dbtp'), /** button */
  ...
  kThemeSoundCancelButtonPress = FOUR_CHAR_CODE('cbtp'), /** button */
  ...
  kThemeSoundRadioPress = FOUR_CHAR_CODE('radp'), /** buttons */
  ...
  kThemeSoundScrollArrowPress = FOUR_CHAR_CODE('sbap'), /** bars */
  ...
  kThemeSoundSliderTrackPress = FOUR_CHAR_CODE('sltp')
      kThemeSoundBalloonOpen = FOUR_CHAR_CODE('blno'), /** balloons */
  kThemeSoundBalloonClose = FOUR_CHAR_CODE('blnc')
      kThemeSoundBevelPress = FOUR_CHAR_CODE('bevp'), /** buttons */
  ...
  kThemeSoundLittleArrowUpPress = FOUR_CHAR_CODE('laup'), /** Arrows */
  ...
  kThemeSoundPopupPress = FOUR_CHAR_CODE('popp'), /** Buttons */
  ...
  kThemeSoundDisclosurePress = FOUR_CHAR_CODE('dscp'), /** Buttons */
  ...
  kThemeSoundDragTargetHilite = FOUR_CHAR_CODE('dthi'), /** manager */
...
// tells us to ignore the offset field and instead simply center the width of thehandle on the window.
//  kThemePopupTabCenterOnOffset   /*      tells us to center the width of the handle around the value passed in *offset.*/

/* The Appearance Manager will try its best to accomodate the requested *placement, but may move the handle slightly to make it fit correctly. */
enum
{
 ...
 *    \mac_os_x         in version 10.0 and later
 */
void InvokeThemeTabTitleDrawUPP(const Rect *bounds, ThemeTabStyle style,
 ...
 *    \mac_os_x         in version 10.0 and later
 */
void InvokeThemeEraseUPP(const Rect *bounds, UInt32 eraseData, SInt16 depth,
 ...
 *    \mac_os_x         in version 10.0 and later
 */
void InvokeThemeButtonDrawUPP(const Rect *bounds, ThemeButtonKind kind,
 ...
 *    \mac_os_x         in version 10.0 and later
 */
void InvokeWindowTitleDrawingUPP(const Rect *bounds, SInt16 depth,
 ...
 *    \mac_os_x         in version 10.0 and later
 */
void InvokeMenuTitleDrawingUPP(const Rect *inBounds, SInt16 inDepth,
 ...
 *    \mac_os_x         in version 10.0 and later
 */
void InvokeMenuItemDrawingUPP(const Rect *inBounds, SInt16 inDepth,
                              Boolean inIsColorDevice, SInt32 inUserData,
 ...
 *      text mode
 *      pattern origin
 * Get/SetThemeDrawingState may save and restore additional port state in the future, but you can rely on them to always save at least this port state. */

/**
 * NormalizeThemeDrawingState() *

        *    \non_carbon_cfm in AppearanceLib 1.1 and
    later
        *    \carbon_lib in CarbonLib 1.0 and
    later
        *    \mac_os_x in version 10.0 and
...
{
    appearanceBadBrushIndexErr = themeInvalidBrushErr, /** index */
    invalid appearanceProcessRegisteredErr = themeProcessRegisteredErr
