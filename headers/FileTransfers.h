...
#pragma pack(2)
  ...
  enum
  {
    ...
    ftReadOpenFile = 0 /** = */
    forkFlags,
    buffer = pblock from PBGetFInfo ftReadDataFork = 1 ftReadRsrcFork = 2,
    ftReadAbort = 3,
    ftReadComplete = 4,
    ftReadSetFPos = 6, /** = */
    forkFlags,
    buffer = pBlock same as PBSetFPos ftReadGetFPos = 7 // count = forkFlags buffer = pBlock same as PBGetFPos
  };
  ...
  enum
  {
    ...
    ftWriteOpenFile = 0 /** = */
    forkFlags,
    buffer = pblock from PBGetFInfo ftWriteDataFork = 1 ftWriteRsrcFork = 2,
    ftWriteAbort = 3,
    ftWriteComplete = 4,
    ftWriteFileInfo = 5,
    ftWriteSetFPos = 6, /** = */
    forkFlags,
    buffer = pBlock same as PBSetFPos ftWriteGetFPos = 7 // count = forkFlags buffer = pBlock same as PBGetFPos
  };

  enum
  {

    fork flags ftOpenDataFork = 1 ftOpenRsrcFork = 2
  };
...
#define NewFileTransferDefUPP(userRoutine)                                     \
  ...
  }; // pascal 2_bytes Func(4_bytes, 4_bytes, 4_bytes, 2_bytes) #ifdef __cplusplus
