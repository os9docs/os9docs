 ...
 *
 *  Availability:
 *    \non_carbon_cfm   in InputSprocketLib 1.1 and later
...
/**
 *
 * ISpElement_NewVirtualFromNeeds(UInt32 count, ISpNeeds *needs,
 ...
 *
 * ISpSuspend turns all devices off and allocates memory so that the state may
 * be later resumed. ISpResume resumes to the previous state of the system after
 * a suspend call.
 *
 * Return Codes
 * memFullErr
 ...
 *
 * These will extract as many device references from the system wide list as
 * will fit in your buffer.
 *
 * inBufferCount - the size of your buffer (in units of
 * sizeof(ISpDeviceReference)) this may be zero buffer - a pointer to your
 * buffer outCount - contains the number of devices in the system
 *
 * ISpDevices_ExtractByClass extracts and counts devices of the specified
 * ISpDeviceClass ISpDevices_ExtractByIdentifier extracts and counts devices of
 * the specified ISpDeviceIdentifier
 *
 * Return Codes
 * paramErr
 *
...
/**
 * ISpDevices_ActivateClass, ISpDevices_DeactivateClass, ISpDevices_Activate,
 * ISpDevices_Deactivate, ISpDevice_IsActive
 *
 ...
 * inDevicesToActivate/inDevicesToDeactivate - a pointer to a block of memory
 * contains the devices references
 *
...
/**
 *
 * takes an ISpElementReference and returns the group that it is in or 0 if
 * there is no group
 *
 * Return Codes
 * paramErr
...
/**
 *
 * takes an ISpElementReference and gives the ISpElementInfo for that Element.
 * This is the the set of standard information.  You get ISpElementKind specific
 * information through ISpElement_GetConfigurationInfo.
 *
 * Return Codes
 ...
 *
 *
 * takes an ISpElementReference and gives the ISpElementKind specific
 * configuration information
 *
 * if buflen is not long enough to hold the information
 * ISpElement_GetConfigurationInfo will copy buflen bytes of the data into the
 * block of memory pointed to by configInfo and will return something error.
 *
 * Return Codes
 ...
 * This is a specialized version of ISpElement_GetComplexState that is only
 * appropriate for elements whose data fits in a signed 32 bit integer.
 *
 *
 *
 * Return Codes
 ...
 *
 * It takes in an element  reference and the buffer size of the
 * ISpElementEventPtr it will set wasEvent to true if there was an event and
 * false otherwise.  If there was not enough space to fill in the whole event
 * structure that event will be dequed, as much of the event as will fit in the
 * buffer will by copied and ISpElement_GetNextEvent will return an error.
 *
 * Return Codes
 * paramErr
 *
 ...
 * ISpElement_Flush
 *
 * It takes an ISpElementReference and flushes all the events on that element.
 * All it guaruntees is that any events that made it to this layer before the
 * time of the flush call will be flushed and it will not flush any events that
 * make it to this layer after the time when the call has returned. What happens
 * to events that occur during the flush is undefined.
 *
 *
 * Return Codes
 * paramErr
 ...
 * ISpElementList_New
 *
 * Creates a new element list and returns it in outElementList.  In count
 specifies
 * the number of element references in the list pointed to by inElements.  If
 inCount
 * is non zero the list is created with inCount elements in at as specified by
 the
 * inElements parameter.  Otherwise the list is created empty.
 *
 *
 * Return Codes
 * out of memory - If it failed to allocate the list because it was out of
 memory it will also set outElementList to 0
 ...
 *
 * ISpElementList_Extract will extract as many of the elements from an element
 * list as possible.  You pass in an element list, a pointer to an array of
 * element references and the number of elements in that array. It will return
 * how many items are in the element list in the outCount parameter and copy the
 * minimum of that number and the size of the array into the buffer.
 *
 * ByKind and ByLabel are the same except that they will only count and copy
 * element references to elements that have the specified kind and label.
 *
 * Return Codes
 * paramErr
 *
 ...
 *
 * It takes in an element list reference and the buffer size of the
 * ISpElementEventPtr it will set wasEvent to true if there was an event and
 * false otherwise.  If there was not enough space to fill in the whole event
 * structure that event will be dequed, as much of the event as will fit in the
 * buffer will by copied and ISpElementList_GetNextEvent will return an error.
 *
 * Return Codes
 * paramErr
 *
 ...
 *
 * It takes an ISpElementListReference and flushes all the events on that list.
 * All it guaruntees is that any events that made it to this layer before the
 * time of the flush call will be flushed and it will not flush any events that
 * make it to this layer after the time when the call has returned. What happens
 * to events that occur during the flush is undefined.
 ...
 *
 *  Availability:
 *    \non_carbon_cfm   in InputSprocketLib 1.2 and later
 *    \carbon_lib        not in Carbon, but InputSprocketLib is compatible with
 * Carbon \mac_os_x         not available
 */
OSStatus
 ...
 * ISpElement_New
 *
 * ISpElement_New takes an element definition struct and gives you back a
 * element reference. When you produce data you use that element reference to
 * give the data to the system.
 *
 * Return Codes
 * paramErr
 * memory error
 *
 * Special Considerations
 * The ISpElement_New function may move or purge memory.  Your application
 * should not call this function at interrupt time.
 *
 */
/**
 *  ISpElement_New()
 ...
 *
 * Special Considerations
 * The ISpElement_Dispose function may move or purge memory.  Your application
 * should not call this function at interrupt time.
 *
 */
/**
 ...
 * was produced.  If UpTime is not available you should fill time.lo with the
 ...
 *
 * ISpElement_PushComplexData is exactly like ISpElement_PushSimpleData except
 * that it is appropriate for times when your data does not fit into a signed 32
 * bit integer.
 *
 * Instead it takes the length of your data (which must match the datasize field
 * of your ISpElementDefinitionStruct) and a ptr to the devices state.
 *
 * Return Codes
 * paramErr
 *
 ...
 *
 *  Availability:
 *    \non_carbon_cfm   in InputSprocketDeferLib 1.3 and later
 *    \carbon_lib        not available
 *    \mac_os_x         not available
 */
OSErr
