/**
    ...
    \brief   NuBus card ROM Definitions.
...
enum {
  ...
  displayVideoAppleTFB =
      16843009L, /**Video with Apple parameters for TFB card.*/
  displayVideoAppleGM = 16843010L, /**Video with Apple parameters for GM card.*/
  networkEtherNetApple3Com =
      33620225L, /**Ethernet with apple parameters for 3-Comm card.*/
...
enum {
  ...
  f32BitMode = 2    /** set => a 32-bit address will be put into dctlDevBase (IM
                       Devices 2-54) */
};
...
enum {
  ...
  sVidAuxParams =
      123, /**more video info for Display Manager -- timing information*/
  ...
  fBuiltInDisplay =
      1, /**      1 - when set is built-in (in the box) display, else not*/
  fDefaultColor =
      2, /**      2 - when set display prefers multi-bit color, else gray*/
  fActiveBlack = 3, /**      3 - when set black on display must be written, else
                       display is naturally black*/
  fDimMinAt1 =
      4, /**      4 - when set should dim backlight to level 1 instead of 0*/
  fBuiltInDetach =
...
enum {
  ...
  TNTBoardID = 0x0670,     /**Board ID for TNT/Alchemy/Hipclipper CPUs (did Nano
                              just make this up?)*/
  ...
  SacSONIC32BoardID =
      0x004F,           /** CPU board types - <CatCPU> <Typ680x0> <DrSwMacCPU>*/
