/**
    ...
    \brief   CoreGraphics remote operation
...
#pragma options(!pack_enums)
     ...
     * of the number of rectangles in the refreshed areas, and a list of the
     * refreshed rectangles.  The rectangles are in global coordinates.
     *
     ...
     * passed to the callback function are cleared from the accumulated refreshed
     * area when the callback is made.
     *
     ...
     * Both the function and the userParameter must match the registered entry to be
     * removed.
     */
    /**
    ...
    /**
     * In some applications it may be preferable to have a seperate thread wait for
     * screen refresh data. This function should be called on a thread seperate from
     * the event processing thread. If screen refresh callback functions are
     * registered, this function should not be used. The mechanisms are mutually
     * exclusive.
     *
     * Deallocate screen refresh rects using CGReleaseScreenRefreshRects().
     *
     * Returns an error code if parameters are invalid or an error occurs in
     * retrieving dirty screen rects from the server.
     */
    /**
     ...
     * mouseCursorPosition should be the global coordinates the mouse is at for the
     * event. updateMouseCursor should be TRUE if the on-screen cursor should be
     * moved to mouseCursorPosition.
     *
     * Based on the values entered, the appropriate mouse-down, mouse-up,
     * mouse-move, or mouse-drag events are generated, by comparing the new state
     ...
     * 'buttonCount' boolean_t values indicating button state.  The first value
     * should reflect the state of the primary button on the mouse. The second
     * value, if any, should reflect the state of the secondary mouse button
     * (right), if any. A third value woule be the center button, and the remaining
     ...
     *
     * The current implemementation of the event system supports a maximum of three
     * wheels.
     *
     * The wheelCount parameter should be followed by 'wheelCount' 32 bit integer
     * values indicating wheel movements.  The first value should reflect the state
     * of the primary wheel on the mouse. The second value, if any, should reflect
     * the state of a secondary mouse wheel, if any.
     *
    ...
    /**
     * Set the period of time in seconds that local hardware events (keyboard and
     * mouse) are supressed after posting an event.  Defaults to 0.25 second.
     */
    /**
    ...
    /**
     * By default, the flags that indicate modifier key state (Command, Alt, Shift,
     * etc.) from the system's keyboard and from other event sources are ORed
     * together as an event is posted into the system, and current key and mouse
     * button state is considered in generating new events. This function allows
     * your application to enable or disable the merging of event state.  When
     * combining is turned off, the event state propagated in the events posted by
     * your app reflect state built up only by your app.  The state within your
     * app's generated event will not be combined with the system's current state,
     * so the system-wide state reflecting key and mouse button state will remain
     * unchanged
     *
     * When called with doCombineState equal to FALSE, this function initializes
     * local (per application) state tracking information to a state of all keys,
     * modifiers, and mouse buttons up.
     *
     * When called with doCombineState equal to TRUE, the current global state of
     * keys, modifiers, and mouse buttons are used in generating events.
     */
    /**
    ...
    /**
     * By default the system supresses local hardware events from the keyboard and
     * mouse during a short interval after a synthetic event is posted (see
     * CGSetLocalEventsSuppressionInterval()) and while a synthetic mouse drag
     * (mouse movement with the left/only mouse button down). Some classes of
     * applications may want to enable events from some of the local hardware. For
     * example, an app may want to post only mouse events, and so may wish to permit
     * local keyboard hardware events to pass through.
     *
     * This interface lets an app specify a state (event supression interval, or
     * mouse drag), and a mask of event categories to be passed through.
     */
     ...
     *
     * This will report mouse movement associated with the last mouse move or drag
     * event.
     *
    ...
    /**
     * Some classes of applications need to detect when the window server process
     * dies, or is not running.  The easiest way to do this is to use a
     * CFMachPortRef.
     *
     ...
     *
     * A program can register a callback function to use a CFMachPortRef to
     * determine when the CoreGraphics window server exits:
     *
     ...
     * which no window server is running, until the operating system starts a new
     * window server/loginwindow pair of processes.  This function will return NULL
     * until a new window server is running.
     *
     * Multiple calls to this function may return multiple CFMachPortRefs, each
     * referring to the same Mach port.  Multiple callbacks registered on multiple
     * CFMachPortRefs obtained in this way may fire in a nondetermanistic manner.
     *
     ...
     * CFMachPortIsValid(CFMachPortRef port) periodically to check if the port is
     * valid.
     */
