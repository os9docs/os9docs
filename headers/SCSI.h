...
#pragma pack(2)
  ...
  {
      scCommErr = 2,  /** error, */
operation timeout     scArbNBErr  =  3       /** timeout */
waiting for not BSY     scBadParmsErr  =  4    /** parameter */
or TIB opcode     scPhaseErr  =  5       /** bus */
not in correct phase for attempted operation     scCompareErr  =  6     /** compare */
error     scMgrBusyErr  =  7     /** Manager */
busy      scSequenceErr  =  8    /** operation */
is out of sequence     scBusTOErr  =  9      // CPU bus timeout     scComplPhaseErr = 10 // SCSI bus wasn't in Status phase
  };
  ...
  {                  // bad parameter or TIB opcode
                    ...
                    <pre>
                        A transfer instructions block tells the SCSI Manager what to do with
                            the data bytes transferred during the data phase.A transfer instruction block
                                contains a pseudo -
                    program consisting of a variable number of instructions;
           it's similar to a subroutine except that the instructions are provided and interpreted by the SCSI Manager itself.The instructions are of a fixed size
               and are of type SCSIInstr.Eight instructions are available;
           their operation codes are specified with the
               following predefined constants : scInc SCINC instruction
                                                            ...
                                                            scLoop SCLOOP instruction
                                                                scNOp SCNOP instruction
                                                                    scStop SCSTOP instruction
                                                                        A description of the instructions is given below : scOpcode = scInc scParam1 = buffer scParam2 = count
                                                                                                                                                                             The SCINC instruction moves count bytes to or
                                                                                                                                                                         from buffer,
                                                                                                                           incrementing
                                                                                                                               buffer by count when done.scOpcode = scNoInc scParam1 = buffer scParam2 = count
                                                                                                                                                                                                             The SCNOINC instruction moves count data bytes to or
                                                                                                                                                                                                         from buffer,
                                                                                                                           leaving
                                                                                                                               buffer unmodified.scOpcode = scAdd scParam1 = addr scParam2 = value
                                                                                                                           ...
                                                                                                                           pseudo - program execution resumes at the current address + relAddr.If the result is 0, pseudo - program execution resumes at the next instruction.RelAddr should be a signed multiple of the instruction size(10 bytes).For example, to loop to the immediately preceding instruction, the relAddr field would contain - 10. To loop forward by three instructions, it would contain 30. scOpcode = scNOp scParam1 = NIL scParam2 = NIL The SCNOP instruction does nothing.</ pre> * \copyright THINK Reference © 1991 - 1992 Symantec Corporation */
        struct SCSIInstr
    {
    ...
    {
        kDataOutPhase = 0, /** MSG, */
        C / D, I / O bits kDataInPhase = 1 kCommandPhase = 2,
        kStatusPhase = 3,
        kPhaseIllegal0 = 4,
        kPhaseIllegal1 = 5,
        kMessageOutPhase = 6,
        kMessageInPhase = 7,
        kBusFreePhase = 8, /** Phases */
        kArbitratePhase = 9 kSelectPhase = 10,
...
// SCSI Phases (used by SIMs to support the Original SCSI Manager
    ...
    \returns<pre> an error code indicating success or
    failure of the function.It will be one of : noErr(0) No error scCommErr(2) Breakdown // Additional Phases
    </ pre> * \note<pre>
    The error codes returned by SCSI Manager routines typically indicate only that a given operation h // Message In Phase with ACK hanging on the bus
    failure,
    another SCSI command needs to be sent asking the device what went
    wrong.</ pre>
  ...
  */
    OSErr
    SCSICmd(Ptr buffer, short count);
...
#define CallSCSICallbackProc(userRoutine, scsiPB) \
  ...
  enum
  {
    SCSINop = 0x00,  /** nothing */
                             SCSIExecIO  =  0x01        /** the */
specified IO                      SCSIBusInquiry  =  0x03    /** parameters */
for entire path of HBAs               SCSIReleaseQ  =  0x04      /** the */
frozen SIM queue for particular LUN       SCSIAbortCommand  =  0x10  /** the */
selected Control Block                 SCSIResetBus  =  0x11      /** the */
SCSI bus                             SCSIResetDevice  =  0x12  // Reset the SCSI device                          SCSITerminateIO = 0x13   // Terminate any pending IO
  };

  // Not available in Carbon on X   enum
  {
      SCSIGetVirtualIDInfo = 0x80,  /** out */
which bus old ID is on     SCSILoadDriver  =  0x82        /** a */
driver for a device ident                 SCSIOldCall  =  0x84           /** private */
call for old-API                SCSICreateRefNumXref  =  0x85  /** a */
DeviceIdent to drvr RefNum xref     SCSILookupRefNumXref  =  0x86  /** DeviceIdent */
to drvr RefNum xref     SCSIRemoveRefNumXref  =  0x87 // Remove a DeviceIdent to drvr RefNum xref     SCSIRegisterWithNewXPT =
  ...
  {
      handshakeDataLength = 8, /** data */
      length maxCDBLength = 16 // Space for the CDB bytes/pointer     vendorIDLength = 16      // ASCII string len for Vendor ID
  };
...
#define SCSIPBHdr            // Space for the CDB bytes/pointer
      ...
      short scsiReserved1;
      UInt16 scsiPBLength; // Define DeviceIdent structure
      UInt8 scsiReserved2;
  ...
  enum
  {
    scsiRequestInProgress = 1, // 1   = PB request is in progress                                         // Execution failed  00-2F     scsiRequestAborted =
        ...
        scsiErrorBase + 4,             /** = */
PB request completed with an err       scsiUnused05  =  scsiErrorBase + 5  /** = */
                           scsiUnused06  =  scsiErrorBase + 6  /** = */
                           scsiUnused07  =  scsiErrorBase + 7  /** = */
                           scsiUnused08  =  scsiErrorBase + 8 // -7928 =                            scsiUnableToTerminate =
        scsiErrorBase + 9,                             /** = */
Unable to Terminate I/O PB req            scsiSelectTimeout  =  scsiErrorBase + 10            /** = */
Target selection timeout     scsiCommandTimeout  =  scsiErrorBase + 11           /** = */
Command timeout     scsiIdentifyMessageRejected  =  scsiErrorBase + 12 // -7924 =     scsiMessageRejectReceived =
        ...
        scsiErrorBase + 16,             /** = */
Autosense: Request sense cmd fail      scsiUnused11  =  scsiErrorBase + 17 // -7919 =                            scsiDataRunError =
        scsiErrorBase + 18,                      /** = */
Data overrun/underrun error         scsiUnexpectedBusFree  =  scsiErrorBase + 19 // -7917 = Unexpected BUS free     scsiSequenceFailed =
        scsiErrorBase + 20, // -7916 = Target bus phase sequence failure      scsiWrongDirection =
        scsiErrorBase + 21,             /** = */
Data phase was in wrong direction      scsiUnused16  =  scsiErrorBase + 22 // -7914 =                            scsiBDRsent =
        scsiErrorBase + 23, // -7913 = A SCSI BDR msg was sent to target      scsiTerminated =
        scsiErrorBase + 24,            /** = */
PB request terminated by the host      scsiNoNexus  =  scsiErrorBase + 25 // -7911 = Nexus is not established     scsiCDBReceived =
  ...
  // New errors for SCSI Family           enum
  {
    scsiUnused74 = scsiErrorBase + 74,       /** = */
                             ...
                             scsiBadDataLength  =  scsiErrorBase + 76 // -7860 = a zero data length in PB     scsiPartialPrepared =
...
// ->   reserved for input                   
  ...
  {// -> Ptr to the next PB in linked cmd chain      
    scsiBusMDP = 0x80,        /** Modify */
Data Pointer message                   scsiBusWide32  =  0x40     /** 32 */
bit wide SCSI                        scsiBusWide16  =  0x20     /** 16 */
bit wide SCSI                        scsiBusSDTR  =  0x10       /** Sync */
Data Transfer Req message                  scsiBusLinkedCDB  =  0x08  /** linked */
CDBs                             scsiBusTagQ  =  0x02      // Supports tag queue message                        scsiBusSoftReset = 0x01  // Supports soft reset                             
};
                  ...
                  {                                 // (internal use, must be nil on entry)
                    ...
                    {                                // -> Device Identifier (bus+target+lun)
                      scsiOddDisconnectUnsafeRead1 = // -> Callback on completion function
                          0x0001,
                      /* Disconnects on od// -> assorted flags
                         blind reads */
                          // <> Ptr for driver private use
                          ...
                          scsiRequiresHandshake =
                              0x0008,              // <- which data types are supported by this SIM cause data corruption */
...
// <- additional BusInquiry PB len 
  ...
  kOrderedQueueTagMsg  =  0x22         // 0x22    kIgnoreWideResidueMsg = 0x23        // 0x23  
};
...
#e // Release SIM Queue PB 
  OSErr
  SCSIKillXPT(SIMInitInfo *parameterBlock);
// (internal use, must be nil on entry)
