...
/* ---------------------------------------------------------------------------
 */
// FORMATS FOR TABLE: lookup tables - used within various other tables enum {
...
/**// a simple trimmed array indexed by glyph code 
    A BinarySearchHeader defines the five standard fields needed to perform
   quick lookups in a lookup table (note that using UInt16s, and not ItemCounts
   or similar types, is important here, since these tables are in fonts, and the
    documented font formats specify 16-bit quantities).
*/
struct SFNTLookupBinarySearchHeader {
  UInt16 unitSize;      // size of a unit in bytes   UInt16 nUnits;        // number of units in table   UInt16 searchRange;   // (largest power of two <= nUnits) * unitSize   UInt16 entrySelector; // log2 (largest power of two <= nUnits)   UInt16 rangeShift; // (nUnits - largest power of two <= nUnits) * unitSize };
 ...
 * values */// size of a unit in bytes 
...
// A format 0 lookup table maps all glyphs in the font to lookup values 
   ...
   lookup values (format 2), or per-glyph lookup values (format 4). Since both
   formats use the same kind of data, only one unified set of segment-related
...
// The overall subtable header struct SFNTLookupTable {
  SFNTLookupTableFormat format;            // table format   SFNTLookupFormatSpecificHeader fsHeader; // format specific header };
...
/* ---------------------------------------------------------------------------
 */
// FORMATS FOR TABLE: 'lcar' // CONSTANTS enum {
  kLCARTag = 0x6C636172, // 'lcar'   kLCARCurrentVersion =
      0x00010000, // current version number for 'lcar' table   kLCARLinearFormat = 0,
...
// TYPES 
// Justification priority levels enum {
...
// TYPES 
   ...
   otherwise transformed, but rather which will be emplaced however many times
...
struct JustDirectionTable {
  UInt16 justClass;          // offset to state table (0=none)   UInt16 widthDeltaClusters; // offset to clusters   UInt16 postcomp;           // offset to postcomp table (0=none)   SFNTLookupTable lookup;
};
...
/* ---------------------------------------------------------------------------
 */
// FORMATS FOR // not really a UInt32; cast as ptr to appropriate action 
  kOPBDTag = 0x6F706264, // 'opbd'   kOPBDCurrentVersion = 0x00010000,
...
struct OpbdTable {// flags controlling grow case 
...
/* ---------------------------------------------------------------------------
 */
// FORMATS FOR TABLE: 'mort' // CONSTANTS enum {
// Overall structure of a postcompensation table is defined in PostcompTable. 
      0x00010000, // current version number for 'mort' table                   // Coverage masks   kMORTCoverVertical = 0x8000,
  kMORTCoverDescending = 0x4000,
  // action records here 
  kMORTCoverTypeMask = 0x000F, // Subtable types   kMORTRearrangementType = 0,
  ...
  kMORTMarkJustTableCountShift = 7, // JustTableIndex for marked character   kMORTCurrJustTableCountMask = 0x007F,
  kMORTCurrJustTableCountShift = 0, // JustTableIndex for current character   kMORTCurrInsertCountMask = 0x03E0,
  kMORTCurrInsertCountShift = 5, // count to insert after current glyphRec   kMORTMarkInsertCountMask = 0x001F,
...
struct MortChain {// Rearrangement subtable actions 
  MortSubtableMaskFlags default//    no action   
  // the subtables follow };//      Ax => xA    
typedef struct MortChain MortCh//      xD => Dx    
struct MortTable {//     AxD => DxA   
...
struct MorxChain {
  MortSubtableMaskFlags defaultFlags; // default flags for this chain   UInt32 length;                      // byte length of this chain   UInt32 nFeatures;                   // number of feature entries   UInt32 nSubtables;                  // number of subtables   MortFeatureEntry featureEntries[1];
...
/* ---------------------------------------------------------------------------
 */
// FORMATS FOR TABLE: 'prop' // CONSTANTS enum {
  kPROPTag = 0x70726F70, // 'prop'   kPROPCurrentVersion =
      0x00020000, // current version number for 'prop' table   kPROPPairOffsetShift = 8,
  kPROPPairOffsetSign = 7,
  kPROPIsFloaterMask = 0x8000, // glyph is floater   kPROPCanHangLTMask = 0x4000, // glyph can hang left/top   kPROPCanHangRBMask = 0x2000, // glyph can hang right/bottom   kPROPUseRLPairMask =
      0x1000, // if glyph lands in RL streak, use paired glyph   kPROPPairOffsetMask = 0x0F00,   // 4-bit signed offset to other pair member   kPROPRightConnectMask = 0x0080, // glyph connects to glyph on right   kPROPZeroReserved = 0x0060,     // must be zero   kPROPDirectionMask = 0x001F     // direction bits };

/* These are the Unicode direction classes (plus the Special European Number
 * class). */
enum {
  kPROPLDirectionClass = 0,  // Left-to-Right   kPROPRDirectionClass = 1,  // Right-to-Left   kPROPALDirectionClass = 2, // Right-to-Left Arabic Letter   kPROPENDirectionClass = 3, // European Number   kPROPESDirectionClass = 4, // European Number Seperator   kPROPETDirectionClass = 5, // European Number Terminator   kPROPANDirectionClass = 6, // Arabic Number   kPROPCSDirectionClass = 7, // Common Number Seperator   kPROPPSDirectionClass =
      8, // Paragraph Seperator (also referred to as Block Separator)   kPROPSDirectionClass = 9,   // Segment Seperator   kPROPWSDirectionClass = 10, // Whitespace   kPROPONDirectionClass = 11, // Other Neutral   kPROPSENDirectionClass =
      12, // Special European Number (not a Unicode class)   kPROPLREDirectionClass = 13, // Left-to-Right Embeding   kPROPLRODirectionClass = 14, // Left-to-Right Override   kPROPRLEDirectionClass = 15, // Right-to-Left Embeding   kPROPRLODirectionClass = 16, // Right-to-Left Override   kPROPPDFDirectionClass = 17, // Pop Directional Format   kPROPNSMDirectionClass = 18, // Non-Spacing Mark   kPROPBNDirectionClass = 19,  // Boundary Neutral   kPROPNumDirectionClasses =
...
/* ---------------------------------------------------------------------------
 */
// FORMATS FOR TABLE: 'kern' // CONSTANTS enum {
  kKERNTag = 0x6B65726E, // 'kern'   kKERNCurrentVersion = 0x00010000,
  kKERNVertical =
      ...
      0x2000, // set if this table contains variation kerning values   kKERNUnusedBits = 0x1F00, // UNUSED, MUST BE ZERO   kKERNFormatMask = 0x00FF  // format of this subtable };

enum {
  kKERNOrderedList = 0, // ordered list of kerning pairs   kKERNStateTable = 1,  // state table for n-way contextual kerning   kKERNSimpleArray = 2, // simple n X m array of kerning values   kKERNIndexArray = 3   // modifed version of SimpleArray };

// Message Type Flags enum {
  kKERNLineStart = 0x00000001,      // Array of glyphs starts a line   kKERNLineEndKerning = 0x00000002, // Array of glyphs ends a line   kKERNNoCrossKerning = 0x00000004, // Prohibit cross kerning   kKERNNotesRequested = 0x00000008, // Caller wants kerning notes   kKERNNoStakeNote =
...
/**// Left-to-Right 
    Kern offset table header.// Right-to-Left 
...
struct KernOffsetTable {// Arabic Number 
...
// Right-to-Left Embeding 
    ...
    offset table, the right-han// Pop Directional Format 
...
// Kern o// Number of Unicode directional types + Special European Number 
  UInt16 rowWidth;          // width, in bytes, of a row in the table   UInt16 leftOffsetTable;   // offset to left-hand offset table   UInt16 rightOffsetTable;  // offset to right-hand offset table   KernArrayOffset theArray; // offset to start of kerning array   UInt16 firstTable[1];     // first offset table starts here... };
...
// TYPES 
  ...
  UInt8 flags; // set to 0 for now   SInt16
      kernValue[1]; // actual kerning values reference by index in kernIndex   UInt8 leftClass[1];  // maps left glyph to offset into kern index   UInt8 rightClass[1]; // maps right glyph to offset into kern index   UInt8 kernIndex[1];  // contains indicies into kernValue };
...
// version 0 subtable header struct KernVersion0SubtableHeader {
  UInt16 version;          // kerning table version number   UInt16 length;           // length in bytes (including this header)   KernSubtableInfo stInfo; // sub-table info   KernFormatSpecificHeader fsHeader; // format specific sub-header };
...
// Overall Subtable header format struct KernSubtableHeader {
  SInt32 length;           // length in bytes (including this header)   KernSubtableInfo stInfo; // subtable info   SInt16 tupleIndex;       // tuple index for variation subtables   KernFormatSpecificHeader fsHeader; // format specific sub-header };
...
struct BslnFormat3Part {// state table for n-way contextual kerning 
 ...
 * baseline table. */// Array of glyphs starts a line 
union BslnFormatUnion {// Array of glyphs ends a line 
...
#endif // __SFNTLAYOUTTYPES__ // defines a single kerning pair of Glyphcodes  
// a single list entry 
// the kerning pair 
