...
enum {
  ...
  kLCARCurrentVersion =
      0x00010000, /* current version number for 'lcar' table */
  kLCARLinearFormat = 0,
...
/**
    A JUSTPCConditionalAddAction defines a glyph substitution and add action. If
...
/* A PCDuctilityAction defines a ductile axis along which the glyph will be
 * varied. */
...
/**
   ...
   are needed to fill the needed gap.
*/
...
/**
   ...
   justClass value (which is actually limited to 7 bits by the state table
...
enum {
  ...
  kMORXCurrentVersion =
      0x00020000, /* version number for current 'morx' table */
                  /* Coverage masks */
...
enum {
  ...
  kPROPCurrentVersion =
      0x00020000, /* current version number for 'prop' table */
  ...
  kPROPUseRLPairMask =
      0x1000, /* if glyph lands in RL streak, use paired glyph */
...
enum {
  ...
  kPROPPSDirectionClass =
      8, /* Paragraph Seperator (also referred to as Block Separator) */
  ...
  kPROPSENDirectionClass =
      12, /* Special European Number (not a Unicode class) */
...
enum {
  ...
  kTRAKCurrentVersion =
      0x00010000,        /* current version number for 'trak' table */
...
enum {
  ...
  kKERNVertical =
      0x8000, /* set if this table has vertical kerning information */
  kKERNResetCrossStream =
      0x8000, /* this value in a cross-stream table means reset to zero */
  kKERNCrossStream =
      0x4000, /* set if this table contains cross-stream kerning values */
  kKERNVariation =
      0x2000, /* set if this table contains variation kerning values */
...
enum {
  ...
  kKERNNoStakeNote =
      1, /* Indicates a glyph was involved in a kerning pair/group */
  kKERNCrossStreamResetNote =
      2, /* Indicates a return-to-baseline in cross-stream kerning */
  kKERNNotApplied =
...
struct KernOrderedListHeader {
  ...
  UInt16
      rangeShift;  /* (nPairs - largest power of two <= nPairs) * entry size */
...
struct KernIndexArrayHeader {
  ...
  SInt16
      kernValue[1]; /* actual kerning values reference by index in kernIndex */
...
enum {
  ...
  kBSLNCurrentVersion =
      0x00010000, /* current version number for 'bsln' table */
...
/**
   ...
   a single glyph to use and the set of control points in that glyph that
